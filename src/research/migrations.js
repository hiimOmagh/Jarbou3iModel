/* Jarbou3i Research Engine migrations v1.0.20. */
(function(global){
  'use strict';
  const root = global.Jarbou3iResearchModules = global.Jarbou3iResearchModules || {};
  const MIGRATION_VERSION = '1.0.20';
  const TARGET_VERSION = '1.0.20';
  ...
  function defaultReleaseNotes(){
    return {release_notes_version:TARGET_VERSION, generated_at:nowIso(), release_title:'v1.0.20 — Source Packet Template Browser QA + Copy Safety', release_type:'patch', runtime_capability_change:false, summary:'Adds local/manual source packet template presets without enabling live fetching, production OAuth, provider changes, backend changes, or storage changes.', notable_changes:['Added source packet template presets for common source types.','Kept template output as drafting scaffolds, not verification.','Added template reports to exports and migration defaults.','Kept runtime behavior and public-demo boundaries unchanged.'], unchanged_boundaries:['Manual/private mode remains the default.','No production OAuth path is enabled.','No new live source connector is enabled.','No provider key or raw token is exported.','No backend endpoint behavior is changed.'], publish_checklist:['Run no-browser CI gates.','Run hosted-demo browser evidence capture before publishing.','Verify GitHub Pages deployment uses the current package files.','Confirm release archive excludes generated and secret-bearing files.'], release_gate:'release_notes_polished'};
  }
