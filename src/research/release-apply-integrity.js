/* Jarbou3i Research Engine release apply integrity helpers v1.0.27. */
(function(global){
  'use strict';
  const root = global.Jarbou3iResearchModules = global.Jarbou3iResearchModules || {};
  const VERSION = '1.0.27';
  const BASE_VERSION = '1.0.26';
  const RELEASE_TITLE = 'v1.0.27 — Release Provenance Ledger Gate';
  const ARTIFACT_NAME = 'jarbou3i-research-engine-v1.0.27-release-provenance-ledger-gate-patch.zip';
  const APPLY_CHECKS = Object.freeze([
    {check_id:'base_version_known', label:'Patch is declared as changed-files-only over v1.0.26', required:true},
  ...
  function buildReleaseApplyIntegrity(input = {}, {version = VERSION, baseVersion = BASE_VERSION, now = nowIso(), artifactName = ARTIFACT_NAME, artifactSha256 = 'computed_after_packaging'} = {}){
    ...
    return {release_apply_integrity_version:version, generated_at:now, release_title:`v${version} — Release Provenance Ledger Gate`, integrity_stage:'release_apply_integrity_checked', release_type:'patch', patch_type:'changed_files_only', base_version:baseVersion, target_version:version, artifact_name:artifactName, artifact_sha256:artifactSha256, runtime_capability_change:false, provider_behavior_changed:false, oauth_behavior_changed:false, backend_behavior_changed:false, source_behavior_changed:false, storage_behavior_changed:false, manual_private_default:true, live_scraping_enabled:false, real_oauth_enabled:false, live_source_verification_enabled:false, zip_existence_sufficient:false, changed_files_manifest_required:true, apply_verification_required:true, checklist, pass_count:passCount, fail_count:failCount, required_commands:[`unzip -o ${artifactName} -d .`,'npm ci --no-audit --no-fund --ignore-scripts','npm run test:v127:no-browser','npm run test:ci:no-browser','PLAYWRIGHT_SKIP_INSTALL=1 npm run test:ci:browser'], blocked_claims:['Do not treat artifact download success as release approval.','Do not treat ZIP existence alone as proof of readiness.','Do not publish without applying the patch to the intended base and rerunning CI.','Do not claim live scraping, production OAuth, new provider behavior, backend endpoint expansion, source connector expansion, or storage expansion.'], release_gate:failCount===0?'release_apply_integrity_pass':'release_apply_integrity_blocked'};
  }
