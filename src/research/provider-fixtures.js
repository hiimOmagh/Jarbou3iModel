...
(function(global){
  ...
  function payload(task){
    ...
    return {
      ...
      packet:{workflow_version:'1.1.0-fix.2'}
    };
  }
  ...
  function runContractFixtureSuite(core = root.providerCore){
    const results = contractFixtures().map((fixture) => {
      ...
      const validation = core.validateProviderResponse(p, response, {version:'1.1.0-fix.2', nowIso:()=>'2026-04-28T00:00:00.000Z'});
