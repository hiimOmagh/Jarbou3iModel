/* Jarbou3i Research Engine provider fixtures v1.1.0. */
(function(global){
  'use strict';
  const root = global.Jarbou3iResearchModules = global.Jarbou3iResearchModules || {};

  const minimalStrategicAnalysis = {
    schema_version:'1.3.0-alpha.10',
  ...
  function payload(task){
    ...
    return {
      request_version:'1.3.0-alpha.10',
      provider:'fixture',
      ...
      packet:{workflow_version:'1.3.0-alpha.10'}
    };
  }
  ...
  function runContractFixtureSuite(core = root.providerCore){
    const results = contractFixtures().map((fixture) => {
      ...
      const validation = core.validateProviderResponse(p, response, {version:'1.3.0-alpha.10', nowIso:()=>'2026-04-28T00:00:00.000Z'});
