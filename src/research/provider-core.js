...
(function(global){
  ...
  function responseContract(task){
    const contracts = {
      ...
      synthesis: {
        ...
        example_shape:{schema_version:'1.3.0-alpha.4', subject:{title:'string'}, interests:[], actors:[], tools:[], narrative:[], results:[], feedback:[], scenarios:{items:[]}}
      },
      repair: {
        ...
        reject_if:['still malformed JSON','removes core analytical content','does not preserve schema_version 1.3.0-alpha.4'],
        diagnostic_hints:['Check parseability','Check missing required fields','Check that repair is marked traceably'],
        example_shape:{schema_version:'1.3.0-alpha.4', analysis_id:'A-REPAIRED', evidence:{items:[]}, scenarios:{items:[]}}
      },
      critique: {
  ...
  function validateProviderResponse(payload, response, options = {}){
    ...
    return {
      validation_version: options.version || '1.3.0-alpha.4',
      validated_at: nowIso(),
