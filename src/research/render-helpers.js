/* v1.4.0-alpha.2 · Provider/Source Execution Policy Matrix + Failure UX Contracts. Planning/preflight only; no live execution. */
(function(global){
  'use strict';
  ...
  const COPY = {
    en: {
      ...
      alphaBadge:'v1.4.0-alpha.2 Provider/Source Execution Policy Matrix + Failure UX Contracts · Planning/Preflight Only',
    ...
    ar: {
      ...
      alphaBadge:'v1.4.0-alpha.2 مصفوفة سياسة تنفيذ المزود/المصدر + عقود تجربة الفشل · تخطيط/فحص مسبق فقط' ,
      planTitle:'خطة البحث',
    ...
    fr: {
      ...
      alphaBadge:'v1.4.0-alpha.2 Matrice de politique d’exécution fournisseur/source + contrats UX d’échec · planification/préflight uniquement',
  ...
  Object.assign(COPY.en, {
    ...
    hostedDemoVerificationBody:'Provider/Source Execution Policy Matrix + Failure UX Contracts is ready for release evidence: the public copy is stable, the golden workflow and Export Pack v3 remain locked, and hosted evidence must report 1.4.0-alpha.2 internally while showing v1.4.0-alpha.2 Provider/Source Execution Policy Matrix + Failure UX Contracts to users. No live scraping, real OAuth, backend expansion, live provider execution, or automatic source verification is enabled.',
  ...
  Object.assign(COPY.fr, {
    ...
    hostedDemoVerificationBody:'La matrice de politique d’exécution fournisseur/source et les contrats UX d’échec est prête pour les preuves de release : le workflow manuel stable v1.3.0 reste verrouillé comme baseline, et les preuves hébergées doivent annoncer 1.4.0-alpha.2 en interne tout en affichant v1.4.0-alpha.2 Matrice de politique d’exécution fournisseur/source + contrats UX d’échec aux utilisateurs. Aucun scraping, OAuth réel, extension backend, fournisseur live ni vérification automatique des sources.',
  ...
  Object.assign(COPY.en, {
    analysisReleaseNote:'1.4.0-alpha.2 prepares controlled provider/source execution with ADRs, threat model, and preflight gates while preserving the locked v1.3.0 manual workflow baseline.',
  ...
  Object.assign(COPY.ar, {
    analysisReleaseNote:'تحضّر 1.4.0-alpha.2 تنفيذ المزود/المصدر المضبوط عبر ADRs ونموذج تهديد وفحوصات مسبقة مع الحفاظ على خط العمل اليدوي المستقر v1.3.0 كخط أساس مقفول.',
  ...
  Object.assign(COPY.fr, {
    analysisReleaseNote:'1.4.0-alpha.2 prépare l’exécution contrôlée fournisseur/source avec ADR, modèle de menace et préflight tout en conservant le workflow manuel stable v1.3.0 comme baseline verrouillée.',
  ...
  Object.assign(COPY.ar, {
    alphaBadge:'v1.4.0-alpha.2 مصفوفة سياسة تنفيذ المزود/المصدر + عقود تجربة الفشل',
  ...
  Object.assign(COPY.fr, {
    alphaBadge:'v1.4.0-alpha.2 Matrice de politique d’exécution fournisseur/source + contrats UX d’échec',
  ...
  Object.assign(COPY.en, {
    hostedDemoVerificationBody:'Provider/Source Execution Policy Matrix + Failure UX Contracts is ready for release evidence: final brief, evidence appendix, contradiction/falsifier appendix, source-gap appendix, and operator signoff/lock-ledger appendix are consolidated into one local publication handoff. Hosted evidence must report 1.4.0-alpha.2 internally while showing v1.4.0-alpha.2 Provider/Source Execution Policy Matrix + Failure UX Contracts to users. No live scraping, real OAuth, backend expansion, live provider execution, automatic signoff, automatic export lock, cryptographic signature claim, publishing permission claim, or automatic source verification is enabled.',
    analysisReleaseNote:'1.4.0-alpha.2 prepares Provider/Source Execution Policy Matrix + Failure UX Contracts: a final brief plus evidence, contradiction/falsifier, source-gap, and operator signoff/lock-ledger appendices are packaged for manual publication review while live/provider/OAuth/backend/storage boundaries remain unchanged.'
  });
  Object.assign(COPY.ar, {
    hostedDemoVerificationBody:'حزمة النشر v4 جاهزة لأدلة الإصدار: يتم تجميع المسودة النهائية وملحق الأدلة وملحق التناقضات/شروط الإبطال وملحق فجوات المصادر وملحق توقيع المشغل وسجل القفل في تسليم محلي واحد. يجب أن تعلن أدلة الاستضافة داخلياً 1.4.0-alpha.2 مع عرض v1.4.0-alpha.2 مصفوفة سياسة تنفيذ المزود/المصدر + عقود تجربة الفشل للمستخدمين. لا يوجد scraping أو OAuth حقيقي أو توسيع خلفية أو تنفيذ مزود حي أو توقيع تلقائي أو قفل تصدير تلقائي أو توقيع تشفيري أو ادعاء إذن نشر أو تحقق آلي من المصادر.',
    analysisReleaseNote:'تحضّر 1.4.0-alpha.2 تنفيذ المزود/المصدر المضبوط: مسودة نهائية مع ملاحق الأدلة والتناقضات/الإبطال وفجوات المصادر وتوقيع المشغل/سجل القفل للمراجعة اليدوية قبل النشر مع بقاء حدود التنفيذ الحي والمزود وOAuth والخلفية والتخزين دون تغيير.'
  });
  Object.assign(COPY.fr, {
    hostedDemoVerificationBody:'La matrice de politique d’exécution fournisseur/source + contrats UX d’échec est prêt pour les preuves de release : brief final, annexe des preuves, annexe contradictions/falsificateurs, annexe des écarts de sources et annexe visa opérateur/registre de verrouillage sont consolidés en une remise locale. Les preuves hébergées doivent annoncer 1.4.0-alpha.2 en interne tout en affichant v1.4.0-alpha.2 Provider/Source Execution Policy Matrix + Failure UX Contracts aux utilisateurs. Aucun scraping, OAuth réel, extension backend, fournisseur live, visa automatique, verrouillage automatique, signature cryptographique, permission de publication revendiquée ni vérification automatique des sources n’est activé.',
    analysisReleaseNote:'1.4.0-alpha.2 prépare Provider/Source Execution Policy Matrix + Failure UX Contracts : brief final et annexes preuves, contradictions/falsificateurs, écarts de sources et visa opérateur/registre de verrouillage pour revue manuelle avant publication, sans changer les limites live/fournisseur/OAuth/backend/stockage.'
  });


  Object.assign(COPY.en, {
    briefPublicationPackTitle:'Provider/Source Execution Policy Matrix + Failure UX Contracts',
    publicationPackStatus:'publication status',
    publicationReady:'publication ready',
    briefPublicationPackBoundary:'Provider/Source Execution Policy Matrix + Failure UX Contracts is local/manual. It consolidates the final brief and appendices without publishing, fetching, verifying, or signing cryptographically.'
  });
  Object.assign(COPY.ar, {
  ...
  Object.assign(COPY.fr, {
    ...
    briefPublicationPackBoundary:'Provider/Source Execution Policy Matrix + Failure UX Contracts est local/manuel. Il consolide le brief final et les annexes sans publication, récupération, vérification ni signature cryptographique.'
  });
