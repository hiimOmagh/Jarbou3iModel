/* v1.4.0-alpha.8 · Credential Boundary Runtime Drill. Fake-secret drills only; no real credentials. */
(function(global){
  'use strict';
  ...
  const COPY = {
    en: {
      ...
      alphaBadge:'v1.4.0-alpha.8 Credential Boundary Runtime Drill · Fake-Secret Drill Only',
    ...
    ar: {
      ...
      alphaBadge:'v1.4.0-alpha.8 تدريب حدود بيانات الاعتماد وقت التشغيل · أسرار وهمية فقط' ,
      planTitle:'خطة البحث',
    ...
    fr: {
      ...
      alphaBadge:'v1.4.0-alpha.8 Drill runtime de frontière des identifiants · faux secrets uniquement',
  ...
  Object.assign(COPY.en, {
    ...
    hostedDemoVerificationBody:'Credential Boundary Runtime Drill is ready for release evidence: fake-secret injection, export leak, log leak, browser-visible text leak, fixture leak, provider payload, and release bundle drills are visible without real credentials. Hosted evidence must report 1.4.0-alpha.8 internally while showing v1.4.0-alpha.8 Credential Boundary Runtime Drill to users. No real OAuth, API keys, raw tokens, token storage, live scraping, backend expansion, live provider execution, hidden background fetching, or automatic source verification is enabled.',
  ...
  Object.assign(COPY.ar, {
    ...
    hostedDemoVerificationBody:'تدريب حدود بيانات الاعتماد وقت التشغيل جاهز لأدلة الإصدار: تختبر الأسرار الوهمية التصدير والسجلات والنص الظاهر والفيكستشرات وحمولات المزوّد وحزمة الإصدار دون بيانات اعتماد حقيقية. يجب أن تعلن أدلة الاستضافة داخلياً 1.4.0-alpha.8 مع عرض v1.4.0-alpha.8 تدريب حدود بيانات الاعتماد وقت التشغيل · أسرار وهمية فقط للمستخدمين. لا يوجد OAuth حقيقي أو مفاتيح API أو رموز خام أو تخزين رموز أو تنفيذ مزود حي أو جلب مصادر حي.',
  ...
  Object.assign(COPY.fr, {
    ...
    hostedDemoVerificationBody:'Le drill runtime de frontière des identifiants est prêt pour les preuves de release : les faux secrets testent exports, journaux, texte visible, fixtures, payloads fournisseur et bundle de release sans identifiants réels. Les preuves hébergées doivent annoncer 1.4.0-alpha.8 en interne tout en affichant v1.4.0-alpha.8 Drill runtime de frontière des identifiants · faux secrets uniquement aux utilisateurs. Aucun OAuth réel, clé API, token brut, stockage de token, fournisseur live ni récupération de source live.',
  ...
  Object.assign(COPY.en, {
    analysisReleaseNote:'1.4.0-alpha.8 adds Credential Boundary Runtime Drill while preserving the locked v1.3.0 manual workflow baseline, locked v1.4.0-alpha.7 source acquisition baseline, locked v1.4.0-alpha.6 mock-to-live baseline, and all live/provider/OAuth/backend/storage boundaries.',
  ...
  Object.assign(COPY.ar, {
    analysisReleaseNote:'تضيف 1.4.0-alpha.8 تدريب حدود بيانات الاعتماد مع الحفاظ على خط العمل اليدوي المستقر v1.3.0 وخط alpha.7 لاكتساب المصادر وخط alpha.6 لمعادلة التنفيذ وكل حدود التنفيذ الحي وOAuth والخلفية والتخزين.',
  ...
  Object.assign(COPY.fr, {
    analysisReleaseNote:'1.4.0-alpha.8 ajoute le drill runtime de frontière des identifiants tout en conservant le workflow manuel stable v1.3.0, la baseline alpha.7 acquisition des sources, la baseline alpha.6 mock-vers-live et les limites live/OAuth/backend/stockage.',
  ...
  Object.assign(COPY.en, {
    hostedDemoVerificationBody:'Credential Boundary Runtime Drill is ready for release evidence: fake-secret injection, export leak, log leak, browser-visible text leak, fixture leak, provider payload, and release bundle drills are visible without real credentials. Hosted evidence must report 1.4.0-alpha.8 internally while showing v1.4.0-alpha.8 Credential Boundary Runtime Drill to users. No real OAuth, API keys, raw tokens, token storage, live scraping, backend expansion, live provider execution, hidden background fetching, automatic signoff, automatic export lock, cryptographic signature claim, publishing permission claim, or automatic source verification is enabled.',
    analysisReleaseNote:'1.4.0-alpha.8 prepares Credential Boundary Runtime Drill: fake-secret vectors test exports, logs, visible text, fixtures, provider payloads, and release bundles while real credential handling remains disabled.'
  });
  Object.assign(COPY.ar, {
    hostedDemoVerificationBody:'تدريب حدود بيانات الاعتماد وقت التشغيل جاهز لأدلة الإصدار: تختبر الأسرار الوهمية التصدير والسجلات والنص الظاهر والفيكستشرات وحمولات المزوّد وحزمة الإصدار دون بيانات اعتماد حقيقية. يجب أن تعلن أدلة الاستضافة داخلياً 1.4.0-alpha.8 مع عرض v1.4.0-alpha.8 تدريب حدود بيانات الاعتماد وقت التشغيل · أسرار وهمية فقط للمستخدمين. لا يوجد OAuth حقيقي أو مفاتيح API أو رموز خام أو تخزين رموز أو تنفيذ مزود حي أو جلب مصادر حي.',
    analysisReleaseNote:'تحضّر 1.4.0-alpha.8 تدريب حدود بيانات الاعتماد: تختبر متجهات الأسرار الوهمية التصدير والسجلات والنص الظاهر والفيكستشرات وحمولات المزوّد وحزم الإصدار مع بقاء التعامل الحقيقي مع بيانات الاعتماد معطلاً.'
  });
  Object.assign(COPY.fr, {
    hostedDemoVerificationBody:'Le drill runtime de frontière des identifiants est prêt pour les preuves de release : les faux secrets testent exports, journaux, texte visible, fixtures, payloads fournisseur et bundle de release sans identifiants réels. Les preuves hébergées doivent annoncer 1.4.0-alpha.8 en interne tout en affichant v1.4.0-alpha.8 Drill runtime de frontière des identifiants · faux secrets uniquement aux utilisateurs. Aucun OAuth réel, clé API, token brut, stockage de token, fournisseur live ni récupération de source live.',
    analysisReleaseNote:'1.4.0-alpha.8 prépare le drill de frontière des identifiants : les vecteurs faux secrets testent exports, logs, texte visible, fixtures, payloads fournisseur et bundles de release sans activer de vrais identifiants.'
  });
