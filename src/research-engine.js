/* Jarbou3i Research Engine v1.1.0-alpha.11 — module split. Manual mode remains first-class. */
(function(){
  'use strict';
  ...
  function copyTable(name){return renderHelpers.COPY?.[getLang()]?.[name]||renderHelpers.COPY?.en?.[name]||{};}
  function localizedLayer(value){return copyTable('layerLabels')[String(value||'')]||String(value||'');}
  function localizedStatus(value){return copyTable('statusLabels')[String(value||'')]||String(value||'').replaceAll('_',' ');}
  function localizedBand(value){return copyTable('bandLabels')[String(value||'')]||String(value||'').replaceAll('_',' ');}
  ...
  function renderDiagnostics(){
    ...
    el.innerHTML = `<div class="researchJsonCard diagnosticsCard"><h4>${esc(tr('diagnosticsTitle'))}</h4><div class="miniChips">${coverageRows || '<span>—</span>'}${migrationHtml}</div><small>${esc(localizedStatus(diagnostics.status || 'review_required'))} · ${esc((diagnostics.gaps || []).length)} ${esc(tr('gaps'))}${migrationReport ? ' · ' + esc(tr('migrationReportExported')) : ''}</small></div>`;
  }
  ...
  function renderSourceLayer(){
    ...
    const policyHtml = `<div class="researchJsonCard sourcePolicyCard"><h4>${esc(tr('sourcePolicyTitle'))}</h4><div class="miniChips"><span>${esc(localizedStatus(policy.verdict))}</span><span>${esc(tr('live'))}:${esc(localizedBoolean(policy.live_fetching_enabled))}</span><span>${esc(policy.current_layer)}</span></div><small>${esc((policy.prohibited_actions || []).slice(0,2).join(' · '))}</small></div>`;
    const diagnosticsHtml = diagnostics ? `<div class="researchJsonCard sourceDiagnosticsCard"><h4>${esc(tr('sourceDiagnosticsTitle'))}</h4><div class="miniChips"><span>${esc(localizedStatus(diagnostics.readiness))}</span><span>${esc(diagnostics.evidence_count)} ${esc(tr('evidenceWord'))}</span><span>${esc(diagnostics.source_type_count)} ${esc(tr('sourceTypes'))}</span></div><ul>${(diagnostics.warnings || [tr('noSourceWarnings')]).map(x=>`<li>${esc(x)}</li>`).join('')}</ul></div>` : '';
    ...
    const resultHtml = latestResult ? `<div class="researchJsonCard sourceResultCard"><h4>${esc(tr('latestSourceResultTitle'))}</h4><div class="miniChips"><span>${esc(latestResult.connector)}</span><span>${esc(tr('candidates'))}:${esc(latestResult.evidence_candidate_count || 0)}</span><span>${esc(tr('queued'))}:${esc(latestResult.queued_review_count || 0)}</span><span>${esc(tr('live'))}:${esc(localizedBoolean(latestResult.source_fetching_performed))}</span></div><small>${esc(latestResult.repo?.full_name || latestResult.provider_id || localizedStatus(latestResult.verdict || ''))}</small></div>` : '';
    const searchHtml = state.search_provider_identity ? `<div class="researchJsonCard sourceSearchCard"><h4>${esc(tr('webSearchAbstractionTitle'))}</h4><div class="miniChips"><span>${esc(state.search_provider_identity.provider_id)}</span><span>${esc(tr('live'))}:${esc(localizedBoolean(state.search_provider_identity.live_enabled))}</span><span>${esc(tr('queries'))}:${esc(state.search_query_budget?.max_queries || 0)}</span></div><small>${esc(localizedStatus(state.search_policy?.verdict || 'web_search_abstraction_ready_no_live_fetch'))}</small></div>` : '';
  ...
  function renderSourcePacketBuilder(){
    ...
    const templateHtml = templateReport ? `<div class="researchJsonCard sourcePacketTemplateReportCard"><h4>${esc(tr('sourcePacketTemplateReportTitle'))}</h4><div class="miniChips sourcePacketBuilderChips"><span>${esc(templateReport.template_count)} ${esc(tr('presets'))}</span><span>${esc(templateReport.active_template_id)}</span><span>${esc(tr('live'))}:${esc(localizedBoolean(templateReport.live_fetching_performed))}</span><span>${esc(tr('verified'))}:${esc(localizedBoolean(templateReport.verification_claimed))}</span><span>${esc(localizedStatus(templateReport.release_gate))}</span></div><small>${esc(localizedPolicy(templateReport.policy))}</small></div>` : '';
    el.innerHTML = templateHtml + `<div class="researchJsonCard sourcePacketBuilderReportCard sourcePacketBuilderQaCard"><h4>${esc(tr('sourcePacketBuilderTitle'))}</h4><div class="miniChips sourcePacketBuilderChips"><span>${esc(packetCount)} ${esc(tr('sourcePacketPackets'))}</span><span>${esc(report?.evidence_item_count || 0)} ${esc(tr('sourcePacketEvidence'))}</span><span>${esc(tr('live'))}:${esc(localizedBoolean(report?.live_fetching_performed))}</span><span>${esc(tr('verified'))}:${esc(localizedBoolean(report?.verification_claimed))}</span><span>${esc(localizedStatus(releaseGate))}</span></div><div class="sourcePacketBuilderRiskGrid"><span>${esc(tr('sourcePacketWarnings'))}: ${esc(warningCount)}</span><span>${esc(tr('sourcePacketWeakTraceability'))}: ${esc(weakTraceability)}</span><span>${esc(tr('sourcePacketAttentionReliabilityRisks'))}: ${esc(attentionRisk)}</span></div><small>${esc(localizedPolicy(report?.policy || 'local_manual_source_packet_builder_no_fetch_no_verification'))}</small><small>${esc(tr('sourcePacketBuilderBrowserQaNote'))}</small>${packetPreview ? `<pre class="sourcePacketBuilderPreview" aria-label="Source packet builder preview">${esc(packetPreview)}</pre>` : ''}</div>`;
  }
  ...
  function renderQuality(){
    ...
    const weakestHtml = report.weakest_dimensions.map(item => '<li><strong>' + esc(localizedDimension(item.dimension)) + '</strong>: ' + esc(item.score) + ' · ' + esc(localizedStatus(item.severity)) + '</li>').join('');
    ...
    const scoringHtml = '<div class="researchJsonCard evidenceScoringCard"><h4>' + esc(tr('qualityScoringTitle')) + '</h4><div class="miniChips"><span>' + esc(tr('reliabilityLabel')) + ' ' + esc(scoringReport.average_reliability_score || 0) + '/100 · ' + esc(localizedBand(scoringReport.reliability_band || '—')) + '</span><span>' + esc(tr('attentionLabel')) + ' ' + esc(scoringReport.average_attention_signal_score || 0) + '/100 · ' + esc(localizedBand(scoringReport.attention_band || '—')) + '</span><span>' + esc(tr('traceabilityLabel')) + ' ' + esc(scoringReport.average_traceability_score || 0) + '/100 · ' + esc(localizedBand(scoringReport.traceability_band || '—')) + '</span><span>' + esc(tr('calibrationWarnings')) + ' ' + esc(scoringReport.calibration_warning_count || 0) + '</span></div><small>' + esc(localizedPolicy(scoringReport.policy || '')) + '</small><small>' + esc(tr('evidenceScoringPolicyNote')) + ' ' + esc(tr('guardLabel')) + ': ' + esc(localizedStatus(scoringReport.score_theater_guard || 'score_theater_guard')) + '</small></div>';
    const reportHtml = scoringHtml + '<div class="researchJsonCard qualityGateV3Card"><h4>' + esc(tr('publicationReadiness')) + ': ' + esc(localizedStatus(report.publication_readiness)) + '</h4><div class="miniChips"><span>' + esc(localizedStatus(report.release_gate)) + '</span><span>' + esc(report.overall_score) + '/100</span><span>' + esc(report.blockers.length) + ' ' + esc(tr('blockers')) + '</span></div><h5>' + esc(tr('weakestDimensions')) + '</h5><ul>' + weakestHtml + '</ul><h5>' + esc(tr('fixActions')) + '</h5><ul>' + actionsHtml + '</ul></div>';
