/* Jarbou3i Research Engine v1.1.0 â€” Release Candidate Hardening and final repo hygiene. Manual mode remains first-class. */
(function(){
  'use strict';
  ...
  function applyReviewBatchDecision(decision){
    ...
    save(); render();
    setStatus((decision === 'accept' ? tr('statusEvidenceAccepted') : decision === 'reject' ? tr('statusEvidenceRejected') : tr('statusEvidenceNeedsEdit')) + ' Â· ' + String(plan.eligible_count || 0), decision === 'reject' ? 'warn' : 'good');
    return plan;
  }
  ...
  function evidenceAsLegacyItems(){
    return state.evidence.map((e, index) => ({
      ...
      source_note: [e.source_title, e.source_type, e.source_date].filter(Boolean).join(' Â· '),
  ...
  function importWorkflowPacket(packet){
    ...
    const suffix = report && report.migrated ? ` ${report.source_version}â†’${report.target_version}` : '';
  ...
  function renderResearchPlanner(){
    ...
    const queryRows = (report.entity_aware_queries || []).slice(0,6).map(q=>`<li><strong>${esc(q.query_id)} Â· ${esc(q.purpose)}</strong>: ${esc(q.query_text)}</li>`).join('');
    el.innerHTML = `<div class="researchJsonCard researchPlannerCard"><div><b>${esc(tr('researchPlannerTitle'))}</b><span>${esc(report.depth_preset)} Â· ${esc(report.planner_quality_score || 0)}/100 Â· ${esc(report.release_gate || '')}</span></div><div class="miniChips">${budgetChips || '<span>â€”</span>'}</div><h4>${esc(tr('researchPlannerQueries'))}</h4><ul>${queryRows || '<li>â€”</li>'}</ul><h4>${esc(tr('researchPlannerCounterEvidence'))}</h4><div class="miniChips">${(report.counter_evidence_targets || []).slice(0,6).map(t=>`<span>${esc(t.target_id || '')}: ${esc(String(t.target || '').slice(0,80))}</span>`).join('') || '<span>â€”</span>'}</div></div>`;
  }
  ...
  function renderAnalysisBrief(){
    ...
    const graphCard = `<div class="researchJsonCard strategicEvidenceGraphCard"><h4>${esc(tr('strategicEvidenceGraphTitle') || 'Strategic Evidence Map')}</h4><div class="miniChips"><span>${esc(graphQuality.node_count || 0)} nodes</span><span>${esc(graphQuality.edge_count || 0)} edges</span><span>${esc((graph.graph_export_report?.formats || []).join('/'))}</span><span>${esc(graphQuality.release_gate || 'graph_review_required')}</span></div>${(graphQuality.graph_gap_flags || []).length ? `<small>${esc((graphQuality.graph_gap_flags || []).join(' Â· '))}</small>` : ''}</div>`;
    const entityCards = entities.slice(0, 8).map(e => `<div class="entityProfileCard"><div><b>${esc(e.entity_id)} Â· ${esc(e.canonical_name || '')}</b><span>${esc(e.category || 'other')}</span></div><p>${esc((e.claim_mentions || [])[0] || '')}</p><div class="miniChips"><span>${esc((e.evidence_ids || []).length)}E</span><span>${esc((e.cluster_ids || []).length)}CL</span><span>${esc(e.strategic_relevance_score ?? 'â€”')}/100</span><span>${esc(e.review_gate || 'entity_review_required')}</span></div>${(e.aliases || []).length ? `<small>aliases: ${esc((e.aliases || []).slice(0,4).join(' Â· '))}</small>` : ''}</div>`).join('');
    const clusterCards = clusters.slice(0, 8).map(c => `<div class="sourceClusterCard"><div><b>${esc(c.cluster_id)} Â· ${esc(c.cluster_label || c.target_id)}</b><span>${esc(c.review_gate || 'review_required')}</span></div><p>${esc(c.primary_claim || (c.claims || [])[0] || '')}</p><div class="miniChips"><span>${esc(c.evidence_ids.length)}E</span><span>R:${esc(c.reliability_score ?? 'â€”')}</span><span>A:${esc(c.attention_signal_score ?? 'â€”')}</span><span>T:${esc(c.traceability_score ?? 'â€”')}</span><span>${esc((c.source_types || []).join('/'))}</span>${c.duplicate_count ? `<span>${esc(c.duplicate_count)} duplicate</span>` : ''}</div>${(c.gap_flags || []).length ? `<small>${esc((c.gap_flags || []).join(' Â· '))}</small>` : ''}</div>`).join('');
    el.innerHTML = `<div class="researchJsonCard analysisBriefCard"><div><b>${esc(brief.topic)}</b><span>${esc(brief.context)} Â· readiness ${esc(brief.readiness_score)}/100</span></div><h4>${esc(tr('researchPlannerTitle'))}</h4>${planner ? `<div class="researchPlannerSummary"><div class="miniChips"><span>${esc(planner.depth_preset || 'standard')}</span><span>${esc(planner.planner_quality_score || 0)}/100</span><span>${esc((planner.entity_aware_queries || []).length)} queries</span><span>${esc((planner.counter_evidence_targets || []).length)} counter-targets</span></div></div>` : '<p class="muted">â€”</p>'}<h4>${esc(tr('entityTitle') || 'Entity profiles')}</h4><div class="entityProfileGrid">${entityCards || '<span>â€”</span>'}</div><h4>${esc(tr('clusterTitle'))}</h4><div class="sourceClusterGrid">${clusterCards || '<span>â€”</span>'}</div><h4>${esc(tr('gapsTitle'))}</h4><ul>${(gaps.length ? gaps : ['No critical compiler gaps detected.']).map(x=>`<li>${esc(x)}</li>`).join('')}</ul>${gapReport.recommendations?.length ? `<h4>Source gap actions</h4><ul>${gapReport.recommendations.map(x=>`<li>${esc(x)}</li>`).join('')}</ul>` : ''}${graphCard}<h4>Handoff</h4><p class="muted">${esc(brief.handoff_summary || '')}</p></div>`;
    renderDiagnostics();
  }
  ...
  function renderDiagnostics(){
    ...
    const migrationHtml = migrationReport ? `<span>migration:${esc(migrationReport.source_version)}â†’${esc(migrationReport.target_version)}</span>` : '';
    el.innerHTML = `<div class="researchJsonCard diagnosticsCard"><h4>${esc(tr('diagnosticsTitle'))}</h4><div class="miniChips">${coverageRows || '<span>â€”</span>'}${migrationHtml}</div><small>${esc(lStatus(diagnostics.status || 'review_required'))} Â· ${esc((diagnostics.gaps || []).length)} ${esc(tr('gaps'))}${migrationReport ? ' Â· ' + esc(tr('migrationReportExported')) : ''}</small></div>`;
  }
  ...
  function renderSourceLayer(){
    ...
    const requestHtml = request ? `<div class="researchJsonCard sourceRequestCard"><h4>${esc(tr('sourceRequestTitle'))}</h4><div class="miniChips"><span>${esc(request.connector)}</span><span>${esc(request.task)}</span><span>${esc(tr('live'))}:${esc(localizedBoolean(request.live_fetching_enabled))}</span></div><small>${esc((request.research_questions || []).length)} ${esc(tr('questions'))} Â· ${esc((request.target_sources || []).length)} ${esc(tr('sourceTargets'))} Â· ${esc((request.keywords || []).length)} ${esc(tr('keywords'))}</small></div>` : `<p class="muted">${esc(tr('sourcePolicyNote'))}</p>`;
    const policyHtml = `<div class="researchJsonCard sourcePolicyCard"><h4>${esc(tr('sourcePolicyTitle'))}</h4><div class="miniChips"><span>${esc(lStatus(policy.verdict))}</span><span>${esc(tr('live'))}:${esc(localizedBoolean(policy.live_fetching_enabled))}</span><span>${esc(policy.current_layer)}</span></div><small>${esc((policy.prohibited_actions || []).slice(0,2).join(' Â· '))}</small></div>`;
  ...
  function renderEvidenceReviewQueue(){
    ...
    const rows = filtered.items.map((item) => {
      ...
      return `<tr>
        ...
        <td><b>${esc(e.claim || '')}</b><small>${esc(e.notes || '')}</small><small>R:${esc(e.evidence_scoring?.reliability_score ?? 'â€”')} Â· A:${esc(e.evidence_scoring?.attention_signal_score ?? 'â€”')} Â· W:${esc(e.evidence_scoring?.synthesis_weight ?? 'â€”')}</small><small>${esc(localizedReviewGate(controlText))}</small></td>
        <td>${esc([e.source_title, lST(e.source_type), e.source_date].filter(Boolean).join(' Â· '))}${e.source_url?`<small>${esc(e.source_url)}</small>`:''}</td>
        <td>${esc((e.supports || []).join(', ') || 'â€”')} / ${esc((e.contradicts || []).join(', ') || 'â€”')}</td>
  ...
  function renderProviderHarness(){
    ...
    if(contractEl){
      contractEl.innerHTML = `<strong>${esc(tr('providerContractLabel'))}</strong><span>${esc(contract.title || contract.type)} Â· ${esc(contract.type)}</span><small>${esc((contract.required || []).length)} ${esc(tr('required'))} Â· ${(contract.required || []).slice(0,5).map(esc).join(', ')}</small>`;
    }
    if(promptEl){
      if(promptPreview){
        promptEl.innerHTML = `<strong>${esc(tr('providerPromptLabel'))}</strong><span>${esc(promptPreview.task)} Â· ${esc(promptPreview.prompt_chars)} ${esc(tr('chars'))} Â· ${esc(promptPreview.prompt_fingerprint)}</span><small>${esc(promptPreview.privacy_mode)}${promptPreview.truncated ? ' Â· ' + tr('truncatedPreview') : ''}</small>`;
      } else {
    ...
    runEl.innerHTML = `<div class="researchTableWrap"><table class="researchTable providerTable"><thead><tr><th>${esc(tr('run'))}</th><th>${esc(tr('providerTask'))}</th><th>${esc(tr('providerName'))}</th><th>${esc(tr('status'))}</th><th>${esc(tr('validation'))}</th><th>${esc(tr('output'))}</th></tr></thead><tbody>${runs.slice().reverse().map(run=>`<tr><td>${esc(run.run_id)}</td><td>${esc(run.task)}</td><td>${esc(run.provider)}</td><td>${esc(run.status)} Â· ${esc(run.duration_ms)}ms</td><td>${esc(validationSummary(run.response_validation, run.repair_trace))}</td><td>${esc(run.output_summary)}<small>${esc((run.warnings || []).join(' Â· '))}</small></td></tr>`).join('')}</tbody></table></div>`;
  }
  ...
  function renderCritique(){
    ...
    el.innerHTML = `<div class="researchJsonCard critiqueCard"><b>${esc(critique.summary)}</b><ul>${critique.findings.map(f=>`<li><strong>${esc(f.type)} Â· ${esc(f.severity)}</strong>: ${esc(f.finding)}</li>`).join('')}</ul><h4>${esc(tr('critiqueNextActions'))}</h4><ul>${critique.recommended_next_actions.map(x=>`<li>${esc(x)}</li>`).join('')}</ul></div>`;
  }
  ...
  function startFirstRun(){
    ...
    if($('timeframeInput') && !($('timeframeInput').value || '').trim()) $('timeframeInput').value = '2024â€“2026, international context';
  ...
  function renderQuality(){
    ...
    const weakestHtml = report.weakest_dimensions.map(item => '<li><strong>' + esc(lDim(item.dimension)) + '</strong>: ' + esc(item.score) + ' Â· ' + esc(lStatus(item.severity)) + '</li>').join('');
    ...
    const scoringHtml = '<div class="researchJsonCard evidenceScoringCard"><h4>' + esc(tr('qualityScoringTitle')) + '</h4><div class="miniChips"><span>' + esc(tr('reliabilityLabel')) + ' ' + esc(scoringReport.average_reliability_score || 0) + '/100 Â· ' + esc(lBand(scoringReport.reliability_band || 'â€”')) + '</span><span>' + esc(tr('attentionLabel')) + ' ' + esc(scoringReport.average_attention_signal_score || 0) + '/100 Â· ' + esc(lBand(scoringReport.attention_band || 'â€”')) + '</span><span>' + esc(tr('traceabilityLabel')) + ' ' + esc(scoringReport.average_traceability_score || 0) + '/100 Â· ' + esc(lBand(scoringReport.traceability_band || 'â€”')) + '</span><span>' + esc(tr('calibrationWarnings')) + ' ' + esc(scoringReport.calibration_warning_count || 0) + '</span></div><small>' + esc(localizedPolicy(scoringReport.policy || '')) + '</small><small>' + esc(tr('evidenceScoringPolicyNote')) + ' ' + esc(tr('guardLabel')) + ': ' + esc(lStatus(scoringReport.score_theater_guard || 'score_theater_guard')) + '</small></div>';
