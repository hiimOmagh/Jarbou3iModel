...
The screenshots must show no horizontal overflow, visible first-run/public-demo/hosted-demo/evidence-review panels, reachable provider/export states, and a metadata snapshot with app version `1.3.0-alpha.4` and the evidence-review panel present.
...
- Current root artifact consolidation: `v1.3.0-alpha.4 — Brief Template UX Polish + Matrix Hygiene Cleanup`
...
Release-lock guard: v1.3.0-alpha.4 Brief Template UX Polish + Matrix Hygiene Cleanup requires green no-browser CI, green browser CI, and reviewed hosted-demo evidence. Screenshots alone are insufficient. ZIP existence alone is insufficient. A local ZIP archive alone is insufficient.
