...
The screenshots must show no horizontal overflow, visible first-run/public-demo/hosted-demo/evidence-review panels, reachable provider/export states, and a metadata snapshot with app version `1.1.0-rc.2-fix.2` and the evidence-review panel present.
...
- Current root artifact consolidation: `v1.1.0-rc.2-fix.2 — Evidence Matrix + Canonical Bundle Validation`

Evidence manifest continuity: v1.1.0-rc.2-fix.2 preserves the single final metadata hosted-demo evidence manifest gate.
...
Release-lock guard: v1.1.0-rc.2-fix.2 Evidence Matrix + Canonical Bundle Validation requires green no-browser CI, green browser CI, and reviewed hosted-demo evidence. Screenshots alone are insufficient. ZIP existence alone is insufficient. A local ZIP archive alone is insufficient.
