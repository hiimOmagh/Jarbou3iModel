...
- Public release label: v1.4.0-alpha.21 Evidence Handoff Readiness Checklist.
...
- Scope: static evidence handoff readiness checklist only. It verifies locked alpha.20 identity, decision-ledger handoff audit availability, evidence matrix availability, visible-text snapshot availability, artifact identity guard availability, behavior-boundary confirmation, and operator review path confirmation.
- Checklist outcomes: handoff_packet_ready_for_review; review_handoff_warnings_before_lock; block_handoff_until_repaired; capture_handoff_evidence_before_review.
...
Planning/control-plane only. Scope: alpha.20 lock completion plus an evidence handoff readiness checklist. The checklist turns handoff audit evidence into pass, warn, fail, and review-required operator handoff outcomes without expanding browser scope.

Evidence/runtime budget guard: evidence_surface_budget_version: 1.4.0-alpha.21; locked_baseline: 1.4.0-alpha.20; browser_check_budget_max: 20; hosted_language_count_expected: 3; hosted_surface_count_expected_max: 13; visible_snapshot_rows_expected_max: 39; runtime_budget_policy: guardrail_only; runtime_budget_enforced_without_network: true; provider_execution_performed: false; live_fetching_performed: false; credential_persistence_allowed: false.
...
Screenshots alone are insufficient. A ZIP archive alone is insufficient. Public Demo evidence remains required for lock: hosted evidence, visible-label snapshots, browser log, Playwright setup logs, and canonical lock bundle must all remain aligned with 1.4.0-alpha.21.

Release manifest tokens for packaging checks: package=jarbou3i-research-engine; version=1.4.0-alpha.21; runtime_capability_change=false; release_type=alpha20-lock-completion-evidence-handoff-readiness-checklist; release_scope=evidence-handoff-readiness-checklist-lock-completion-only; test:ci:browser required.
...
Legacy alpha.15 budget tokens: evidence_surface_budget_version: 1.4.0-alpha.21; locked_baseline: 1.4.0-alpha.14; browser_check_budget_max: 20; hosted_language_count_expected: 3; hosted_surface_count_expected_max: 13; visible_snapshot_rows_expected_max: 39; runtime_budget_policy: guardrail_only; runtime_budget_enforced_without_network: true; provider_execution_performed: false; live_fetching_performed: false; credential_persistence_allowed: false.
...
Release evidence policy exact tokens: Screenshots alone are not sufficient. ZIP existence alone is not sufficient. green no-browser CI. green browser CI. hosted-demo-metadata.json. desktop-first-screen. mobile-first-screen. provider-mode. quality-export. No live scraping. no real OAuth. PLAYWRIGHT_SKIP_INSTALL=1 npm run test:ci:browser. changed-files-only patch handoff.
