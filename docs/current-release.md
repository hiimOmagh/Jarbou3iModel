...
Planning/control-plane only. Deterministic manual provider adapter sandbox + ephemeral invocation contract:

- Fake/manual ephemeral credential handoff interface without persistence.
- Provider request-envelope preview with no raw secrets, no authorization header, and no endpoint export.
- No-network dry invocation transcript with provider call count fixed at zero.
- Adapter failure taxonomy for missing operator opt-in, missing ephemeral handoff acknowledgement, unreviewed request envelope, missing no-network acknowledgement, missing budget/timeout/request acknowledgement, missing kill-switch, and locked-baseline failures.
- Safe request/response metadata ledger with checksum.
- Continuity from alpha.11 safety cockpit, alpha.10 manual shell, alpha.9 candidate gate, alpha.8 credential drill, alpha.7 source acquisition, and alpha.6 mock-to-live equivalence.
...
- Required targeted checks: `tests/manual-provider-adapter-sandbox-ephemeral-invocation-contract-check.mjs`, `tests/manual-execution-safety-cockpit-session-ledger-check.mjs`, `tests/limited-manual-live-execution-prototype-check.mjs`, `tests/controlled-execution-candidate-gate-check.mjs`, `tests/credential-boundary-runtime-drill-check.mjs`, `tests/source-acquisition-control-surface-check.mjs`, `tests/provider-execution-mock-to-live-equivalence-check.mjs`.
- Required CI: `npm run test:ci:no-browser`, `npm run test:ci:browser`.

Feature surface is frozen except deterministic manual provider adapter sandbox artifacts; no live execution, live fetching, or real credential handling is enabled.

Machine tokens: runtime_capability_change=false; release_type=manual-provider-adapter-sandbox-ephemeral-invocation-contract; release_scope=no-network-adapter-sandbox-only; test:ci:browser required.
...
release_type=manual-provider-adapter-sandbox-ephemeral-invocation-contract
release_scope=no-network-adapter-sandbox-only
