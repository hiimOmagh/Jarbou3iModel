...
Manual workflow baseline: `v1.3.0-alpha.10 — Brief Publication Pack v4`. Do not patch v1.4.0-alpha.1, v1.3.0, rc.2, or alpha.10 further.

## Scope

Planning/preflight only. Planning/control-plane only:
...
- Required targeted checks: `tests/provider-execution-threat-model-check.mjs`, `tests/provider-execution-preflight-check.mjs`, `tests/provider-source-execution-policy-matrix-check.mjs`, `tests/provider-source-failure-ux-contracts-check.mjs`.
- Required CI: `npm run test:ci:no-browser`, `npm run test:ci:browser`.

Feature surface is frozen except planning/control-plane artifacts; no live execution is enabled.
...
Machine tokens: runtime_capability_change=false; release_type=provider-source-execution-policy-failure-ux-contracts; release_scope=planning-control-plane-only; test:ci:browser required.
...
Public Demo boundary: v1.4.0-alpha.2 keeps the locked public demo/manual workflow behavior unchanged while defining policy and failure UX contracts.
