...
Locked baseline completed immediately before this candidate: `v1.4.0-alpha.14 — Adapter Replay Fixture Corpus + Coverage Matrix`.
...
Planning/control-plane only. Roadmap lock completion + UX/runtime-budget guard:

- Mark alpha.14 as locked across release truth, roadmap, QA, and release-evidence docs.
- Make alpha.15 the current candidate.
- Remove obsolete pre-lock wording from current release docs.
- Compress manual provider adapter/replay evidence review copy.
- Preserve alpha.14 Adapter Replay Fixture Corpus + Coverage Matrix unchanged as locked baseline evidence.
- Add static evidence/runtime budget metadata: browser check budget max 20, hosted languages expected 3, hosted surfaces max 13, visible snapshot rows max 39.
...
```text
evidence_surface_budget_version: 1.4.0-alpha.15
...
- Required targeted checks: `tests/manual-provider-adapter-ux-compression-evidence-runtime-budget-check.mjs`, `tests/adapter-replay-fixture-corpus-coverage-matrix-check.mjs`, `tests/adapter-contract-test-bench-no-network-invocation-replay-qa-check.mjs`, `tests/manual-provider-adapter-sandbox-ephemeral-invocation-contract-check.mjs`, `tests/manual-execution-safety-cockpit-session-ledger-check.mjs`, `tests/limited-manual-live-execution-prototype-check.mjs`, `tests/controlled-execution-candidate-gate-check.mjs`, `tests/credential-boundary-runtime-drill-check.mjs`, `tests/source-acquisition-control-surface-check.mjs`, `tests/provider-execution-mock-to-live-equivalence-check.mjs`.
- Required CI: `npm run test:ci:no-browser`, `npm run test:ci:browser`.

Feature surface is frozen except static UX/runtime-budget guard metadata and public evidence-copy compression; no live execution, live fetching, real credential handling, backend/storage expansion, or source behavior expansion is enabled.

Machine tokens: runtime_capability_change=false; release_type=roadmap-lock-completion-manual-provider-adapter-ux-compression; release_scope=manual-provider-adapter-ux-compression-runtime-budget-guard-only; test:ci:browser required.
...
release_type=roadmap-lock-completion-manual-provider-adapter-ux-compression
release_scope=manual-provider-adapter-ux-compression-runtime-budget-guard-only
