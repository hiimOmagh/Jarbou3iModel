...
Purpose: mark locked alpha.17 as the completed baseline, make alpha.18 the current candidate, and convert evidence-budget regression results into pass, warn, fail, and review-required operator actions without expanding browser scope.
...
## Next after lock
Do not start the next milestone until alpha.18 has green no-browser CI, green browser CI, hosted evidence, and canonical lock evidence bundle.
