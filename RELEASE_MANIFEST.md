# Release Manifest — v1.1.0-alpha.2
...
- Release name: `Expansion Lane Acceptance Criteria Matrix`
- Release type: patch
...
## Required v1.1.0-alpha.2 release-lock files
...
v1.1.0-alpha.2 must not change provider behavior, OAuth behavior, backend endpoint behavior, live source connector behavior, storage model, source verification behavior, or public-demo capability surface. The patch locks release-application integrity only.

## Required apply sequence

```bash
unzip -o jarbou3i-research-engine-v1.1.0-alpha.2-expansion-lane-acceptance-criteria-matrix-patch.zip -d .
