...
const currentReleaseContract = readJson(path.join(root, 'tests/current-release-contract.json'));
const evidenceMatrixConfig = readJson(path.join(root, 'tests/evidence/evidence-matrix.config.json'));
const publicVersionLabel = evidenceMatrixConfig.public_version_label || `v${version} ${currentReleaseContract.milestone_name}`;
const release = `v${version} — ${currentReleaseContract.milestone_name}`;
