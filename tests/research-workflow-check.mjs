...
if (schema.properties?.workflow_version?.const !== '1.3.0-alpha.2') fail('workflow_version const mismatch');
...
if (!fixture.analysis_template || fixture.analysis_template.template_version !== '1.3.0-alpha.2' || fixture.analysis_template.template_id !== 'strategic_analysis_engine') fail('fixture needs analysis template metadata');
const plan = fixture.research_plan;
const evidence = fixture.evidence_matrix;
const causalLinks = fixture.causal_links;
if (fixture.workflow_version !== '1.3.0-alpha.2') fail('fixture workflow version mismatch');
if (!plan || plan.plan_version !== '1.3.0-alpha.2' || plan.analysis_template_id !== fixture.analysis_template.template_id) fail('fixture missing templated research plan');
...
if (!fixture.analysis_brief || fixture.analysis_brief.brief_version !== '1.3.0-alpha.2' || fixture.analysis_brief.analysis_template?.template_id !== fixture.analysis_template.template_id || !fixture.analysis_brief.template_fit_report || !fixture.analysis_brief.quality_gate_report) fail('fixture needs templated compiled analysis brief with quality gate report');
...
if (!fixture.diagnostics || fixture.diagnostics.diagnostics_version !== '1.3.0-alpha.2') fail('fixture needs diagnostics');
...
if (!fixture.project_workspace || fixture.project_workspace.workspace_version !== '1.3.0-alpha.2' || fixture.project_workspace.storage_mode !== 'local_only' || !fixture.project_workspace.quota) fail('fixture needs local-only project workspace metadata');
if (!fixture.quality_gate || fixture.quality_gate.quality_gate_version !== '1.3.0-alpha.2' || typeof fixture.quality_gate.overall_score !== 'number' || !Array.isArray(fixture.quality_gate.fix_actions)) fail('fixture needs Advanced Quality Gate v3 metadata');
if (!fixture.backend_hardening || fixture.backend_hardening.hardening_version !== '1.3.0-alpha.2' || fixture.backend_hardening.audit_policy?.prompt_logged !== false || fixture.backend_hardening.release_gate !== 'backend_hardening_checked') fail('fixture needs backend hardening metadata');

if (!fixture.privacy_export || fixture.privacy_export.guard_version !== '1.3.0-alpha.2' || fixture.privacy_export.key_exported !== false || fixture.privacy_export.raw_token_exported !== false) fail('fixture needs privacy export guard metadata');
...
if (!fixture.source_to_brief_workbench || fixture.source_to_brief_workbench.source_to_brief_version !== '1.3.0-alpha.2' || !Array.isArray(fixture.source_to_brief_workbench.claim_map) || !fixture.source_to_brief_workbench.confidence_review || fixture.source_to_brief_workbench.automatic_source_verification_claimed !== false || fixture.source_to_brief_workbench.live_fetching_performed !== false) fail('fixture needs source-to-brief workbench with no automatic verification claim');
...
if (!fixture.ai_runs.every((run) => run.run_version === '1.3.0-alpha.2' && run.provider && run.task && run.response_contract && run.response_validation && run.repair_trace && run.provider_safety && run.provider_identity && run.provider_billing_policy)) fail('fixture provider runs need contracts, identity, billing, and safety metadata');
