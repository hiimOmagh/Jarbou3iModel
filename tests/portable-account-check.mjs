...
const disconnected = mod.disconnected('1.1.0-alpha.13');
...
const connected = mod.connect({version:'1.1.0-alpha.13', accountHint:'fixture'});
...
const status = mod.status(connected, {version:'1.1.0-alpha.13'});
if(!status.connected || !status.token_present) fail('status must mark connected token presence');
const refreshed = mod.refresh(connected, {version:'1.1.0-alpha.13'});
if(refreshed.token_hash === connected.token_hash) fail('refresh should rotate mock token hash');
const exported = mod.exportableStatus(refreshed, {version:'1.1.0-alpha.13'});
