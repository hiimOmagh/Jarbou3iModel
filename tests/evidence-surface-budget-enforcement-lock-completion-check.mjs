...
const TARGET_MODULE = 'src/research/evidence-surface-budget-enforcement-evidence-runtime-budget.js';
const LEGACY_BUDGET_MODULE = 'src/research/manual-provider-adapter-ux-compression-evidence-runtime-budget.js';
const TARGET_CHECK = 'tests/evidence-surface-budget-enforcement-lock-completion-check.mjs';
...
for (const text of [current, roadmap, changelog, releaseHistory, releaseEvidence, qa, readme, publicDemo]) {
  assert.ok(text.includes(RELEASE) || text.includes(PUBLIC_LABEL), 'alpha.16 current candidate identity missing');
...
assert.ok(current.includes('Status: current candidate. Lock is pending green no-browser CI'), 'alpha.16 must be current candidate before its own lock');
assert.equal(/alpha\.15[^\n.]*lock pending/i.test(current), false, 'current release must not contain alpha.15 lock pending wording');
assert.equal(/alpha\.15[^\n.]*Lock is pending/i.test(current), false, 'current release must not contain alpha.15 Lock is pending wording');
...
assert.ok(!roadmap.includes('v1.4.0-alpha.15 — Evidence Surface Budget Enforcement'), 'roadmap must not reuse alpha.15 as current candidate');
...
assert.ok(index.includes('content="1.4.0-alpha.16"'));
assert.ok(index.includes('src="src/research/evidence-surface-budget-enforcement-evidence-runtime-budget.js" defer'));
...
for (const gate of ['no-browser','current-no-browser','provider','release']) {
  assert.ok(ciRegistry.gates[gate].node_checks.includes(LEGACY_BUDGET_CHECK) || ciRegistry.gates[gate].node_checks.includes('tests/evidence-surface-budget-enforcement-evidence-runtime-budget-check.mjs'), `${gate} must preserve alpha.15 budget compatibility check`);
}
...
assert.ok(workflow.includes('lock-evidence-bundle_1.4.0-alpha.16_${{ github.run_id }}'));
assert.ok(workflow.includes("summary.internal_build_version !== '1.4.0-alpha.16'"));
...
console.log('Evidence surface budget enforcement and alpha.15 lock-completion checks passed.');
process.exit(0);
