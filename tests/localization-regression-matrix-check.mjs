...
assert.equal(matrixConfig.public_version_labels.en, 'v1.4.0-alpha.38 Source-to-Brief Operator Continuity Console', 'English public version label must identify v1.4.0-alpha.28');
assert.equal(matrixConfig.public_version_labels.ar, 'v1.4.0-alpha.38 وحدة استمرارية المشغّل من المصدر إلى الموجز', 'Arabic public version label must identify the current release');
assert.equal(matrixConfig.public_version_labels.fr, 'v1.4.0-alpha.38 Console de continuité opérateur source-vers-brief', 'French public version label must identify the current release');
...
for (const body of arabicHostedDemoBodies) {
  assert.ok(body.includes('وحدة استمرارية المشغّل من المصدر إلى الموجز') || body.includes('فجوات الأدلة'), 'Arabic current-release description must identify alpha.38 source-to-brief continuity console');
  for (const staleCurrentDescription of [
    'النموذج الأولي المحدود للتنفيذ الحي اليدوي جاهز لأدلة الإصدار',
    'قمرة أمان التنفيذ اليدوي + سجل الجلسة جاهزة لأدلة الإصدار',
    'صندوق رمل محوّل المزوّد اليدوي + عقد الاستدعاء العابر جاهز لأدلة الإصدار'
