...
assert.equal(matrixConfig.public_version_labels.en, 'v1.4.0-alpha.34 Adapter Replay Review Pack Triage Workbench', 'English public version label must identify v1.4.0-alpha.28');
assert.equal(matrixConfig.public_version_labels.ar, 'v1.4.0-alpha.34 منضدة فرز حزمة مراجعة إعادة التشغيل', 'Arabic public version label must identify the current release');
assert.equal(matrixConfig.public_version_labels.fr, 'v1.4.0-alpha.34 Workbench de triage du pack de revue de rejeu', 'French public version label must identify the current release');
...
for (const body of arabicHostedDemoBodies) {
  assert.ok(body.includes('منضدة فرز حزمة مراجعة إعادة التشغيل') || body.includes('بطاقات الفرز'), 'Arabic current-release description must identify alpha.33 triage workbench');
  for (const staleCurrentDescription of [
    'النموذج الأولي المحدود للتنفيذ الحي اليدوي جاهز لأدلة الإصدار',
    'قمرة أمان التنفيذ اليدوي + سجل الجلسة جاهزة لأدلة الإصدار',
    'صندوق رمل محوّل المزوّد اليدوي + عقد الاستدعاء العابر جاهز لأدلة الإصدار'
