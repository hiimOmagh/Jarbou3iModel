...
assert.equal(matrixConfig.public_version_labels.en, 'v1.4.0-alpha.42 Manual Workflow UX Consolidation', 'English public version label must identify v1.4.0-alpha.28');
assert.equal(matrixConfig.public_version_labels.ar, 'v1.4.0-alpha.42 دمج تجربة سير العمل اليدوي', 'Arabic public version label must identify the current release');
assert.equal(matrixConfig.public_version_labels.fr, 'v1.4.0-alpha.42 Consolidation UX du workflow manuel', 'French public version label must identify the current release');
...
for (const body of arabicHostedDemoBodies) {
  assert.ok(body.includes('دمج تجربة سير العمل اليدوي') || body.includes('مسار العمل الأساسي') || body.includes('خريطة دمج الأسطح'), 'Arabic current-release description must identify alpha.42 manual workflow UX consolidation');
  for (const staleCurrentDescription of [
    'النموذج الأولي المحدود للتنفيذ الحي اليدوي جاهز لأدلة الإصدار',
    'قمرة أمان التنفيذ اليدوي + سجل الجلسة جاهزة لأدلة الإصدار',
    'صندوق رمل محوّل المزوّد اليدوي + عقد الاستدعاء العابر جاهز لأدلة الإصدار'
