...
for (const body of arabicHostedDemoBodies) {
  assert.ok(
    body.includes('توقيت الالتقاط') ||
    body.includes('ميزانية المهلة') ||
    body.includes('أدلة الاستضافة') ||
    body.includes('فشل متذبذب'),
    'Arabic current-release description must identify alpha.58 hosted evidence timeout budget guard'
  );
  for (const staleCurrentDescription of [
    'النموذج الأولي المحدود للتنفيذ الحي اليدوي جاهز لأدلة الإصدار',
    'قمرة أمان التنفيذ اليدوي + سجل الجلسة جاهزة لأدلة الإصدار',
