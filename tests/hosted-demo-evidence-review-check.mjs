...
const smoke = hosted.buildHostedDemoSmokeFixes({ metadata_artifact_written:false }, { version:'1.0.17', now:'2026-05-01T00:00:00.000Z' });
assert.equal(smoke.release_gate, 'hosted_demo_smoke_blocked');
assert.equal(smoke.fail_count, 1);
const smokeOk = hosted.buildHostedDemoSmokeFixes({}, { version:'1.0.17', now:'2026-05-01T00:00:00.000Z' });
...
const review = hosted.buildHostedDemoEvidenceReview({ provider_mode:false }, { version:'1.0.17', now:'2026-05-01T00:00:00.000Z' });
assert.equal(review.release_gate, 'evidence_review_blocked');
assert.equal(review.missing_review_count, 1);
const reviewOk = hosted.buildHostedDemoEvidenceReview({}, { version:'1.0.17', now:'2026-05-01T00:00:00.000Z' });
...
for (const token of [
  ...
  assert.ok(docs.includes(token), `v1.0.17 doc missing ${token}`);
}
