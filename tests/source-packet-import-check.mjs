...
const migrationFixture = JSON.parse(read('fixtures/migrations/v1.0.13-packet.json'));
const privacyFixture = JSON.parse(read('fixtures/privacy/browser-generated-export-v1.0.13.json'));
...
assert.equal(importer.VERSION, '1.0.13');
...
assert.equal(schema.$defs.source_import_report.properties.import_version.const, '1.0.13');
...
for (const packet of [fixture, migrationFixture, privacyFixture]) {
  ...
  assert.equal(packet.source_import_report.import_version, '1.0.13');
