...
assert.equal(scorer.VERSION, '1.1.0-alpha.20');
...
assert.equal(schema.$defs.evidence_scoring.properties.scorer_version.const, '1.1.0-alpha.20');
...
assert.equal(schema.$defs.evidence_scoring_report.properties.evidence_scoring_version.const, '1.1.0-alpha.20');
...
for (const packet of [fixture, migrationFixture, privacyFixture]) {
  ...
  assert.equal(packet.evidence_scoring_report.evidence_scoring_version, '1.1.0-alpha.20');
  ...
  for (const item of packet.evidence_matrix) {
    ...
    assert.equal(item.evidence_scoring.scorer_version, '1.1.0-alpha.20');
