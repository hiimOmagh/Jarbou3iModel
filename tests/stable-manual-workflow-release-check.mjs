...
for (const text of [current, roadmap, changelog, publicDemo]) {
  assert.ok(text.includes(RELEASE) || text.includes(PUBLIC_LABEL), 'release docs must expose alpha.3 identity');
...
assert.ok(roadmap.includes('v1.4.0-alpha.5'));

assert.ok(workflow.includes('lock-evidence-bundle_1.4.0-alpha.5_${{ github.run_id }}'), 'CI workflow must upload alpha.5 lock bundle');
assert.ok(workflow.includes("summary.internal_build_version !== '1.4.0-alpha.5'"), 'CI workflow must assert alpha.5 evidence matrix version');
