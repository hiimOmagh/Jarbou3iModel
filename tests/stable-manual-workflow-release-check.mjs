...
const PUBLIC_LABEL = 'v1.4.0-alpha.1 Controlled Provider/Source Execution Preparation';
...
assert.ok(pkg.description.includes(PUBLIC_LABEL), 'package description must expose alpha.1 public label');
...
assert.equal(manifest.release_type, 'controlled-provider-source-execution-preparation');
...
assert.ok(manifest.release_scope.includes('Planning/preflight milestone'));
assert.ok(manifest.release_scope.includes(STABLE_BASELINE));

for (const text of [current, roadmap, changelog, publicDemo]) {
  assert.ok(text.includes(RELEASE) || text.includes(PUBLIC_LABEL), 'release docs must expose alpha.1 identity');
...
assert.ok(workflow.includes('lock-evidence-bundle_1.4.0-alpha.1_${{ github.run_id }}'), 'CI workflow must upload alpha.1 lock bundle');
assert.ok(workflow.includes("summary.internal_build_version !== '1.4.0-alpha.1'"), 'CI workflow must assert alpha.1 evidence matrix version');
