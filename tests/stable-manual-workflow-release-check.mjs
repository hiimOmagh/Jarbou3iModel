...
for (const text of [current, roadmap, changelog, publicDemo]) {
  assert.ok(text.includes(RELEASE) || text.includes(PUBLIC_LABEL), 'release docs must expose alpha.7 identity');
...
assert.ok(roadmap.includes('v1.4.0-alpha.7'));

assert.ok(workflow.includes('lock-evidence-bundle_1.4.0-alpha.7_${{ github.run_id }}'), 'CI workflow must upload alpha.7 lock bundle');
assert.ok(workflow.includes("summary.internal_build_version !== '1.4.0-alpha.7'"), 'CI workflow must assert alpha.7 evidence matrix version');
