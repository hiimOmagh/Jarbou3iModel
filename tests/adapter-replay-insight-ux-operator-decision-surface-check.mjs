...
assert.ok([VERSION, '1.4.0-alpha.28', '1.4.0-alpha.30'].includes(registry.runtime_optimization.version), 'runtime optimization may advance to alpha.27 while preserving alpha.26 insight check');
assert.ok(['adapter_replay_insight_ux_operator_decision_surface', 'adapter_replay_review_pack_operator_handoff_export', 'release_identity_single_source_contract'].includes(registry.runtime_optimization.optimization_scope), 'runtime optimization scope may advance to alpha.27 while preserving alpha.26 insight check');
