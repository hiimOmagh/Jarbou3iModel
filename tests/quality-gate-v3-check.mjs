...
assert.equal(gate.QUALITY_GATE_VERSION, '1.1.0-rc.1-copyfix.1');
...
const strongState = {
  ...
  packet_migration_report: { ok: true, import_safe: true, target_version: '1.1.0-rc.1-copyfix.1', removed_sensitive_fields: [] },
...
assert.equal(report.quality_gate_version, '1.1.0-rc.1-copyfix.1');
...
for (const key of ['completeness', 'evidence_strength', 'evidence_reliability', 'attention_signal_integrity', 'contradiction_coverage', 'source_diversity', 'actor_layer_coverage', 'causal_link_density', 'provider_safety', 'privacy_safety', 'migration_safety', 'template_fit']) {
...
assert.equal(schema.properties.quality_gate.properties.quality_gate_version.const, '1.1.0-rc.1-copyfix.1');

const fixture = JSON.parse(fs.readFileSync('fixtures/research/sample-research-workflow-en.json', 'utf8'));
assert.equal(fixture.quality_gate.quality_gate_version, '1.1.0-rc.1-copyfix.1');
assert.equal(fixture.analysis_brief.quality_gate_report.quality_gate_version, '1.1.0-rc.1-copyfix.1');
assert.ok(Array.isArray(fixture.quality_gate.fix_actions));
assert.equal(typeof fixture.quality_gate.dimensions.evidence_reliability, 'number');
assert.equal(typeof fixture.quality_gate.dimensions.attention_signal_integrity, 'number');
