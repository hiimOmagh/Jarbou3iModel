...
assert.equal(gate.QUALITY_GATE_VERSION, '1.0.25');
...
const strongState = {
  ...
  packet_migration_report: { ok: true, import_safe: true, target_version: '1.0.25', removed_sensitive_fields: [] },
...
assert.equal(report.quality_gate_version, '1.0.25');
...
assert.equal(schema.properties.quality_gate.properties.quality_gate_version.const, '1.0.25');

const fixture = JSON.parse(fs.readFileSync('fixtures/research/sample-research-workflow-en.json', 'utf8'));
assert.equal(fixture.quality_gate.quality_gate_version, '1.0.25');
assert.equal(fixture.analysis_brief.quality_gate_report.quality_gate_version, '1.0.25');
