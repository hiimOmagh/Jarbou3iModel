...
assert.equal(templates.VERSION, '1.3.0-alpha.6');
assert.equal(schema.properties.workflow_version.const, '1.3.0-alpha.6');
assert.equal(schema.$defs.source_packet_template_report.properties.template_report_version.const, '1.3.0-alpha.6');
...
for (const packet of [fixture, migrationFixture, privacyFixture]) {
  ...
  assert.equal(packet.source_packet_template_report.template_report_version, '1.3.0-alpha.6');
