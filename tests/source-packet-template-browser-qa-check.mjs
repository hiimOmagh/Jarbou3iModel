...
assert.equal(templates.VERSION, '1.0.30');
assert.equal(schema.properties.workflow_version.const, '1.0.30');
assert.equal(schema.$defs.source_packet_template_report.properties.template_report_version.const, '1.0.30');
...
for (const packet of [fixture, migrationFixture, privacyFixture]) {
  ...
  assert.equal(packet.source_packet_template_report.template_report_version, '1.0.30');
