...
assert.ok(['limited_manual_live_execution_prototype','manual_execution_safety_cockpit_session_ledger','manual_provider_adapter_sandbox_ephemeral_invocation_contract'].includes(registry.runtime_optimization.optimization_scope), 'runtime optimization may point to current alpha while preserving alpha.10 check');
assert.ok(['1.4.0-alpha.10','1.4.0-alpha.11','1.4.0-alpha.12'].includes(registry.runtime_optimization.version), 'runtime optimization version may point to current alpha while preserving alpha.10 check');
