...
assert.ok(['manual_provider_adapter_sandbox_ephemeral_invocation_contract','adapter_contract_test_bench_no_network_invocation_replay_qa','adapter_replay_fixture_corpus_coverage_matrix','manual_provider_adapter_ux_compression_evidence_runtime_budget','handoff_productivity_runbook_gate', 'adapter_replay_insight_ux_operator_decision_surface', 'adapter_replay_review_pack_operator_handoff_export', 'release_identity_single_source_contract'].includes(registry.runtime_optimization.optimization_scope), 'runtime optimization scope may point to current alpha while preserving alpha.12 check');
assert.ok([VERSION,'1.4.0-alpha.28', '1.4.0-alpha.30'].includes(registry.runtime_optimization.version), 'runtime optimization version may point to current alpha while preserving alpha.12 check');
