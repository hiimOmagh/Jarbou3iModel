...
const registries = [
  {
    ...
    expectedEntries: 56,
    currentFixture: () => getMigrationFixture('fixtures/migrations/v1.1.0-alpha.8-packet.json')
  },
  {
    label: 'privacy',
    ...
    expectedEntries: 46,
    currentFixture: () => getPrivacyFixture('fixtures/privacy/browser-generated-export-v1.1.0-alpha.8.json')
  }
];
...
for (const item of registries) {
  ...
  assert.ok(currentBytes <= REGISTRY_BUDGET_BYTES, `${item.label} registry exceeds alpha.8 byte budget: ${currentBytes}`);
