...
assert.equal(pkg.version, '1.4.0-alpha.5', 'package.json must identify v1.4.0-alpha.5');
assert.equal(lock.version, '1.4.0-alpha.5', 'package-lock root version must identify v1.4.0-alpha.5');
assert.equal(lock.packages[''].version, '1.4.0-alpha.5', 'package-lock package root must identify v1.4.0-alpha.5');
...
for (const text of [releaseNotes, changelog, manifest, roadmap, qaMatrix]) {
  assert.ok(text.includes('v1.4.0-alpha.5') || text.includes('v1.3.0'), 'release documentation must mention current alpha.1 or stable baseline');
