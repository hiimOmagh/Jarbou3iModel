...
assert.equal(builder.VERSION, '1.1.0-alpha.11');
assert.equal(importer.VERSION, '1.1.0-alpha.11');
assert.equal(roundtrip.VERSION, '1.1.0-alpha.11');
...
assert.equal(result.packet.workflow_version, '1.1.0-alpha.11');
...
assert.equal(schema.$defs.source_packet_roundtrip_report.properties.roundtrip_version.const, '1.1.0-alpha.11');
...
for (const packet of [fixture, migrationFixture, privacyFixture]) {
  ...
  assert.equal(packet.source_packet_roundtrip_report.roundtrip_version, '1.1.0-alpha.11');
