...
const ARTIFACT = 'jarbou3i-research-engine-v1.1.0-rc.2-fix.2-evidence-pack-export-v3-brief-traceability-patch.zip';
...
const releaseDoc = readReleaseDoc('docs/v1.1.0-rc.2-fix.2-evidence-pack-export-v3-brief-traceability.md');

assert.equal(pkg.version, VERSION, 'package version must be v1.1.0-rc.2-fix.2');
assert.equal(lock.version, VERSION, 'package-lock root version must be v1.1.0-rc.2-fix.2');
assert.equal(lock.packages[''].version, VERSION, 'package-lock package version must be v1.1.0-rc.2-fix.2');
assert.ok(index.includes('v1.1.0-rc.2-fix.2 · Evidence Matrix + Canonical Bundle Validation'), 'index badge must expose v1.1.0-rc.2-fix.2 visual-freeze identity');
...
for (const packet of [migrationFixture, privacyFixture]) {
  assert.equal(packet.workflow_version, VERSION, 'current v1.1.0-rc.2-fix.2 fixtures must identify the target release');
  assert.equal(packet.release_notes.release_title, RELEASE, 'release notes title must match v1.1.0-rc.2-fix.2 visual freeze');
  ...
  assert.equal(packet.release_apply_integrity.artifact_name, ARTIFACT, 'release apply integrity artifact must match v1.1.0-rc.2-fix.2 patch ZIP');
...
assert.equal(schema.properties.workflow_version.const, VERSION, 'schema workflow version must be v1.1.0-rc.2-fix.2');
assert.ok(migrations.includes("const TARGET_VERSION = '1.1.0-rc.2-fix.2'"), 'migration target must be v1.1.0-rc.2-fix.2');
assert.ok(migrations.includes("'1.0.28','1.0.29','1.0.30','1.1.0-alpha.1','1.1.0-alpha.2','1.1.0-alpha.3','1.1.0-alpha.8','1.1.0-alpha.9','1.1.0-alpha.10','1.1.0-rc.2-fix.2'"), 'migration order must preserve v1.0.30 freeze baseline and append v1.1.0-rc.2-fix.2');
...
assert.ok(releaseDocExists('docs/v1.1.0-rc.2-fix.2-evidence-pack-export-v3-brief-traceability.md'), 'v1.1.0-rc.2-fix.2 release doc must exist');
assert.ok(releaseDoc.includes('Mobile Header Geometry Lock'), 'v1.1.0-rc.2-fix.2 release doc must state the visual-freeze scope');
assert.ok(releaseDoc.includes('No live scraping') || releaseDoc.includes('live scraping'), 'v1.1.0-rc.2-fix.2 release doc must preserve public-demo honesty boundaries');
