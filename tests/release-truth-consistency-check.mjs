...
const CONTROL_BASELINE = 'v1.4.0-alpha.1 — Controlled Provider/Source Execution Preparation';
...
for (const [name, text] of Object.entries({ current, roadmap, readme, changelog, publicDemo })) {
  assert.ok(text.includes(RELEASE) || text.includes(PUBLIC_LABEL), `${name} must expose alpha.2 release identity`);
...
assert.ok(current.includes(CONTROL_BASELINE), 'current-release must preserve alpha.1 control baseline');
