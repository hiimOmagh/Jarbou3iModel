...
assert.ok(pkg.description.includes(PUBLIC_LABEL), 'package description must expose stable public label');
...
for (const [name, text] of Object.entries({ current, roadmap, readme, changelog, publicDemo })) {
  assert.ok(text.includes(RELEASE) || text.includes(PUBLIC_LABEL), `${name} must expose stable release identity`);
...
assert.ok(current.includes(`Last locked release: \`${LOCKED_BASELINE}\``), 'current-release must explicitly mark rc.2 as locked baseline');
...
assert.ok(current.includes('Status: built locally. Lock is pending green no-browser CI, green browser CI'), 'current-release must describe stable lock state precisely');
assert.ok(current.includes('Stable Manual Workflow Release'));
