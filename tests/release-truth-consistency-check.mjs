...
const RELEASE = 'v1.3.0-alpha.7 — Release Truth Sweep + Roadmap Compression';
const PUBLIC_LABEL = 'v1.3.0-alpha.7 Release Truth Sweep + Roadmap Compression';
const LOCKED_BASELINE = 'v1.3.0-alpha.6 — Operator Signoff State + Export Lock Ledger';
...
assert.ok(manifest.release_scope.includes('Release-truth and roadmap compression pass'));
...
assert.ok(current.includes('Last locked release: `v1.3.0-alpha.6 — Operator Signoff State + Export Lock Ledger`'), 'current-release must explicitly mark alpha.6 as locked baseline');
assert.equal(current.includes('built locally, no-browser validated pending browser lock evidence'), false, 'current-release must not keep stale alpha.6 pending-browser wording');
assert.ok(current.includes('Status: built locally. Lock is pending green no-browser CI, green browser CI'), 'current-release must describe alpha.7 lock state precisely');

assert.ok(roadmap.includes('v1.3.0-alpha.8 — Signed Export Handoff Pack + Lock Ledger Review Surface'));
