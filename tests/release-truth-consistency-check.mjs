...
for (const [name, text] of Object.entries({ current, roadmap, readme, changelog, publicDemo })) {
  assert.ok(text.includes(RELEASE) || text.includes(PUBLIC_LABEL), `${name} must expose alpha.1 release identity`);
...
assert.ok(current.includes('Status: built locally. Lock is pending green no-browser CI, green browser CI'), 'current-release must describe alpha.1 pre-lock state precisely');
assert.ok(current.includes('Provider execution threat model'));
assert.ok(current.includes('Provider execution preflight gate'));
