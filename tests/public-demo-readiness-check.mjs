...
assert.equal(publicDemo.VERSION, '1.1.0-alpha.23');
...
assert.ok(index.includes('v1.1.0-alpha.23 · Publication Review Gate + Claim Boundary Audit'), 'public demo badge missing');
...
assert.equal(schema.properties.public_demo.properties.public_demo_version.const, '1.1.0-alpha.23');
assert.equal(schema.properties.release_notes.properties.release_notes_version.const, '1.1.0-alpha.23');

const readiness = publicDemo.buildPublicDemoReadiness({}, { version:'1.1.0-alpha.23', now:'2026-04-30T00:00:00.000Z' });
...
const blocked = publicDemo.buildPublicDemoReadiness({ release_notes_polished:false }, { version:'1.1.0-alpha.23', now:'2026-04-30T00:00:00.000Z' });
assert.equal(blocked.release_gate, 'public_demo_blocked');
assert.equal(blocked.fail_count, 1);

const notes = publicDemo.releaseNotes({ version:'1.1.0-alpha.23', now:'2026-04-30T00:00:00.000Z' });
...
for (const packet of [fixture, migrationFixture, privacyFixture]) {
  assert.equal(packet.public_demo.public_demo_version, '1.1.0-alpha.23');
  ...
  assert.equal(packet.release_notes.release_notes_version, '1.1.0-alpha.23');
