...
assert.equal(hosted.VERSION, '1.1.0-rc.2-fix.2');
...
assert.ok(index.includes('v1.1.0-rc.2-fix.2 · Evidence Matrix + Canonical Bundle Validation'), 'v1.1.0-rc.2-fix.2 badge missing');
...
assert.equal(schema.properties.hosted_demo_verification.properties.hosted_demo_version.const, '1.1.0-rc.2-fix.2');
assert.equal(schema.properties.browser_evidence_capture.properties.browser_evidence_version.const, '1.1.0-rc.2-fix.2');
assert.equal(schema.properties.hosted_demo_smoke_fixes.properties.smoke_fixes_version.const, '1.1.0-rc.2-fix.2');
assert.equal(schema.properties.hosted_demo_evidence_review.properties.evidence_review_version.const, '1.1.0-rc.2-fix.2');

const report = hosted.buildHostedDemoVerification({}, { version:'1.1.0-rc.2-fix.2', now:'2026-05-01T00:00:00.000Z' });
...
const blocked = hosted.buildHostedDemoVerification({ mobile_evidence_captured:false }, { version:'1.1.0-rc.2-fix.2', now:'2026-05-01T00:00:00.000Z' });
assert.equal(blocked.release_gate, 'hosted_demo_blocked');
assert.equal(blocked.fail_count, 1);

const evidence = hosted.buildBrowserEvidence({}, { version:'1.1.0-rc.2-fix.2', now:'2026-05-01T00:00:00.000Z' });
...
const smoke = hosted.buildHostedDemoSmokeFixes({}, { version:'1.1.0-rc.2-fix.2', now:'2026-05-01T00:00:00.000Z' });
...
const review = hosted.buildHostedDemoEvidenceReview({}, { version:'1.1.0-rc.2-fix.2', now:'2026-05-01T00:00:00.000Z' });
...
for (const packet of [fixture, migrationFixture, privacyFixture]) {
  assert.equal(packet.workflow_version, '1.1.0-rc.2-fix.2');
  assert.equal(packet.hosted_demo_verification.hosted_demo_version, '1.1.0-rc.2-fix.2');
  ...
  assert.equal(packet.browser_evidence_capture.browser_evidence_version, '1.1.0-rc.2-fix.2');
