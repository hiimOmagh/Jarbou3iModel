...
assert.equal(schema.properties.workflow_version.const, VERSION, 'workflow schema target must identify v1.0.29');
assert.equal(sample.workflow_version, VERSION, 'sample workflow must identify v1.0.29');
...
for (const packet of [sample, migrationFixture, privacyFixture]) {
  assert.equal(packet.release_notes.release_title, RELEASE, 'release notes title must identify v1.0.29 release lock');
...
assert.equal(publicDemo.VERSION, VERSION, 'public demo module must identify v1.0.29');
...
assert.ok(migrations.includes("const TARGET_VERSION = '1.0.29'"), 'migrations target must be v1.0.29');
assert.ok(migrations.includes("'1.0.23','1.0.24','1.0.25','1.0.26','1.0.27','1.0.28','1.0.29'"), 'migration order must preserve v1.0.24 and append v1.0.29');
