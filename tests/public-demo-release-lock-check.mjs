...
const TITLE = 'Release Provenance Ledger Gate';
...
const releaseDoc = `${read('docs/v1.0.25-public-demo-release-lock.md')}\n${read('docs/v1.0.27-release-provenance-ledger-gate.md')}`;

assert.equal(pkg.version, VERSION, 'package.json must identify v1.0.27');
assert.equal(lock.version, VERSION, 'package-lock root version must identify v1.0.27');
assert.equal(lock.packages[''].version, VERSION, 'package-lock package root must identify v1.0.27');
assert.equal(schema.properties.workflow_version.const, VERSION, 'workflow schema target must identify v1.0.27');
assert.equal(sample.workflow_version, VERSION, 'sample workflow must identify v1.0.27');
assert.equal(migrationFixture.workflow_version, VERSION, 'migration fixture must identify v1.0.27');
assert.equal(privacyFixture.workflow_version, VERSION, 'privacy fixture must identify v1.0.27');

for (const packet of [sample, migrationFixture, privacyFixture]) {
  assert.equal(packet.release_notes.release_title, RELEASE, 'release notes title must identify v1.0.27 release lock');
...
assert.equal(publicDemo.VERSION, VERSION, 'public demo module must identify v1.0.27');
...
assert.ok(migrations.includes("const TARGET_VERSION = '1.0.27'"), 'migrations target must be v1.0.27');
assert.ok(migrations.includes("'1.0.23','1.0.24','1.0.25','1.0.26','1.0.27'"), 'migration order must preserve v1.0.24 and append v1.0.27');
...
for (const corpus of [readme, releaseNotes, releaseManifest, changelog, publicDemoGuide, browserEvidence, hostedDemo, roadmap, qaMatrix, architecture, aiIntegration, privacyAudit, releaseDoc]) {
  assert.ok(corpus.includes('v1.0.27') || corpus.includes('1.0.27'), 'release corpus must mention v1.0.27');
