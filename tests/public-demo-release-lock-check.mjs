...
assert.equal(schema.properties.workflow_version.const, VERSION, 'workflow schema target must identify v1.1.0-alpha.5');
assert.equal(sample.workflow_version, VERSION, 'sample workflow must identify v1.1.0-alpha.5');
...
for (const packet of [sample, migrationFixture, privacyFixture]) {
  assert.equal(packet.release_notes.release_title, RELEASE, 'release notes title must identify v1.1.0-alpha.5 release lock');
...
assert.equal(publicDemo.VERSION, VERSION, 'public demo module must identify v1.1.0-alpha.5');
...
assert.ok(migrations.includes("const TARGET_VERSION = '1.1.0-alpha.5'"), 'migrations target must be v1.1.0-alpha.5');
assert.ok(migrations.includes("'1.0.23','1.0.24','1.0.25','1.0.26','1.0.27','1.0.28','1.0.29','1.0.30','1.1.0-alpha.1','1.1.0-alpha.2','1.1.0-alpha.3','1.1.0-alpha.5'"), 'migration order must preserve v1.0.24 and append v1.1.0-alpha.5');
...
assert.ok(pkg.scripts['test:version-registry']?.includes('version-suite-registry-check.mjs'), 'package must expose v125 no-browser suite');
assert.ok(pkg.scripts['test:version-registry']?.includes('version-suite-registry-check.mjs'), 'package must expose v125 browser gate composition');
