...
const PATCH_ARTIFACT = 'jarbou3i-research-engine-v1.1.0-alpha.14-prompt-compiler-research-plan-upgrade-patch.zip';
...
for (const version of ['v0.11.0','v1.0.30','v1.1.0-alpha.3','v1.1.0-alpha.14']) {
  assert.ok(getMigrationFixture(`fixtures/migrations/${version}-packet.json`), `${version} migration fixture must load from registry`);
}
for (const version of ['v1.0.24','v1.0.30','v1.1.0-alpha.3','v1.1.0-alpha.14']) {
...
const currentMigration = getMigrationFixture('fixtures/migrations/v1.1.0-alpha.14-packet.json');
const currentPrivacy = getPrivacyFixture('fixtures/privacy/browser-generated-export-v1.1.0-alpha.14.json');
...
assert.ok(releaseDocExists(`docs/v${VERSION}-prompt-compiler-research-plan-upgrade.md`));
