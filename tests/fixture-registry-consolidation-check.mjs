...
const PATCH_ARTIFACT = 'jarbou3i-research-engine-v1.1.0-alpha.9-test-matrix-runtime-optimization-release-doc-timeline-pruning-patch.zip';
...
assert.equal(migrationRegistry.entries.length, 57, 'migration registry preserves historical packets plus current alpha.9');
...
assert.equal(privacyRegistry.entries.length, 47, 'privacy registry preserves historical exports plus current alpha.9');
...
for (const version of ['v0.11.0','v1.0.30','v1.1.0-alpha.3','v1.1.0-alpha.9']) {
  assert.ok(getMigrationFixture(`fixtures/migrations/${version}-packet.json`), `${version} migration fixture must load from registry`);
}
for (const version of ['v1.0.24','v1.0.30','v1.1.0-alpha.3','v1.1.0-alpha.9']) {
...
const currentMigration = getMigrationFixture('fixtures/migrations/v1.1.0-alpha.9-packet.json');
const currentPrivacy = getPrivacyFixture('fixtures/privacy/browser-generated-export-v1.1.0-alpha.9.json');
...
assert.ok(releaseDocExists(`docs/v${VERSION}-test-matrix-runtime-optimization-release-doc-timeline-pruning.md`));
