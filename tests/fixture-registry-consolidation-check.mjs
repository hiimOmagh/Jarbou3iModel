...
const PATCH_ARTIFACT = 'jarbou3i-research-engine-v1.1.0-alpha.11-fixture-test-debt-ledger-source-file-refactor-readiness-audit-patch.zip';
...
assert.equal(migrationRegistry.entries.length, 58, 'migration registry preserves historical packets plus current alpha.10');
...
assert.equal(privacyRegistry.entries.length, 48, 'privacy registry preserves historical exports plus current alpha.10');
...
for (const version of ['v0.11.0','v1.0.30','v1.1.0-alpha.3','v1.1.0-alpha.11']) {
  assert.ok(getMigrationFixture(`fixtures/migrations/${version}-packet.json`), `${version} migration fixture must load from registry`);
}
for (const version of ['v1.0.24','v1.0.30','v1.1.0-alpha.3','v1.1.0-alpha.11']) {
...
const currentMigration = getMigrationFixture('fixtures/migrations/v1.1.0-alpha.11-packet.json');
const currentPrivacy = getPrivacyFixture('fixtures/privacy/browser-generated-export-v1.1.0-alpha.11.json');
...
assert.ok(releaseDocExists(`docs/v${VERSION}-fixture-test-debt-ledger-source-file-refactor-readiness-audit.md`));
