...
const signedPacket = {
  ...
  operator_signoff_input:{
    ...
    notes:'Reviewed Brief Publication Pack v4 appendices and lock ledger.'
  }
};
...
assert.ok(markdown.includes('Brief Publication Pack v4'));
...
assert.ok(pack.files.find((file)=>file.path === 'source-to-brief/brief-publication-pack-v4.md').content.includes('Brief Publication Pack v4'));
...
for (const marker of ['briefPublicationPackPanel','brief-publication-pack-v4','briefPublicationFiles']) assert.ok(renderer.includes(marker), `renderer missing ${marker}`);
const helpers = fs.readFileSync('src/research/render-helpers.js','utf8');
for (const marker of ['Brief Publication Pack v4','حزمة النشر v4','Brief Publication Pack v4']) assert.ok(helpers.includes(marker), `localized copy missing ${marker}`);
