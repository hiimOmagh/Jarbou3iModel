...
assert.equal(pkg.version, '1.3.0-alpha.9', 'package version must be v1.1.0');
assert.equal(pkg.type, 'module', 'package.json must declare package-level ESM mode');
assert.ok(worker.includes("const VERSION = '1.3.0-alpha.9'"), 'Worker proxy version must be v1.1.0');
