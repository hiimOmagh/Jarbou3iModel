...
assert.equal(migrations.TARGET_VERSION, '1.1.0-rc.2-fix.3');
...
assert.ok(fixtureNames.includes('v1.1.0-rc.2-fix.3-packet.json'), 'alpha.10 migration coverage must be present in registry');

for (const entry of entries) {
  const file = entry.file;
  const input = entry.packet;
  const result = migrations.migrateResearchPacket(input, { targetVersion: '1.1.0-rc.2-fix.3' });
  assert.equal(result.ok, true, `${file} should migrate`);
  assert.equal(result.packet.workflow_version, '1.1.0-rc.2-fix.3', `${file} workflow version`);
  assert.equal(result.packet.research_plan.plan_version, '1.1.0-rc.2-fix.3', `${file} plan version`);
  ...
  assert.equal(result.packet.analysis_template.template_version, '1.1.0-rc.2-fix.3', `${file} analysis template version`);
  assert.equal(result.packet.analysis_template.template_id, 'strategic_analysis_engine', `${file} default analysis template`);
  assert.equal(result.packet.quality_gate.quality_gate_version, '1.1.0-rc.2-fix.3', `${file} quality gate version`);
  assert.equal(result.packet.onboarding.onboarding_version, '1.1.0-rc.2-fix.3', `${file} onboarding version`);
  assert.equal(result.packet.onboarding.release_gate, 'first_run_success_checked', `${file} onboarding release gate`);
  assert.equal(result.packet.release_candidate.release_candidate_version, '1.1.0-rc.2-fix.3', `${file} release candidate version`);
  ...
  assert.equal(result.packet.export_pack.export_pack_version, '1.1.0-rc.2-fix.3', `${file} export pack version`);
  assert.equal(result.packet.export_pack.format, 'export_pack_v3', `${file} export pack format`);
  assert.equal(result.packet.backend_hardening.hardening_version, '1.1.0-rc.2-fix.3', `${file} backend hardening version`);
  assert.equal(result.packet.backend_hardening.audit_policy.prompt_logged, false, `${file} backend audit prompt logging disabled`);
  assert.equal(result.packet.browser_qa_hardening.hardening_version, '1.1.0-rc.2-fix.3', `${file} browser QA hardening version`);
  ...
  assert.equal(result.packet.hosted_demo_verification.hosted_demo_version, '1.1.0-rc.2-fix.3', `${file} hosted demo verification version`);
  assert.equal(result.packet.hosted_demo_verification.release_gate, 'hosted_demo_verified', `${file} hosted demo release gate`);
  assert.equal(result.packet.browser_evidence_capture.browser_evidence_version, '1.1.0-rc.2-fix.3', `${file} browser evidence version`);
  ...
  assert.equal(result.packet.hosted_demo_smoke_fixes.smoke_fixes_version, '1.1.0-rc.2-fix.3', `${file} hosted demo smoke fixes version`);
  ...
  assert.equal(result.packet.hosted_demo_evidence_review.evidence_review_version, '1.1.0-rc.2-fix.3', `${file} hosted demo evidence review version`);
  ...
  assert.equal(result.packet.release_apply_integrity.release_apply_integrity_version, '1.1.0-rc.2-fix.3', `${file} release apply integrity version`);
  ...
  assert.equal(result.packet.packet_migration_report.target_version, '1.1.0-rc.2-fix.3', `${file} report target`);
...
const redacted = migrations.migrateResearchPacket(unsafeLegacy, { targetVersion: '1.1.0-rc.2-fix.3' });
