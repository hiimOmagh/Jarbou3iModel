...
const TARGET_MODULE = 'src/research/manual-provider-adapter-ux-compression-evidence-runtime-budget.js';
const TARGET_CHECK = 'tests/manual-provider-adapter-ux-compression-evidence-runtime-budget-check.mjs';
...
assert.ok(!roadmap.includes('v1.4.0-alpha.14 — Manual Provider Adapter UX Compression'), 'roadmap must not reuse alpha.14 for the next UX milestone');
...
assert.ok(index.includes('content="1.4.0-alpha.15"'));
assert.ok(index.includes(PUBLIC_LABEL));
assert.ok(index.includes('Manual Provider Adapter UX Compression'));
assert.ok(index.includes('src/research/manual-provider-adapter-ux-compression-evidence-runtime-budget.js'));
...
assert.ok(workflow.includes('lock-evidence-bundle_1.4.0-alpha.15_${{ github.run_id }}'));
assert.ok(workflow.includes("summary.internal_build_version !== '1.4.0-alpha.15'"));
