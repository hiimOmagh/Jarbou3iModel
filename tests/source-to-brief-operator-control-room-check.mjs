...
assert.equal(controlRoom.manual_workflow_ux_consolidation_version, VERSION);
...
assert.equal(controlRoom.control_room_safety_contract.manual_workflow_ux_consolidation_only, true);
...
assert.ok(index.includes('Manual Workflow UX Consolidation'), 'index must expose current control room title');
assert.ok(index.includes('src="src/research/manual-workflow-ux-consolidation.js" defer'), 'index must load current control room module');
assert.ok(index.includes('data-browser-qa="manual-workflow-ux-consolidation"'), 'index must expose current control room browser QA surface');
