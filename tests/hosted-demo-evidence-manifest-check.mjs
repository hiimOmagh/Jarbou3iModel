...
assert.ok(migrations.includes("const TARGET_VERSION = '1.1.0-alpha.2'"));
assert.ok(migrations.includes("'1.0.26','1.0.27','1.0.28','1.0.29','1.0.30','1.1.0-alpha.1','1.1.0-alpha.2'"), 'migration order must preserve v1.0.28 and append v1.1.0-alpha.2');
...
for (const corpus of [
  ...
  assert.ok(/Expansion Lane Acceptance Criteria Matrix|planning gate|evidence manifest|single final metadata|capture manifest|visual freeze|mobile header/i.test(corpus), 'release corpus must describe evidence manifest gate');
}
