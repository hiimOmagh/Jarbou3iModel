...
assert.ok(index.includes('Adapter Contract Test Bench + No-Network Invocation Replay QA'), 'index must expose alpha.13 label');
assert.ok(index.includes('data-browser-qa="adapter-contract-test-bench-no-network-invocation-replay-qa"'), 'index must expose browser QA alpha.13 card');
...
assert.equal(registry.runtime_optimization.optimization_scope, 'adapter_contract_test_bench_no_network_invocation_replay_qa');
