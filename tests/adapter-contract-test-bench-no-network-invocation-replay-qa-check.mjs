...
assert.ok(index.includes('Source-to-Brief Publication Readiness Suite') || index.includes('Adapter Contract Test Bench + No-Network Invocation Replay QA'), 'index must expose current adapter replay/contract QA label');
...
assert.ok([VERSION, '1.4.0-alpha.28', '1.4.0-alpha.40', '1.4.0-alpha.40'].includes(registry.runtime_optimization.version), 'runtime optimization may advance to alpha.14 while preserving alpha.13 check');
