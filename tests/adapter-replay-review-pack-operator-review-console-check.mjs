...
const MILESTONE = 'v1.4.0-alpha.36 — Adapter Replay Review Pack Operator Review Console';
...
assert.equal(mod.MODEL, 'adapter_replay_review_pack_operator_review_console.v1');
...
assert.equal(reviewConsole.adapter_replay_review_pack_operator_review_console_version, VERSION);
...
assert.ok(index.includes('Adapter Replay Review Pack Operator Review Console'), 'index must expose alpha.36 visible title');
...
assert.equal(registry.runtime_optimization.optimization_scope, 'adapter_replay_review_pack_operator_review_console');
