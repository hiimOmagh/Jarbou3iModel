...
assert.ok(researchCopy.en.evidenceReviewGateBody.includes('Controlled Connector Architecture MVP'), 'English evidence review gate body must describe alpha.18 controlled connector scope');
assert.ok(researchCopy.ar.evidenceReviewGateBody.includes('معمارية موصلات'), 'Arabic evidence review gate body must describe alpha.18 controlled connector scope');
assert.ok(researchCopy.fr.evidenceReviewGateBody.includes('architecture de connecteurs contrôlés'), 'French evidence review gate body must describe alpha.18 controlled connector scope');
assert.equal(researchCopy.fr.evidenceReviewGateBody.includes('preserves the public-demo freeze'), false, 'French alpha.18 evidence review gate body must not inherit English release copy');
assert.equal(researchCopy.fr.evidenceReviewGateBody.includes('Source packets remain local/manual'), false, 'French alpha.18 evidence review gate body must not expose English source-packet release copy');
