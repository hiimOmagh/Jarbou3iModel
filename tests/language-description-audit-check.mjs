...
assert.ok(researchCopy.en.evidenceReviewGateBody.includes('final repo hygiene audit') && researchCopy.en.evidenceReviewGateBody.includes('stale release-copy sweep'), 'English evidence review gate body must describe alpha.25 final hygiene scope');
assert.ok(researchCopy.ar.evidenceReviewGateBody.includes('تدقيق نظافة المستودع') && researchCopy.ar.evidenceReviewGateBody.includes('كنس نسخ الإصدار القديمة'), 'Arabic evidence review gate body must describe alpha.25 final hygiene scope');
assert.ok(researchCopy.fr.evidenceReviewGateBody.includes('audit d’hygiène') && researchCopy.fr.evidenceReviewGateBody.includes('balayage des copies obsolètes'), 'French evidence review gate body must describe alpha.25 final hygiene scope');
assert.equal(researchCopy.fr.evidenceReviewGateBody.includes('preserves the public-demo freeze'), false, 'French alpha.25 evidence review gate body must not inherit English release copy');
assert.equal(researchCopy.fr.evidenceReviewGateBody.includes('provider routing'), false, 'French alpha.25 evidence review gate body must not expose alpha.20 provider-router release copy');
