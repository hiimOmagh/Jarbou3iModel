...
assert.equal(fixture.release_candidate.stable_release_version, '1.2.0-alpha.2');
...
assert.ok(renderHelpers.includes('v1.2.0-alpha.2 Source-to-Brief Intelligence Workbench') && renderHelpers.includes('Source-to-Brief Intelligence Workbench'), 'English stable badge must be fixed');
assert.ok(renderHelpers.includes('Ù…Ø±Ø´Ø­ Ù…Ø³ØªÙ‚Ø±'), 'Arabic release badge must be professional and current');
...
assert.ok(renderHelpers.includes('Ø§Ù„Ø¯Ù…Ø¬ Ø§Ù„Ù…Ø³ØªÙ‚Ø±') && renderHelpers.includes('v1.1.0'), 'Arabic hosted evidence body must describe rc.1 canonical evidence bundling scope');
...
assert.ok(renderHelpers.includes("v1.1.0 Â· Candidat dÃ©mo publique"), 'French alpha badge must reflect Source-to-Brief Intelligence Workbench');
assert.ok(renderHelpers.includes('verrou cohÃ©rence export') || renderHelpers.includes('cohÃ©rence Export Pack v3'), 'French rc.0 hosted evidence body must describe export consistency lock');
