...
assert.equal(fixture.release_candidate.stable_release_version, '1.1.0-alpha.25');
...
assert.ok(migrations.includes("'1.0.0','1.0.1','1.0.2','1.0.3','1.0.4','1.0.5','1.0.6','1.0.7','1.0.8','1.0.9','1.0.10','1.0.11','1.0.12','1.0.13','1.0.14','1.0.15','1.0.16','1.0.17','1.0.18','1.0.19','1.0.20','1.0.21','1.0.22','1.0.23','1.0.24','1.0.25','1.0.26','1.0.27','1.0.28','1.0.29','1.0.30','1.1.0-alpha.1','1.1.0-alpha.2','1.1.0-alpha.3','1.1.0-alpha.8','1.1.0-alpha.9','1.1.0-alpha.10','1.1.0-alpha.25'"), 'migrations must support v1.0.5 → v1.1.0-alpha.25');
...
assert.ok(renderHelpers.includes("alphaBadge:'v1.1.0-alpha.25 · Release Candidate Hardening + Final Repo Hygiene'"), 'English stable badge must be fixed');
assert.ok(renderHelpers.includes('تقوية مرشح الإصدار وتنظيف المستودع النهائي'), 'Arabic release badge must be professional and current');
assert.ok(renderHelpers.includes('Durcissement RC + hygiène finale du dépôt'), 'French release badge must be professional and current');
assert.ok(renderHelpers.includes('final repo hygiene audit') && renderHelpers.includes('stale release-copy sweep'), 'English hosted evidence body must describe alpha.25 final hygiene scope');
assert.ok(renderHelpers.includes('تدقيق نظافة المستودع') && renderHelpers.includes('كنس نسخ الإصدار القديمة'), 'Arabic hosted evidence body must describe alpha.25 final hygiene scope');
assert.ok(renderHelpers.includes('audit d’hygiène') && renderHelpers.includes('balayage des copies obsolètes'), 'French hosted evidence body must describe alpha.25 final hygiene scope');
...
assert.ok(renderHelpers.includes("v1.1.0-alpha.25 · Durcissement RC + hygiène finale du dépôt"), 'French alpha badge must reflect Release Candidate Hardening + Final Repo Hygiene');
assert.ok(renderHelpers.includes('cohérence Export Pack v3'), 'French alpha.25 hosted evidence body must describe claim boundary audit scope');
assert.equal(renderHelpers.includes("evidenceReviewGateBody:'v1.1.0-alpha.25 preserves the public-demo freeze"), false, 'French evidence review copy must not inherit English alpha.20 prose');
