...
assert.equal(registry.VERSION, '1.1.0-alpha.11');
...
assert.ok(registryHasMigrationFixture('fixtures/migrations/v1.1.0-alpha.11-packet.json'));
assert.ok(registryHasPrivacyFixture('fixtures/privacy/browser-generated-export-v1.1.0-alpha.11.json'));

const blueprint = registry.strategyBlueprint({version:'1.1.0-alpha.11', now:'2026-05-02T00:00:00.000Z'});
assert.equal(blueprint.source_strategy_version, '1.1.0-alpha.11');
...
for (const packet of [fixture, migrationFixture, privacyFixture]) {
  ...
  assert.equal(packet.source_capability_registry.source_strategy_version, '1.1.0-alpha.11');
  ...
  assert.equal(packet.release_notes.release_title, 'v1.1.0-alpha.11 — Fixture/Test Debt Ledger + Source-File Refactor Readiness Audit');
}
...
assert.equal(schema.$defs.source_capability_registry.properties.source_strategy_version.const, '1.1.0-alpha.11');
...
for (const corpus of [manifest, changelog, roadmap, qaMatrix]) {
  ...
  assert.ok(corpus.includes('Fixture/Test Debt Ledger + Source-File Refactor Readiness Audit'), 'release corpus missing v1.1.0-alpha.11 title');
}
