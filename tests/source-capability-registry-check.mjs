...
assert.equal(registry.VERSION, '1.0.16');
...
assert.ok(fs.existsSync('docs/v1.0.16-source-packet-builder-ui-scoring-review-controls.md'));
assert.ok(fs.existsSync('tests/v112-no-browser-suite.mjs'));
assert.ok(fs.existsSync('fixtures/migrations/v1.0.16-packet.json'));
assert.ok(fs.existsSync('fixtures/privacy/browser-generated-export-v1.0.16.json'));

const blueprint = registry.strategyBlueprint({version:'1.0.16', now:'2026-05-02T00:00:00.000Z'});
assert.equal(blueprint.source_strategy_version, '1.0.16');
...
for (const packet of [fixture, migrationFixture, privacyFixture]) {
  ...
  assert.equal(packet.source_capability_registry.source_strategy_version, '1.0.16');
  ...
  assert.equal(packet.release_notes.release_title, 'v1.0.16 — Source Packet Builder UI + Scoring Review Controls');
}
...
assert.equal(schema.$defs.source_capability_registry.properties.source_strategy_version.const, '1.0.16');
...
for (const corpus of [manifest, changelog, roadmap, qaMatrix]) {
  ...
  assert.ok(corpus.includes('Source Packet Builder UI + Scoring Review Controls'), 'release corpus missing v1.0.16 title');
}
