...
for (const corpus of [manifest, changelog, roadmap, qaMatrix]) {
  assert.ok(corpus.includes('v1.4.0-alpha.17') || corpus.includes('v1.3.0'), 'release corpus missing current alpha.1 or stable baseline');
  assert.ok(corpus.includes('Evidence Budget Regression Dashboard') || corpus.includes('Stable Manual Workflow Release'), 'release corpus missing current alpha.1 or stable baseline title');
}
