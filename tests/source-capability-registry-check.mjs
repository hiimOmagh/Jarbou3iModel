...
for (const corpus of [manifest, changelog, roadmap, qaMatrix]) {
  assert.ok(corpus.includes('v1.4.0-alpha.12') || corpus.includes('v1.3.0'), 'release corpus missing current alpha.1 or stable baseline');
  assert.ok(corpus.includes('Manual Provider Adapter Sandbox + Ephemeral Invocation Contract') || corpus.includes('Stable Manual Workflow Release'), 'release corpus missing current alpha.1 or stable baseline title');
}
