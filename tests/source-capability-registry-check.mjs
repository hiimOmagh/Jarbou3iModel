...
for (const corpus of [manifest, changelog, roadmap, qaMatrix]) {
  ...
  assert.ok(corpus.includes('Adapter Replay Review Pack + Operator Handoff Export') || corpus.includes('Stable Manual Workflow Release'), 'release corpus missing current alpha.1 or stable baseline title');
}
