...
assert.equal(registry.VERSION, '1.3.0-alpha.2');
...
assert.ok(registryHasMigrationFixture('fixtures/migrations/v1.3.0-alpha.2-packet.json'));
assert.ok(registryHasPrivacyFixture('fixtures/privacy/browser-generated-export-v1.3.0-alpha.2.json'));

const blueprint = registry.strategyBlueprint({version:'1.3.0-alpha.2', now:'2026-05-02T00:00:00.000Z'});
assert.equal(blueprint.source_strategy_version, '1.3.0-alpha.2');
...
for (const packet of [fixture, migrationFixture, privacyFixture]) {
  ...
  assert.equal(packet.source_capability_registry.source_strategy_version, '1.3.0-alpha.2');
  ...
  assert.equal(packet.release_notes.release_title, 'v1.3.0-alpha.2 — Guided Session UX Compression + Brief Assembly Export QA');
}
...
assert.equal(schema.$defs.source_capability_registry.properties.source_strategy_version.const, '1.3.0-alpha.2');
