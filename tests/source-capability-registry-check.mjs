...
assert.equal(registry.VERSION, '1.0.20');
...
assert.ok(fs.existsSync('fixtures/migrations/v1.0.20-packet.json'));
assert.ok(fs.existsSync('fixtures/privacy/browser-generated-export-v1.0.20.json'));

const blueprint = registry.strategyBlueprint({version:'1.0.20', now:'2026-05-02T00:00:00.000Z'});
assert.equal(blueprint.source_strategy_version, '1.0.20');
...
for (const packet of [fixture, migrationFixture, privacyFixture]) {
  ...
  assert.equal(packet.source_capability_registry.source_strategy_version, '1.0.20');
...
assert.equal(schema.$defs.source_capability_registry.properties.source_strategy_version.const, '1.0.20');
...
for (const corpus of [manifest, changelog, roadmap, qaMatrix]) {
  ...
  assert.ok(corpus.includes('Source Packet Template Browser QA + Copy Safety'), 'release corpus missing v1.0.20 title');
}
