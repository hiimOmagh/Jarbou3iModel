...
assert.equal(registry.VERSION, '1.1.0-alpha.23');
...
const blueprint = registry.strategyBlueprint({version:'1.1.0-alpha.23', now:'2026-05-02T00:00:00.000Z'});
assert.equal(blueprint.source_strategy_version, '1.1.0-alpha.23');
...
for (const packet of [fixture, migrationFixture, privacyFixture]) {
  ...
  assert.equal(packet.source_capability_registry.source_strategy_version, '1.1.0-alpha.23');
...
assert.equal(schema.$defs.source_capability_registry.properties.source_strategy_version.const, '1.1.0-alpha.23');
...
for (const corpus of [manifest, changelog, roadmap, qaMatrix]) {
  ...
  assert.ok(corpus.includes('Publication Review Gate + Claim Boundary Audit'), 'release corpus missing v1.1.0-alpha.23 title');
}
