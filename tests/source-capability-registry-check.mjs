...
assert.equal(registry.VERSION, '1.0.19');
...
assert.ok(fs.existsSync('docs/v1.0.19-source-packet-template-presets.md'));
assert.ok(fs.existsSync('tests/v112-no-browser-suite.mjs'));
assert.ok(fs.existsSync('fixtures/migrations/v1.0.19-packet.json'));
assert.ok(fs.existsSync('fixtures/privacy/browser-generated-export-v1.0.19.json'));

const blueprint = registry.strategyBlueprint({version:'1.0.19', now:'2026-05-02T00:00:00.000Z'});
assert.equal(blueprint.source_strategy_version, '1.0.19');
...
for (const packet of [fixture, migrationFixture, privacyFixture]) {
  ...
  assert.equal(packet.source_capability_registry.source_strategy_version, '1.0.19');
  ...
  assert.equal(packet.release_notes.release_title, 'v1.0.19 — Source Packet Template Presets');
}
...
assert.equal(schema.$defs.source_capability_registry.properties.source_strategy_version.const, '1.0.19');
...
for (const corpus of [manifest, changelog, roadmap, qaMatrix]) {
  ...
  assert.ok(corpus.includes('Source Packet Template Presets'), 'release corpus missing v1.0.19 title');
}
