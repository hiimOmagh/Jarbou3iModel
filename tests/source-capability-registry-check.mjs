...
for (const corpus of [manifest, changelog, roadmap, qaMatrix]) {
  assert.ok(corpus.includes('v1.4.0-alpha.9') || corpus.includes('v1.3.0'), 'release corpus missing current alpha.1 or stable baseline');
  assert.ok(corpus.includes('Controlled Execution Candidate Gate') || corpus.includes('Stable Manual Workflow Release'), 'release corpus missing current alpha.1 or stable baseline title');
}
