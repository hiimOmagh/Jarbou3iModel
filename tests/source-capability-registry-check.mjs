...
assert.equal(registry.VERSION, '1.1.0-alpha.3');
...
assert.ok(fs.existsSync('fixtures/migrations/v1.1.0-alpha.3-packet.json'));
assert.ok(fs.existsSync('fixtures/privacy/browser-generated-export-v1.1.0-alpha.3.json'));

const blueprint = registry.strategyBlueprint({version:'1.1.0-alpha.3', now:'2026-05-02T00:00:00.000Z'});
assert.equal(blueprint.source_strategy_version, '1.1.0-alpha.3');
...
for (const packet of [fixture, migrationFixture, privacyFixture]) {
  ...
  assert.equal(packet.source_capability_registry.source_strategy_version, '1.1.0-alpha.3');
...
assert.equal(schema.$defs.source_capability_registry.properties.source_strategy_version.const, '1.1.0-alpha.3');
...
for (const corpus of [manifest, changelog, roadmap, qaMatrix]) {
  ...
  assert.ok(corpus.includes('CI Result Review + Browser Evidence Artifact Audit'), 'release corpus missing v1.1.0-alpha.3 title');
}
