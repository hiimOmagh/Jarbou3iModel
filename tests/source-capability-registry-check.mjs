...
assert.equal(registry.VERSION, '1.1.0-alpha.20');
...
const blueprint = registry.strategyBlueprint({version:'1.1.0-alpha.20', now:'2026-05-02T00:00:00.000Z'});
assert.equal(blueprint.source_strategy_version, '1.1.0-alpha.20');
...
for (const packet of [fixture, migrationFixture, privacyFixture]) {
  ...
  assert.equal(packet.source_capability_registry.source_strategy_version, '1.1.0-alpha.20');
...
assert.equal(schema.$defs.source_capability_registry.properties.source_strategy_version.const, '1.1.0-alpha.20');
...
for (const corpus of [manifest, changelog, roadmap, qaMatrix]) {
  ...
  assert.ok(corpus.includes('Provider Router + Cost-Aware Run Ledger'), 'release corpus missing v1.1.0-alpha.20 title');
}
