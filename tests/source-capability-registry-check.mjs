...
assert.equal(registry.VERSION, '1.1.0-alpha.24');
...
const blueprint = registry.strategyBlueprint({version:'1.1.0-alpha.24', now:'2026-05-02T00:00:00.000Z'});
assert.equal(blueprint.source_strategy_version, '1.1.0-alpha.24');
...
for (const packet of [fixture, migrationFixture, privacyFixture]) {
  ...
  assert.equal(packet.source_capability_registry.source_strategy_version, '1.1.0-alpha.24');
...
assert.equal(schema.$defs.source_capability_registry.properties.source_strategy_version.const, '1.1.0-alpha.24');
...
for (const corpus of [manifest, changelog, roadmap, qaMatrix]) {
  ...
  assert.ok(corpus.includes('Golden Workflow Corpus + End-to-End Demo Run'), 'release corpus missing v1.1.0-alpha.24 title');
}
