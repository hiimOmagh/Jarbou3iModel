...
assert.equal(registry.VERSION, '1.0.12');
...
assert.ok(fs.existsSync('docs/v1.0.12-research-source-strategy-blueprint.md'));
assert.ok(fs.existsSync('tests/v112-no-browser-suite.mjs'));
assert.ok(fs.existsSync('fixtures/migrations/v1.0.12-packet.json'));
assert.ok(fs.existsSync('fixtures/privacy/browser-generated-export-v1.0.12.json'));

const blueprint = registry.strategyBlueprint({version:'1.0.12', now:'2026-05-02T00:00:00.000Z'});
assert.equal(blueprint.source_strategy_version, '1.0.12');
...
assert.equal(blueprint.audit.verdict, 'source_capability_registry_ready_no_runtime_expansion');
assert.ok(blueprint.registry.some((item) => item.source_id === 'manual_private' && item.demo_visibility === 'show'));
assert.ok(blueprint.registry.some((item) => item.source_id === 'manual_source_packet' && item.status === 'planned_next_safe_increment'));
...
for (const packet of [fixture, migrationFixture, privacyFixture]) {
  ...
  assert.equal(packet.source_capability_registry.source_strategy_version, '1.0.12');
  ...
  assert.equal(packet.source_capability_registry.audit.verdict, 'source_capability_registry_ready_no_runtime_expansion');
  assert.equal(packet.release_notes.release_title, 'v1.0.12 — Research Source Strategy Blueprint');
}
...
assert.equal(schema.$defs.source_capability_registry.properties.source_strategy_version.const, '1.0.12');
...
for (const corpus of [manifest, changelog, roadmap, qaMatrix]) {
  ...
  assert.ok(corpus.includes('Research Source Strategy Blueprint'), 'release corpus missing v1.0.12 title');
}
