...
const CURRENT_PUBLIC = 'v1.4.0-alpha.27 Adapter Replay Decision Drilldown + Evidence Trace Links';
...
assert.equal(registry.release_title, 'v1.4.0-alpha.27 — Adapter Replay Decision Drilldown + Evidence Trace Links');
assert.ok(index.includes('content="1.4.0-alpha.27"'), 'hosted metadata must report the alpha.15 internal version');
...
assert.ok(current.includes(LOCKED_ALPHA14));
assert.ok(readme.includes(LOCKED_ALPHA14));
assert.ok(publicDemo.includes(LOCKED_ALPHA14));
...
assert.ok(current.includes('no live execution') || current.includes('Feature surface is frozen'));
...
console.log('Stable baseline and alpha.15 public-surface checks passed.');
process.exit(0);
