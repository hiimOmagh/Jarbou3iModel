...
const scorerText = read('src/research/evidence-scorer.js');
...
new vm.Script(scorerText).runInContext(context);
...
if (!parsed.evidence.every((e) => e.evidence_scoring?.scoring_version === 'evidence_scoring.v1')) fail('imported evidence needs source packet builder browser QA and UX tightening');
...
if (fixture.workflow_version !== '1.1.0-rc.1-copyfix.1') fail('research fixture version mismatch');
...
if (!fixture.source_imports.some((item) => item.queue_only === true)) fail('source imports should be queue_only in v1.1.0-rc.1-copyfix.1');
