...
const RELEASE = 'v1.4.0-alpha.37 — Adapter Replay Review Pack Compact Navigation UX';
...
assert.ok(index.includes('v1.4.0-alpha.37') && (index.includes('Adapter Replay Review Pack Compact Navigation UX') || fs.readFileSync('src/research/render-helpers.js', 'utf8').includes('Adapter Replay Review Pack Compact Navigation UX')), 'index/release helpers must expose alpha.30 release identity');
...
assert.ok(arabicHostedDemoBodies.some((body)=>body.includes('تجربة التنقل المختصر لحزمة مراجعة إعادة التشغيل') || body.includes('خريطة انتقال سريعة')), 'Arabic current-release description must identify alpha.37 compact navigation UX');
...
assert.ok(publicDemo.includes('1.4.0-alpha.37') && /hosted.*evidence|hosted.*metadata/i.test(publicDemo), 'public demo must state hosted evidence lock requirement');
