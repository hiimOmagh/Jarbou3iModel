...
assert.ok(arabicHostedDemoBodies.some((body)=>body.includes('التقاط أدلة الاستضافة المستهدفة') || body.includes('لقطات محددة بالمحدّد') || body.includes('ربط المنطقة بالادعاء')), 'Arabic current-release description must identify alpha.43 targeted hosted evidence capture');
...
assert.ok(publicDemo.includes('1.4.0-alpha.43') && /hosted.*evidence|hosted.*metadata/i.test(publicDemo), 'public demo must state hosted evidence lock requirement');
