...
assert.ok(index.includes('v1.4.0-alpha.15 Manual Provider Adapter UX Compression') && (index.includes('Runtime-Budget Guard Only') || fs.readFileSync('src/research/render-helpers.js', 'utf8').includes('Runtime-Budget Guard Only')), 'index visible badge must show alpha.15 manual provider adapter UX compression identity');
...
assert.ok(arabicHostedDemoBodies.some((body)=>body.includes('ضغط تجربة محوّل المزوّد اليدوي جاهز لأدلة الإصدار')), 'Arabic current-release description must identify alpha.15 manual provider adapter UX compression');
...
assert.ok(publicDemo.includes('1.4.0-alpha.15') && /hosted.*evidence|hosted.*metadata/i.test(publicDemo), 'public demo must state hosted evidence lock requirement');
