...
assert.ok(arabicHostedDemoBodies.some((body)=>body.includes('دلالات مصفوفة الأدلة') || body.includes('تقوية الإثبات المستهدف') || body.includes('لقطات محددة بالمحدّد') || body.includes('ربط المنطقة بالادعاء')), 'Arabic current-release description must identify canonical alpha.44 evidence semantics and targeted proof hardening');
...
assert.ok(publicDemo.includes(CURRENT_VERSION) && /hosted.*evidence|hosted.*metadata|Evidence Matrix Semantics|Targeted Proof Hardening/i.test(publicDemo), 'public demo must state current hosted evidence lock requirement');
