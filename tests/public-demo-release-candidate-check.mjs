...
const RELEASE = 'v1.4.0-alpha.42 — Manual Workflow UX Consolidation';
...
assert.ok(index.includes('v1.4.0-alpha.42') && (index.includes('Manual Workflow UX Consolidation') || fs.readFileSync('src/research/render-helpers.js', 'utf8').includes('Manual Workflow UX Consolidation')), 'index/release helpers must expose alpha.30 release identity');
...
assert.ok(arabicHostedDemoBodies.some((body)=>body.includes('دمج تجربة سير العمل اليدوي') || body.includes('مسار العمل الأساسي') || body.includes('خريطة دمج الأسطح')), 'Arabic current-release description must identify alpha.42 manual workflow UX consolidation');
...
assert.ok(publicDemo.includes('1.4.0-alpha.42') && /hosted.*evidence|hosted.*metadata/i.test(publicDemo), 'public demo must state hosted evidence lock requirement');
