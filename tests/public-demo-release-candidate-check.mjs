...
const CURRENT_TITLE = 'Alpha.17 Lock Completion + Evidence Budget Dashboard Actionability';
const VERSION = '1.3.0';
...
assert.ok(index.includes('v1.4.0-alpha.18 Evidence Budget Dashboard Actionability') && (index.includes('Runtime-Budget Guard Only') || fs.readFileSync('src/research/render-helpers.js', 'utf8').includes('Runtime-Budget Guard Only')), 'index visible badge must show alpha.18 evidence budget dashboard actionability identity');
...
assert.ok(arabicHostedDemoBodies.some((body)=>body.includes('قابلية عمل لوحة ميزانية الأدلة جاهزة لأدلة الإصدار')), 'Arabic current-release description must identify alpha.18 evidence budget dashboard actionability');
...
assert.ok(publicDemo.includes('1.4.0-alpha.18') && /hosted.*evidence|hosted.*metadata/i.test(publicDemo), 'public demo must state hosted evidence lock requirement');
