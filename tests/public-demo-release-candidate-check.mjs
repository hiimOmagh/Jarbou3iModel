...
const CURRENT_TITLE = 'Alpha.16 Lock Completion + Evidence Budget Regression Dashboard';
const VERSION = '1.3.0';
...
assert.ok(index.includes('v1.4.0-alpha.17 Evidence Budget Regression Dashboard') && (index.includes('Runtime-Budget Guard Only') || fs.readFileSync('src/research/render-helpers.js', 'utf8').includes('Runtime-Budget Guard Only')), 'index visible badge must show alpha.15 evidence budget regression dashboard identity');
...
assert.ok(arabicHostedDemoBodies.some((body)=>body.includes('ضغط تجربة محوّل المزوّد اليدوي جاهز لأدلة الإصدار')), 'Arabic current-release description must identify alpha.15 evidence budget regression dashboard');
...
assert.ok(publicDemo.includes('1.4.0-alpha.17') && /hosted.*evidence|hosted.*metadata/i.test(publicDemo), 'public demo must state hosted evidence lock requirement');
