...
const CURRENT_TITLE = 'Alpha.18 Lock Completion + Evidence Dashboard Decision Ledger';
const VERSION = '1.3.0';
...
assert.ok(index.includes('v1.4.0-alpha.19 Evidence Dashboard Decision Ledger') && (index.includes('Runtime-Budget Guard Only') || fs.readFileSync('src/research/render-helpers.js', 'utf8').includes('Runtime-Budget Guard Only')), 'index visible badge must show alpha.18 evidence dashboard decision ledger identity');
...
assert.ok(publicDemo.includes('1.4.0-alpha.19') && /hosted.*evidence|hosted.*metadata/i.test(publicDemo), 'public demo must state hosted evidence lock requirement');
