...
assert.ok(index.includes('v1.3.0-alpha.8 Signed Export Handoff Pack + Lock Ledger Review Surface') && index.includes('Diagnostic Repair Queue + Export Risk Resolution'), 'index visible badge must show stable candidate consolidation');
...
assert.ok(publicDemo.includes('1.3.0-alpha.8') && /hosted.*evidence|hosted.*metadata/i.test(publicDemo), 'public demo must state hosted evidence lock requirement');
