...
assert.ok(index.includes('v1.4.0-alpha.25 Release System Consolidation + Effective Diff Guard') && (index.includes('Runtime-Budget Guard Only') || fs.readFileSync('src/research/render-helpers.js', 'utf8').includes('Runtime-Budget Guard Only')), 'index visible badge must show alpha.18 evidence dashboard decision ledger identity');
...
assert.ok(arabicHostedDemoBodies.some((body)=>body.includes('توحيد نظام الإصدار وحارس الفرق الفعلي جاهز لأدلة الإصدار')), 'Arabic current-release description must identify alpha.18 evidence dashboard decision ledger');
...
assert.ok(publicDemo.includes('1.4.0-alpha.25') && /hosted.*evidence|hosted.*metadata/i.test(publicDemo), 'public demo must state hosted evidence lock requirement');
