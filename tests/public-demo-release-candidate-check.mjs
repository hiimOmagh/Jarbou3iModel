...
const RELEASE = 'v1.2.0-alpha.5 — Claim Traceability Console + Review Decision Ledger';
...
assert.ok(index.includes('v1.2.0-alpha.5 Claim Traceability Console + Review Decision Ledger') && index.includes('Claim Traceability Console + Review Decision Ledger'), 'index visible badge must show stable candidate consolidation');
...
assert.ok(publicDemo.includes('1.2.0-alpha.5') && /hosted.*evidence|hosted.*metadata/i.test(publicDemo), 'public demo must state hosted evidence lock requirement');
...
console.log('Claim Traceability Console + Review Decision Ledger checks passed.');
process.exit(0);
