...
assert.ok(arabicHostedDemoBodies.some((body)=>body.includes('توقيت الالتقاط') || body.includes('ميزانية المهلة') || body.includes('أدلة الاستضافة') || body.includes('فشل متذبذب')), 'Arabic current-release description must identify alpha.58 hosted evidence timeout budget guard');
...
assert.ok(publicDemo.includes(CURRENT_VERSION) && /hosted.*evidence|hosted.*metadata|release lock|evidence digest|lock dashboard|targeted region/i.test(publicDemo), 'public demo must state current hosted evidence lock requirement');
