...
assert.ok(index.includes('v1.4.0-alpha.29 Adapter Replay Review Pack UI Polish + Export Preview') && (index.includes('Export Preview Polish') || fs.readFileSync('src/research/render-helpers.js', 'utf8').includes('Export Preview Polish')), 'index visible badge must show alpha.29 adapter replay export-preview identity');
...
assert.ok(arabicHostedDemoBodies.some((body)=>body.includes('صقل واجهة حزمة المراجعة + معاينة التصدير جاهز لأدلة الإصدار')), 'Arabic current-release description must identify alpha.29 adapter replay export-preview');
...
assert.ok(publicDemo.includes('1.4.0-alpha.29') && /hosted.*evidence|hosted.*metadata/i.test(publicDemo), 'public demo must state hosted evidence lock requirement');
