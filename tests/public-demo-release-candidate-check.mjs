...
assert.ok(index.includes('v1.2.0-alpha.4 Evidence Review Throughput + Source-to-Brief Export Polish') && index.includes('Evidence Review Throughput + Source-to-Brief Export Polish'), 'index visible badge must show stable candidate consolidation');
...
assert.ok(publicDemo.includes('1.2.0-alpha.4') && /hosted.*evidence|hosted.*metadata/i.test(publicDemo), 'public demo must state hosted evidence lock requirement');
...
console.log('Evidence Review Throughput + Source-to-Brief Export Polish checks passed.');
process.exit(0);
