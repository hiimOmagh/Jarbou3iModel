...
assert.ok(index.includes('v1.3.0-alpha.10 Brief Publication Pack v4') && index.includes('Diagnostic Repair Queue + Export Risk Resolution'), 'index visible badge must show stable candidate consolidation');
...
assert.ok(publicDemo.includes('1.3.0-alpha.10') && /hosted.*evidence|hosted.*metadata/i.test(publicDemo), 'public demo must state hosted evidence lock requirement');
