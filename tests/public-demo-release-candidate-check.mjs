...
assert.ok(index.includes('v1.4.0-alpha.28 Adapter Replay Review Pack + Operator Handoff Export') && (index.includes('No-Network Review Pack') || fs.readFileSync('src/research/render-helpers.js', 'utf8').includes('No-Network Review Pack')), 'index visible badge must show alpha.28 adapter replay review pack identity');
...
assert.ok(arabicHostedDemoBodies.some((body)=>body.includes('حزمة مراجعة إعادة التشغيل + تصدير تسليم المشغّل جاهزة لأدلة الإصدار')), 'Arabic current-release description must identify alpha.28 adapter replay review pack');
...
assert.ok(publicDemo.includes('1.4.0-alpha.28') && /hosted.*evidence|hosted.*metadata/i.test(publicDemo), 'public demo must state hosted evidence lock requirement');
