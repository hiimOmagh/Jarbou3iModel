...
const RELEASE = 'v1.4.0-alpha.33 — Adapter Replay Review Pack Decision Queue';
...
assert.ok(index.includes('v1.4.0-alpha.33') && (index.includes('Adapter Replay Review Pack Decision Queue') || fs.readFileSync('src/research/render-helpers.js', 'utf8').includes('Adapter Replay Review Pack Decision Queue')), 'index/release helpers must expose alpha.30 release identity');
...
assert.ok(arabicHostedDemoBodies.some((body)=>body.includes('قائمة قرارات حزمة مراجعة إعادة التشغيل') || body.includes('بطاقات القرار')), 'Arabic current-release description must identify alpha.33 decision queue');
...
assert.ok(publicDemo.includes('1.4.0-alpha.33') && /hosted.*evidence|hosted.*metadata/i.test(publicDemo), 'public demo must state hosted evidence lock requirement');
