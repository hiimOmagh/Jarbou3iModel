...
const VERSION = '1.4.0-alpha.30';
const RELEASE = 'v1.4.0-alpha.30 — Release Identity Single Source Contract';
...
assert.ok(index.includes('v1.4.0-alpha.30') && (index.includes('Release Identity Single Source Contract') || fs.readFileSync('src/research/render-helpers.js', 'utf8').includes('Release Identity Single Source Contract')), 'index/release helpers must expose alpha.30 release identity');
...
assert.ok(arabicHostedDemoBodies.some((body)=>body.includes('عقد هوية الإصدار من مصدر واحد') || body.includes('هوية الإصدار')), 'Arabic current-release description must identify alpha.30 release identity contract');
...
assert.ok(publicDemo.includes('1.4.0-alpha.30') && /hosted.*evidence|hosted.*metadata/i.test(publicDemo), 'public demo must state hosted evidence lock requirement');
