...
const RELEASE = 'v1.4.0-alpha.35 — Adapter Replay Review Pack Handoff Dossier';
...
assert.ok(index.includes('v1.4.0-alpha.35') && (index.includes('Adapter Replay Review Pack Handoff Dossier') || fs.readFileSync('src/research/render-helpers.js', 'utf8').includes('Adapter Replay Review Pack Handoff Dossier')), 'index/release helpers must expose alpha.30 release identity');
...
assert.ok(arabicHostedDemoBodies.some((body)=>body.includes('ملف تسليم حزمة مراجعة إعادة التشغيل') || body.includes('بطاقات الفرز')), 'Arabic current-release description must identify alpha.33 triage workbench');
...
assert.ok(publicDemo.includes('1.4.0-alpha.35') && /hosted.*evidence|hosted.*metadata/i.test(publicDemo), 'public demo must state hosted evidence lock requirement');
