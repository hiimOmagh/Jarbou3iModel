...
const RELEASE = 'v1.4.0-alpha.31 — Adapter Replay Review Pack Operator Workflow Polish';
...
assert.ok(index.includes('v1.4.0-alpha.31') && (index.includes('Adapter Replay Review Pack Operator Workflow Polish') || fs.readFileSync('src/research/render-helpers.js', 'utf8').includes('Adapter Replay Review Pack Operator Workflow Polish')), 'index/release helpers must expose alpha.30 release identity');
...
assert.ok(arabicHostedDemoBodies.some((body)=>body.includes('صقل سير عمل المشغّل لحزمة مراجعة إعادة التشغيل') || body.includes('أولوية قرار المشغّل')), 'Arabic current-release description must identify alpha.31 operator workflow polish');
...
assert.ok(publicDemo.includes('1.4.0-alpha.31') && /hosted.*evidence|hosted.*metadata/i.test(publicDemo), 'public demo must state hosted evidence lock requirement');
