...
const RELEASE = 'v1.4.0-alpha.32 — Adapter Replay Review Pack Evidence Trace Reader';
...
assert.ok(index.includes('v1.4.0-alpha.32') && (index.includes('Adapter Replay Review Pack Evidence Trace Reader') || fs.readFileSync('src/research/render-helpers.js', 'utf8').includes('Adapter Replay Review Pack Evidence Trace Reader')), 'index/release helpers must expose alpha.30 release identity');
...
assert.ok(arabicHostedDemoBodies.some((body)=>body.includes('قارئ تتبع الأدلة لحزمة مراجعة إعادة التشغيل') || body.includes('بطاقات التتبع')), 'Arabic current-release description must identify alpha.32 evidence trace reader');
...
assert.ok(publicDemo.includes('1.4.0-alpha.32') && /hosted.*evidence|hosted.*metadata/i.test(publicDemo), 'public demo must state hosted evidence lock requirement');
