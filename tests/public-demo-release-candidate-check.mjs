...
assert.ok(index.includes('v1.4.0-alpha.26 Adapter Replay Insight UX + Operator Decision Surface') && (index.includes('Runtime-Budget Guard Only') || fs.readFileSync('src/research/render-helpers.js', 'utf8').includes('Runtime-Budget Guard Only')), 'index visible badge must show alpha.18 evidence dashboard decision ledger identity');
...
assert.ok(arabicHostedDemoBodies.some((body)=>body.includes('رؤى إعادة تشغيل المحوّل وسطح قرار المشغّل جاهزة لأدلة الإصدار')), 'Arabic current-release description must identify alpha.18 evidence dashboard decision ledger');
...
assert.ok(publicDemo.includes('1.4.0-alpha.26') && /hosted.*evidence|hosted.*metadata/i.test(publicDemo), 'public demo must state hosted evidence lock requirement');
