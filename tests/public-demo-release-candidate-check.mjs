...
assert.ok(index.includes('v1.1.0 Stable Candidate') && index.includes('Stable Consolidation + Public Surface Freeze'), 'index visible badge must show stable candidate consolidation');
...
assert.ok(publicDemo.includes('1.1.0-rc.1-copyfix.1') && /hosted.*evidence|hosted.*metadata/i.test(publicDemo), 'public demo must state hosted evidence lock requirement');
...
console.log('Stable Consolidation + Public Surface Freeze checks passed.');
process.exit(0);
