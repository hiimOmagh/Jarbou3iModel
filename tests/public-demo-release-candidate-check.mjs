...
assert.ok(index.includes('v1.4.0-alpha.27 Adapter Replay Decision Drilldown + Evidence Trace Links') && (index.includes('No-Network Replay Drilldown') || fs.readFileSync('src/research/render-helpers.js', 'utf8').includes('No-Network Replay Drilldown')), 'index visible badge must show alpha.27 adapter replay decision drilldown identity');
...
assert.ok(arabicHostedDemoBodies.some((body)=>body.includes('تفصيل قرار إعادة التشغيل وروابط تتبع الأدلة جاهز لأدلة الإصدار')), 'Arabic current-release description must identify alpha.27 adapter replay decision drilldown');
...
assert.ok(publicDemo.includes('1.4.0-alpha.27') && /hosted.*evidence|hosted.*metadata/i.test(publicDemo), 'public demo must state hosted evidence lock requirement');
