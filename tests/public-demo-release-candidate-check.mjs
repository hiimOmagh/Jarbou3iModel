...
const CURRENT_TITLE = 'Alpha.15 Lock Completion + Evidence Surface Budget Enforcement';
const VERSION = '1.3.0';
...
assert.ok(index.includes('v1.4.0-alpha.16 Evidence Surface Budget Enforcement') && (index.includes('Runtime-Budget Guard Only') || fs.readFileSync('src/research/render-helpers.js', 'utf8').includes('Runtime-Budget Guard Only')), 'index visible badge must show alpha.15 evidence surface budget enforcement identity');
...
assert.ok(arabicHostedDemoBodies.some((body)=>body.includes('ضغط تجربة محوّل المزوّد اليدوي جاهز لأدلة الإصدار')), 'Arabic current-release description must identify alpha.15 evidence surface budget enforcement');
...
assert.ok(publicDemo.includes('1.4.0-alpha.16') && /hosted.*evidence|hosted.*metadata/i.test(publicDemo), 'public demo must state hosted evidence lock requirement');
