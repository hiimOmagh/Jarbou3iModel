...
assert.ok(index.includes('v1.2.0-alpha.3 Source-to-Brief UX Compression + Operator Flow Polish') && index.includes('Source-to-Brief UX Compression + Operator Flow Polish'), 'index visible badge must show stable candidate consolidation');
...
assert.ok(publicDemo.includes('1.2.0-alpha.3') && /hosted.*evidence|hosted.*metadata/i.test(publicDemo), 'public demo must state hosted evidence lock requirement');
...
console.log('Source-to-Brief UX Compression + Operator Flow Polish checks passed.');
process.exit(0);
