...
]) assert.equal(actual, VERSION, `${label} must be v1.1.0-alpha.10`);
...
for (const packet of [sample, migrationFixture, privacyFixture]) {
  ...
  assert.ok(packet.release_apply_integrity.required_commands.some((cmd) => cmd.includes('npm run test:v110a1:no-browser')), 'apply integrity must require v1.1.0-alpha.10 gate');
...
assert.ok(migrations.includes("const TARGET_VERSION = '1.1.0-alpha.10'"), 'migration target mismatch');
assert.ok(migrations.includes("'1.0.24','1.0.25','1.0.26','1.0.27','1.0.28','1.0.29','1.0.30','1.1.0-alpha.1','1.1.0-alpha.2','1.1.0-alpha.3','1.1.0-alpha.8','1.1.0-alpha.9','1.1.0-alpha.10'"), 'migration order must preserve v1.0.26 and append v1.1.0-alpha.10');
