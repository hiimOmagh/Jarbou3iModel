...
const TITLE = 'Source-to-Brief UX Compression + Operator Flow Polish';
const RELEASE = `v${VERSION} — ${TITLE}`;
const ARTIFACT = 'jarbou3i-research-engine-v1.2.0-alpha.3-post-stable-capability-roadmap-expansion-gate.zip';
...
]) assert.equal(actual, VERSION, `${label} must be v1.1.0`);
...
for (const packet of [sample, migrationFixture, privacyFixture]) {
  ...
  assert.ok(packet.release_apply_integrity.required_commands.some((cmd) => cmd.includes('npm run test:ci:no-browser')), 'apply integrity must require v1.1.0 gate');
  ...
  assert.ok(ledger.required_commands.some((cmd) => cmd.includes('npm run test:ci:no-browser')));
...
assert.ok(migrations.includes("const TARGET_VERSION = '1.2.0-alpha.3'"), 'migration target mismatch');
assert.ok(migrations.includes("'1.0.24','1.0.25','1.0.26','1.0.27','1.0.28','1.0.29','1.0.30','1.1.0-alpha.1','1.1.0-alpha.2','1.1.0-alpha.3','1.1.0-alpha.8','1.1.0-alpha.9','1.1.0-alpha.10','1.2.0-alpha.3'"), 'migration order must preserve v1.0.26 and append v1.1.0');
...
for (const corpus of [
  ...
  assert.ok(corpus.includes('v1.1.0') || corpus.includes('1.2.0-alpha.3'), 'release corpus must mention v1.1.0');
