...
const BASE_VERSION = '1.0.26';
const TITLE = 'Release Provenance Ledger Gate';
const RELEASE = `v${VERSION} — ${TITLE}`;
const ARTIFACT = 'jarbou3i-research-engine-v1.0.27-release-provenance-ledger-gate-patch.zip';
...
]) assert.equal(actual, VERSION, `${label} must be v1.0.27`);
...
for (const packet of [sample, migrationFixture, privacyFixture]) {
  ...
  assert.ok(packet.release_apply_integrity.required_commands.some((cmd) => cmd.includes('npm run test:v127:no-browser')), 'apply integrity must require v127 gate');
  ...
  assert.ok(ledger.required_commands.some((cmd) => cmd.includes('npm run test:v127:no-browser')));
...
assert.ok(migrations.includes("const TARGET_VERSION = '1.0.27'"), 'migration target mismatch');
assert.ok(migrations.includes("'1.0.24','1.0.25','1.0.26','1.0.27'"), 'migration order must preserve v1.0.26 and append v1.0.27');
...
for (const corpus of [
  ...
  assert.ok(/Release Provenance Ledger Gate|provenance ledger|changed-files-only/i.test(corpus), 'release corpus must describe provenance gate');
}

assert.ok(pkg.scripts['test:release-provenance-ledger']?.includes('release-provenance-ledger-check.mjs'));
assert.ok(pkg.scripts['test:v127:no-browser']?.includes('v127-no-browser-suite.mjs'));
assert.ok(pkg.scripts['test:v127']?.includes('test:ci:browser'));
...
assert.ok(ciNoBrowser.includes('run_node --check tests/v127-no-browser-suite.mjs'));
assert.equal(ciNoBrowser.includes('run_node tests/v127-no-browser-suite.mjs'), false, 'CI wrapper must not recursively run v127 suite');

for (const file of ['src/research/release-provenance-ledger.js', 'tests/release-provenance-ledger-check.mjs', 'tests/v127-no-browser-suite.mjs']) {
