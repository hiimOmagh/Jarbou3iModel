...
assert.equal(templates.VERSION, '1.0.14');
...
for (const id of ['strategic_analysis_engine','geopolitical_event_analysis','policy_impact_analysis','market_technology_trend_analysis','actor_incentive_map','contradiction_audit','scenario_forecast']) {
  ...
  assert.equal(profile.template_version, '1.0.14');
...
assert.equal(fit.fit_version, '1.0.14');
...
assert.equal(fixture.analysis_brief.template_fit_report.fit_version, '1.0.14');
