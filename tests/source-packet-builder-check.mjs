...
assert.equal(builder.VERSION, '1.0.30');
...
assert.equal(schema.$defs.source_packet_builder_report.properties.builder_version.const, '1.0.30');
...
for (const packetFixture of [fixture, migrationFixture, privacyFixture]) {
  assert.equal(packetFixture.workflow_version, '1.0.30');
  assert.equal(packetFixture.source_packet_builder_report.builder_version, '1.0.30');
  ...
  assert.equal(packetFixture.release_notes.release_title, 'v1.0.30 — Mobile Header Geometry Lock / Final Public Demo Visual Freeze');
}
