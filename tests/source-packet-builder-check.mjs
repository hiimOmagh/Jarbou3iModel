...
assert.equal(builder.VERSION, '1.1.0-alpha.24');
...
assert.equal(schema.$defs.source_packet_builder_report.properties.builder_version.const, '1.1.0-alpha.24');
...
for (const packetFixture of [fixture, migrationFixture, privacyFixture]) {
  assert.equal(packetFixture.workflow_version, '1.1.0-alpha.24');
  assert.equal(packetFixture.source_packet_builder_report.builder_version, '1.1.0-alpha.24');
  ...
  assert.equal(packetFixture.release_notes.release_title, 'v1.1.0-alpha.24 — Golden Workflow Corpus + End-to-End Demo Run');
}
