...
assert.equal(schema.$defs.source_packet_builder_report.properties.builder_version.const, '1.3.0-alpha.4');
...
for (const packetFixture of [fixture, migrationFixture, privacyFixture]) {
  assert.equal(packetFixture.workflow_version, '1.3.0-alpha.4');
  assert.equal(packetFixture.source_packet_builder_report.builder_version, '1.3.0-alpha.4');
  ...
  assert.equal(packetFixture.release_notes.release_title, 'v1.3.0-alpha.4 — Brief Template UX Polish + Matrix Hygiene Cleanup');
}
