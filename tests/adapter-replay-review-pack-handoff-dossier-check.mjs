...
assert.ok([CURRENT_RUNTIME_SCOPE, CURRENT_VERSION, VERSION, '1.4.0-alpha.43'].includes(registry.runtime_optimization.version), 'runtime optimization may advance to alpha.36 while preserving alpha.35 handoff dossier');
assert.ok([CURRENT_RUNTIME_SCOPE, 'adapter_replay_review_pack_handoff_dossier', 'source_to_brief_operator_continuity_console', 'targeted_hosted_evidence_capture'].includes(registry.runtime_optimization.optimization_scope), 'runtime optimization may advance to alpha.36 operator review console while preserving alpha.35 handoff dossier');
