...
assert.ok(workflow.includes(`summary.internal_build_version !== '1.4.0-alpha.33'`), 'browser matrix guard must validate v1.4.0-alpha.28 internal build version');
assert.ok(workflow.includes(`lock-evidence-bundle_1.4.0-alpha.33_`), 'lock evidence artifact must be versioned as v1.4.0-alpha.28');
...
assert.equal(evidenceMatrix.internal_build_version, '1.4.0-alpha.33', 'evidence matrix internal version must identify v1.4.0-alpha.28');
...
assert.equal(evidenceMatrix.public_version_labels.ar, `v${CURRENT_VERSION} قائمة قرارات حزمة مراجعة إعادة التشغيل`, 'Arabic public label must identify current release');
assert.equal(evidenceMatrix.public_version_labels.fr, `v${CURRENT_VERSION} File de décision du pack de revue de rejeu`, 'French public label must identify current release');
