...
assert.ok(workflow.includes(`summary.internal_build_version !== '1.4.0-alpha.30'`), 'browser matrix guard must validate v1.4.0-alpha.28 internal build version');
assert.ok(workflow.includes(`lock-evidence-bundle_1.4.0-alpha.30_`), 'lock evidence artifact must be versioned as v1.4.0-alpha.28');
...
assert.equal(evidenceMatrix.internal_build_version, '1.4.0-alpha.30', 'evidence matrix internal version must identify v1.4.0-alpha.28');
...
assert.equal(evidenceMatrix.public_version_labels.ar, `v${CURRENT_VERSION} عقد هوية الإصدار من مصدر واحد`, 'Arabic public label must identify current release');
assert.equal(evidenceMatrix.public_version_labels.fr, `v${CURRENT_VERSION} Contrat source unique d’identité de release`, 'French public label must identify current release');
