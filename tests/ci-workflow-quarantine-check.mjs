...
assert.ok(workflow.includes(`run-name: ${RELEASE} guided session`), 'workflow run-name must expose v1.3.0-alpha.3 guided session identity');
...
assert.ok(workflow.includes(`summary.internal_build_version !== '1.3.0-alpha.3'`), 'browser matrix guard must validate v1.3.0-alpha.3 internal build version');
assert.ok(workflow.includes(`lock-evidence-bundle_1.3.0-alpha.3_`), 'lock evidence artifact must be versioned as v1.3.0-alpha.3');

assert.equal(registry.release_title, `${RELEASE} — ${TITLE}`, 'CI registry release title must identify v1.3.0-alpha.3 guided session release');
assert.equal(evidenceMatrix.internal_build_version, '1.3.0-alpha.3', 'evidence matrix internal version must identify v1.3.0-alpha.3');
assert.equal(evidenceMatrix.public_version_label, `${RELEASE} ${TITLE}`, 'English public label must identify v1.3.0-alpha.3 guided session release');
assert.equal(evidenceMatrix.public_version_labels.ar, `${RELEASE} نظام قوالب الموجز + تدقيق متغيرات التجميع`, 'Arabic public label must identify v1.3.0-alpha.3 guided session release');
assert.equal(evidenceMatrix.public_version_labels.fr, `${RELEASE} Système de modèles de brief + QA des variantes d’assemblage`, 'French public label must identify v1.3.0-alpha.3 guided session release');

for (const marker of [
  `${RELEASE} ${TITLE}`,
  `${RELEASE} نظام قوالب الموجز + تدقيق متغيرات التجميع`,
  `${RELEASE} Système de modèles de brief + QA des variantes d’assemblage`
]) {
...
for (const marker of [
  ...
  'Brief Template System + Assembly Variant QA'
]) {
