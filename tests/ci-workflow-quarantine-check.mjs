...
assert.ok(workflow.includes(`run-name: ${RELEASE} guided session`), 'workflow run-name must expose v1.3.0-alpha.6 guided session identity');
...
assert.ok(workflow.includes(`summary.internal_build_version !== '1.3.0-alpha.6'`), 'browser matrix guard must validate v1.3.0-alpha.6 internal build version');
assert.ok(workflow.includes(`lock-evidence-bundle_1.3.0-alpha.6_`), 'lock evidence artifact must be versioned as v1.3.0-alpha.6');

assert.equal(registry.release_title, `${RELEASE} — ${TITLE}`, 'CI registry release title must identify v1.3.0-alpha.6 guided session release');
assert.equal(evidenceMatrix.internal_build_version, '1.3.0-alpha.6', 'evidence matrix internal version must identify v1.3.0-alpha.6');
assert.equal(evidenceMatrix.public_version_label, `${RELEASE} ${TITLE}`, 'English public label must identify v1.3.0-alpha.6 guided session release');
assert.equal(evidenceMatrix.public_version_labels.ar, `${RELEASE} حالة اعتماد المشغّل + سجل قفل التصدير`, 'Arabic public label must identify v1.3.0-alpha.6 guided session release');
assert.equal(evidenceMatrix.public_version_labels.fr, `${RELEASE} État de visa opérateur + registre de verrouillage export`, 'French public label must identify v1.3.0-alpha.6 guided session release');

for (const marker of [
  `${RELEASE} ${TITLE}`,
  `${RELEASE} حالة اعتماد المشغّل + سجل قفل التصدير`,
  `${RELEASE} État de visa opérateur + registre de verrouillage export`
]) {
...
for (const marker of [
  ...
  'Operator Signoff State + Export Lock Ledger'
]) {
