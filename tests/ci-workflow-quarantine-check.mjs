...
assert.ok(workflow.includes(`summary.internal_build_version !== '1.4.0-alpha.42'`), 'browser matrix guard must validate v1.4.0-alpha.28 internal build version');
assert.ok(workflow.includes(`lock-evidence-bundle_1.4.0-alpha.42_`), 'lock evidence artifact must be versioned as v1.4.0-alpha.28');
...
assert.equal(evidenceMatrix.internal_build_version, '1.4.0-alpha.42', 'evidence matrix internal version must identify v1.4.0-alpha.28');
...
assert.equal(evidenceMatrix.public_version_labels.ar, `v${CURRENT_VERSION} دمج تجربة سير العمل اليدوي`, 'Arabic public label must identify current release');
assert.equal(evidenceMatrix.public_version_labels.fr, `v${CURRENT_VERSION} Consolidation UX du workflow manuel`, 'French public label must identify current release');
...
for (const marker of [
  ...
  'Manual Workflow UX Consolidation'
]) {
