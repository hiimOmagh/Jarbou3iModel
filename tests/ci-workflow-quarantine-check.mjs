...
import { CURRENT_PUBLIC_LABEL, CURRENT_RELEASE, CURRENT_RUNTIME_SCOPE, CURRENT_TITLE, CURRENT_VERSION, evidenceMatrixConfig } from './current-release-identity.mjs';
...
assert.ok(workflow.includes(`summary.internal_build_version !== '${CURRENT_VERSION}'`), 'browser matrix guard must validate v1.4.0-alpha.28 internal build version');
assert.ok(workflow.includes(`lock-evidence-bundle_${CURRENT_VERSION}_`), 'lock evidence artifact must be versioned as v1.4.0-alpha.28');
...
assert.equal(evidenceMatrix.public_version_labels.ar, evidenceMatrixConfig.public_version_labels.ar, 'Arabic public label must identify current release');
assert.equal(evidenceMatrix.public_version_labels.fr, evidenceMatrixConfig.public_version_labels.fr, 'French public label must identify current release');
