...
assert.ok(workflow.includes(`run-name: ${RELEASE} guided session`), 'workflow run-name must expose v1.3.0-alpha.4 guided session identity');
...
assert.ok(workflow.includes(`summary.internal_build_version !== '1.3.0-alpha.4'`), 'browser matrix guard must validate v1.3.0-alpha.4 internal build version');
assert.ok(workflow.includes(`lock-evidence-bundle_1.3.0-alpha.4_`), 'lock evidence artifact must be versioned as v1.3.0-alpha.4');

assert.equal(registry.release_title, `${RELEASE} — ${TITLE}`, 'CI registry release title must identify v1.3.0-alpha.4 guided session release');
assert.equal(evidenceMatrix.internal_build_version, '1.3.0-alpha.4', 'evidence matrix internal version must identify v1.3.0-alpha.4');
assert.equal(evidenceMatrix.public_version_label, `${RELEASE} ${TITLE}`, 'English public label must identify v1.3.0-alpha.4 guided session release');
assert.equal(evidenceMatrix.public_version_labels.ar, `${RELEASE} تحسين تجربة قوالب الموجز + تنظيف مصفوفة الأدلة`, 'Arabic public label must identify v1.3.0-alpha.4 guided session release');
assert.equal(evidenceMatrix.public_version_labels.fr, `${RELEASE} Polish UX des modèles de brief + nettoyage de la matrice`, 'French public label must identify v1.3.0-alpha.4 guided session release');

for (const marker of [
  `${RELEASE} ${TITLE}`,
  `${RELEASE} تحسين تجربة قوالب الموجز + تنظيف مصفوفة الأدلة`,
  `${RELEASE} Polish UX des modèles de brief + nettoyage de la matrice`
]) {
...
for (const marker of [
  ...
  'Brief Template UX Polish + Matrix Hygiene Cleanup'
]) {
