...
assert.ok(workflow.includes(`run-name: ${RELEASE} guided session`), 'workflow run-name must expose v1.3.0-alpha.1 guided session identity');
...
assert.ok(workflow.includes(`summary.internal_build_version !== '1.3.0-alpha.1'`), 'browser matrix guard must validate v1.3.0-alpha.1 internal build version');
assert.ok(workflow.includes(`lock-evidence-bundle_1.3.0-alpha.1_`), 'lock evidence artifact must be versioned as v1.3.0-alpha.1');

assert.equal(registry.release_title, `${RELEASE} — ${TITLE}`, 'CI registry release title must identify v1.3.0-alpha.1 guided session release');
assert.equal(evidenceMatrix.internal_build_version, '1.3.0-alpha.1', 'evidence matrix internal version must identify v1.3.0-alpha.1');
assert.equal(evidenceMatrix.public_version_label, `${RELEASE} ${TITLE}`, 'English public label must identify v1.3.0-alpha.1 guided session release');
assert.equal(evidenceMatrix.public_version_labels.ar, `${RELEASE} محرك جلسة البحث الموجّه + سير تجميع الموجز`, 'Arabic public label must identify v1.3.0-alpha.1 guided session release');
assert.equal(evidenceMatrix.public_version_labels.fr, `${RELEASE} Moteur de session de recherche guidée + workflow d’assemblage du brief`, 'French public label must identify v1.3.0-alpha.1 guided session release');

for (const marker of [
  `${RELEASE} ${TITLE}`,
  `${RELEASE} محرك جلسة البحث الموجّه + سير تجميع الموجز`,
  `${RELEASE} Moteur de session de recherche guidée + workflow d’assemblage du brief`
]) {
...
for (const marker of [
  ...
  'Guided Research Session Engine + Brief Assembly Workflow'
]) {
