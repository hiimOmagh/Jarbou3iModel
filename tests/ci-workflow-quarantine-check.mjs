...
assert.ok(workflow.includes(`run-name: ${RELEASE} source-to-claim gap closure`), 'workflow run-name must expose v1.3.0-alpha.10 source-to-claim gap closure identity');
...
assert.ok(workflow.includes(`summary.internal_build_version !== '1.3.0-alpha.10'`), 'browser matrix guard must validate v1.3.0-alpha.10 internal build version');
assert.ok(workflow.includes(`lock-evidence-bundle_1.3.0-alpha.10_`), 'lock evidence artifact must be versioned as v1.3.0-alpha.10');

assert.equal(registry.release_title, `${RELEASE} — ${TITLE}`, 'CI registry release title must identify v1.3.0-alpha.10 source-to-claim gap closure release');
assert.equal(evidenceMatrix.internal_build_version, '1.3.0-alpha.10', 'evidence matrix internal version must identify v1.3.0-alpha.10');
assert.equal(evidenceMatrix.public_version_label, `${RELEASE} ${TITLE}`, 'English public label must identify v1.3.0-alpha.10 source-to-claim gap closure release');
assert.equal(evidenceMatrix.public_version_labels.ar, `${RELEASE} قائمة إغلاق فجوات المصدر إلى الادعاء`, 'Arabic public label must identify v1.3.0-alpha.10 source-to-claim gap closure release');
assert.equal(evidenceMatrix.public_version_labels.fr, `${RELEASE} File de clôture des écarts source-affirmation`, 'French public label must identify v1.3.0-alpha.10 source-to-claim gap closure release');
