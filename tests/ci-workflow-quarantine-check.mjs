...
const RELEASE = 'v1.4.0-alpha.22';
const TITLE = 'Alpha.21 Lock Completion + Handoff Productivity Command Center';
const PUBLIC_TITLE = 'Handoff Productivity Command Center';
...
assert.ok(workflow.includes(`run-name: ${RELEASE} ${TITLE} — \${{ github.ref_name }} @ \${{ github.sha }}`), 'workflow run-name must expose v1.4.0-alpha.22 provider execution equivalence identity');
...
assert.ok(workflow.includes(`summary.internal_build_version !== '1.4.0-alpha.22'`), 'browser matrix guard must validate v1.4.0-alpha.22 internal build version');
assert.ok(workflow.includes(`lock-evidence-bundle_1.4.0-alpha.22_`), 'lock evidence artifact must be versioned as v1.4.0-alpha.22');

assert.equal(registry.release_title, `${RELEASE} — ${TITLE}`, 'CI registry release title must identify v1.4.0-alpha.22 controlled provider/source execution preparation release');
assert.equal(evidenceMatrix.internal_build_version, '1.4.0-alpha.22', 'evidence matrix internal version must identify v1.4.0-alpha.22');
assert.equal(evidenceMatrix.public_version_label, `${RELEASE} ${PUBLIC_TITLE}`, 'English public label must identify v1.4.0-alpha.22 controlled provider/source execution preparation release');
assert.equal(evidenceMatrix.public_version_labels.ar, `${RELEASE} مركز إنتاجية التسليم`, 'Arabic public label must identify v1.4.0-alpha.22 controlled provider/source execution preparation release');
assert.equal(evidenceMatrix.public_version_labels.fr, `${RELEASE} Centre de productivité de remise`, 'French public label must identify v1.4.0-alpha.22 controlled provider/source execution preparation release');

for (const marker of [
  `${RELEASE} ${PUBLIC_TITLE}`,
  `${RELEASE} مركز إنتاجية التسليم`,
  `${RELEASE} Centre de productivité de remise`
]) {
