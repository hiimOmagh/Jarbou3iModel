...
assert.ok(index.includes('Adapter Replay Review Pack UI Polish + Export Preview') || index.includes('Source-to-Brief Publication Readiness Suite'), 'index must expose alpha.29 preview or current release identity title');
...
assert.ok([VERSION, '1.4.0-alpha.40', '1.4.0-alpha.40'].includes(registry.runtime_optimization.version), 'runtime optimization may advance to alpha.30 while preserving alpha.29 export-preview check');
