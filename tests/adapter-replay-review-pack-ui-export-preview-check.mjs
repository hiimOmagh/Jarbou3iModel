...
assert.ok(index.includes('Adapter Replay Review Pack UI Polish + Export Preview') || index.includes('Source-to-Brief Operator Continuity Console'), 'index must expose alpha.29 preview or current release identity title');
...
assert.ok([VERSION, '1.4.0-alpha.38', '1.4.0-alpha.38'].includes(registry.runtime_optimization.version), 'runtime optimization may advance to alpha.30 while preserving alpha.29 export-preview check');
assert.ok(['adapter_replay_review_pack_ui_export_preview', 'source_to_brief_operator_continuity_console', 'source_to_brief_operator_continuity_console'].includes(registry.runtime_optimization.optimization_scope), 'runtime optimization may advance to alpha.30 while preserving alpha.29 export-preview check');
