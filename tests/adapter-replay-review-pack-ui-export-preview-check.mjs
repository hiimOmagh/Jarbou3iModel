...
assert.ok(preview.preview_cards.some((card) => card.preview_id === 'json-preview' && card.body.includes('evidence_trace_bundle')), 'JSON preview must include evidence trace bundle');
...
assert.ok(index.includes('Adapter Replay Review Pack UI Polish + Export Preview') || index.includes('Adapter Replay Review Pack Operator Workflow Polish'), 'index must expose alpha.29 preview or current release identity title');
...
assert.ok([VERSION, '1.4.0-alpha.31'].includes(registry.runtime_optimization.version), 'runtime optimization may advance to alpha.30 while preserving alpha.29 export-preview check');
assert.ok(['adapter_replay_review_pack_ui_export_preview', 'adapter_replay_review_pack_operator_workflow_polish'].includes(registry.runtime_optimization.optimization_scope), 'runtime optimization may advance to alpha.30 while preserving alpha.29 export-preview check');
