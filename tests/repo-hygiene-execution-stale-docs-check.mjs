...
assert.equal(lock.version, CURRENT_VERSION, 'package-lock root version must identify v1.4.0-alpha.3');
assert.equal(lock.packages[''].version, CURRENT_VERSION, 'package-lock package root must identify v1.4.0-alpha.3');
...
assert.ok(aiIntegration.includes('v1.4.0-alpha.3') || aiIntegration.includes('v1.3.0') || aiIntegration.includes('## Current state: v1.1.0'), 'AI integration doc must name current alpha.1 or stable state');
...
assert.ok(architecture.includes('v1.4.0-alpha.3') || architecture.includes('v1.3.0') || architecture.includes('## Current v1.1.0 pipeline'), 'architecture doc must name current alpha.1 or stable pipeline');
...
assert.ok(privacyAudit.includes('v1.4.0-alpha.3') || privacyAudit.includes('v1.3.0') || privacyAudit.includes('`v1.1.0` treats every exported JSON payload as a security boundary'), 'privacy audit doc must name current alpha.1 or stable boundary');
