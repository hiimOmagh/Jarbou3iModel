...
const migrationFixture = getMigrationFixture('fixtures/migrations/v1.1.0-rc.1-packet.json');
const privacyFixture = getPrivacyFixture('fixtures/privacy/browser-generated-export-v1.1.0-rc.1.json');
const releaseDoc = `${readReleaseDoc('docs/v1.0.25-public-demo-release-lock.md')}\n${readReleaseDoc('docs/v1.1.0-rc.1-evidence-pack-export-v3-brief-traceability.md')}`;
...
assert.equal(pkg.version, VERSION, 'package.json must identify v1.1.0-rc.1');
assert.equal(lock.version, VERSION, 'package-lock root version must identify v1.1.0-rc.1');
assert.equal(lock.packages[''].version, VERSION, 'package-lock package root must identify v1.1.0-rc.1');
assert.equal(schema.properties.workflow_version.const, VERSION, 'schema workflow version must identify v1.1.0-rc.1');
assert.equal(sample.workflow_version, VERSION, 'sample fixture must identify v1.1.0-rc.1');
assert.equal(migrationFixture.workflow_version, VERSION, 'migration fixture must identify v1.1.0-rc.1');
assert.equal(privacyFixture.workflow_version, VERSION, 'privacy fixture must identify v1.1.0-rc.1');
assert.equal(sample.release_notes.release_title, RELEASE, 'sample release title must identify v1.1.0-rc.1');
assert.equal(migrationFixture.release_notes.release_title, RELEASE, 'migration fixture release title must identify v1.1.0-rc.1');
assert.equal(privacyFixture.release_notes.release_title, RELEASE, 'privacy fixture release title must identify v1.1.0-rc.1');

for (const corpus of [releaseDoc, manifest, notes, changelog, roadmap, qaMatrix]) {
  assert.ok(corpus.includes('v1.1.0-rc.1'), 'release corpus must mention v1.1.0-rc.1');
...
for (const required of [
  ...
  'fixtures/migrations/v1.1.0-rc.1-packet.json',
  'fixtures/privacy/browser-generated-export-v1.1.0-rc.1.json',
...
assert.ok(aiIntegration.includes('## Current state: v1.1.0-rc.1'), 'AI integration doc must name current v1.1.0-rc.1 state');
...
assert.ok(architecture.includes('## Current v1.1.0-rc.1 pipeline'), 'architecture doc must name current v1.1.0-rc.1 pipeline');
...
assert.ok(privacyAudit.includes('`v1.1.0-rc.1` treats every exported JSON payload as a security boundary'), 'privacy audit doc must name v1.1.0-rc.1 boundary');
...
assert.ok(migrations.includes("const TARGET_VERSION = '1.1.0-rc.1'"), 'migration target must be v1.1.0-rc.1');
assert.ok(migrations.includes("'1.0.21','1.0.22','1.0.23','1.0.24','1.0.25','1.0.26','1.0.27','1.0.28','1.0.29','1.0.30','1.1.0-alpha.1','1.1.0-alpha.2','1.1.0-alpha.3','1.1.0-alpha.8','1.1.0-alpha.9','1.1.0-alpha.10','1.1.0-rc.1'"), 'migration order must preserve v1.0.24 and append v1.1.0-rc.1');
assert.ok(migrations.includes("release_title:'v1.1.0-rc.1 — Stable Consolidation + Public Surface Freeze'"), 'migration default release title must identify v1.1.0-rc.1');
