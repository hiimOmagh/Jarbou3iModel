...
assert.equal(schema.properties.workflow_version.const, VERSION, 'schema workflow version must identify v1.0.26');
assert.equal(sample.workflow_version, VERSION, 'sample fixture must identify v1.0.26');
...
assert.equal(sample.release_notes.release_title, RELEASE, 'sample release title must identify v1.0.26');
assert.equal(migrationFixture.release_notes.release_title, RELEASE, 'migration fixture release title must identify v1.0.26');
assert.equal(privacyFixture.release_notes.release_title, RELEASE, 'privacy fixture release title must identify v1.0.26');

for (const corpus of [releaseDoc, manifest, notes, changelog, roadmap, qaMatrix]) {
  assert.ok(corpus.includes('v1.0.26'), 'release corpus must mention v1.0.26');
...
assert.ok(aiIntegration.includes('## Current state: v1.0.26'), 'AI integration doc must name current v1.0.26 state');
...
assert.ok(architecture.includes('## Current v1.0.26 pipeline'), 'architecture doc must name current v1.0.26 pipeline');
...
assert.ok(privacyAudit.includes('`v1.0.26` treats every exported JSON payload as a security boundary'), 'privacy audit doc must name v1.0.26 boundary');
...
assert.ok(migrations.includes("const TARGET_VERSION = '1.0.26'"), 'migration target must be v1.0.26');
assert.ok(migrations.includes("'1.0.21','1.0.22','1.0.23','1.0.24','1.0.25','1.0.26'"), 'migration order must preserve v1.0.24 and append v1.0.26');
assert.ok(migrations.includes("release_title:'v1.0.26 — Release Apply Integrity Gate'"), 'migration default release title must identify v1.0.26');

assert.ok(ciNoBrowser.includes('tests/public-demo-release-lock-check.mjs'), 'no-browser CI must run v1.0.26 repo hygiene guard');
...
assert.ok(pkg.scripts['test:stable']?.includes('public-demo-release-lock-check.mjs'), 'stable suite must include v1.0.26 hygiene check');
assert.ok(pkg.scripts['test:patch']?.includes('public-demo-release-lock-check.mjs'), 'patch suite must include v1.0.26 hygiene check');
