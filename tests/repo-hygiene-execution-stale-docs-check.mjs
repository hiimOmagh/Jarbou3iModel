...
for (const corpus of [releaseDoc, manifest, notes, changelog, roadmap, qaMatrix]) {
  assert.ok(corpus.includes('v1.1.0-alpha.4'), 'release corpus must mention v1.1.0-alpha.4');
...
assert.ok(aiIntegration.includes('## Current state: v1.1.0-alpha.4'), 'AI integration doc must name current v1.1.0-alpha.4 state');
...
assert.ok(architecture.includes('## Current v1.1.0-alpha.4 pipeline'), 'architecture doc must name current v1.1.0-alpha.4 pipeline');
...
assert.ok(privacyAudit.includes('`v1.1.0-alpha.4` treats every exported JSON payload as a security boundary'), 'privacy audit doc must name v1.1.0-alpha.4 boundary');
...
assert.ok(migrations.includes("release_title:'v1.1.0-alpha.4 — Migration + Privacy Fixture Registry Consolidation'"), 'migration default release title must identify v1.1.0-alpha.4');

assert.ok(ciNoBrowser.includes('tests/public-demo-release-lock-check.mjs'), 'no-browser CI must run v1.1.0-alpha.4 repo hygiene guard');
...
assert.ok(pkg.scripts['test:stable']?.includes('public-demo-release-lock-check.mjs'), 'stable suite must include v1.1.0-alpha.4 hygiene check');
assert.ok(pkg.scripts['test:patch']?.includes('public-demo-release-lock-check.mjs'), 'patch suite must include v1.1.0-alpha.4 hygiene check');
