...
const releaseDoc = read('docs/v1.0.24-repo-hygiene-execution-stale-documentation-correction.md');
...
for (const corpus of [releaseDoc, manifest, notes, changelog, roadmap, qaMatrix]) {
  assert.ok(corpus.includes('v1.0.24'), 'release corpus must mention v1.0.24');
...
assert.ok(aiIntegration.includes('## Current state: v1.0.24'), 'AI integration doc must name current v1.0.24 state');
...
assert.ok(architecture.includes('## Current v1.0.24 pipeline'), 'architecture doc must name current v1.0.24 pipeline');
...
assert.ok(privacyAudit.includes('`v1.0.24` treats every exported JSON payload as a security boundary'), 'privacy audit doc must name v1.0.24 boundary');
...
assert.ok(migrations.includes("release_title:'v1.0.24 — Repo Hygiene Execution + Stale Documentation Correction'"), 'migration default release title must identify v1.0.24');

assert.ok(ciNoBrowser.includes('tests/repo-hygiene-execution-stale-docs-check.mjs'), 'no-browser CI must run v1.0.24 repo hygiene guard');
...
assert.ok(pkg.scripts['test:stable']?.includes('repo-hygiene-execution-stale-docs-check.mjs'), 'stable suite must include v1.0.24 hygiene check');
assert.ok(pkg.scripts['test:patch']?.includes('repo-hygiene-execution-stale-docs-check.mjs'), 'patch suite must include v1.0.24 hygiene check');
...
for (const file of ['tests/repo-hygiene-execution-stale-docs-check.mjs', 'tests/v124-no-browser-suite.mjs']) {
  const syntax = spawnSync(process.execPath, ['--check', file], { encoding: 'utf8' });
  assert.equal(syntax.status, 0, syntax.stderr || syntax.stdout || `${file} syntax check failed`);
}

console.log('Repo hygiene execution and stale documentation correction checks passed.');
process.exit(0);
