...
assert.equal(schema.properties.workflow_version.const, VERSION, 'schema workflow version must identify v1.1.0-alpha.1');
assert.equal(sample.workflow_version, VERSION, 'sample fixture must identify v1.1.0-alpha.1');
...
assert.equal(sample.release_notes.release_title, RELEASE, 'sample release title must identify v1.1.0-alpha.1');
assert.equal(migrationFixture.release_notes.release_title, RELEASE, 'migration fixture release title must identify v1.1.0-alpha.1');
assert.equal(privacyFixture.release_notes.release_title, RELEASE, 'privacy fixture release title must identify v1.1.0-alpha.1');

for (const corpus of [releaseDoc, manifest, notes, changelog, roadmap, qaMatrix]) {
  assert.ok(corpus.includes('v1.1.0-alpha.1'), 'release corpus must mention v1.1.0-alpha.1');
...
assert.ok(aiIntegration.includes('## Current state: v1.1.0-alpha.1'), 'AI integration doc must name current v1.1.0-alpha.1 state');
...
assert.ok(architecture.includes('## Current v1.1.0-alpha.1 pipeline'), 'architecture doc must name current v1.1.0-alpha.1 pipeline');
...
assert.ok(privacyAudit.includes('`v1.1.0-alpha.1` treats every exported JSON payload as a security boundary'), 'privacy audit doc must name v1.1.0-alpha.1 boundary');
...
assert.ok(migrations.includes("release_title:'v1.1.0-alpha.1 — Post-Freeze Product Expansion Planning Gate'"), 'migration default release title must identify v1.1.0-alpha.1');

assert.ok(ciNoBrowser.includes('tests/public-demo-release-lock-check.mjs'), 'no-browser CI must run v1.1.0-alpha.1 repo hygiene guard');
...
assert.ok(pkg.scripts['test:stable']?.includes('public-demo-release-lock-check.mjs'), 'stable suite must include v1.1.0-alpha.1 hygiene check');
assert.ok(pkg.scripts['test:patch']?.includes('public-demo-release-lock-check.mjs'), 'patch suite must include v1.1.0-alpha.1 hygiene check');
