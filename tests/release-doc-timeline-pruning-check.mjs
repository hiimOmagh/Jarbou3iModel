...
assert.ok(history.includes(`v${VERSION} — ${TITLE}`), 'release history must include current alpha.11 title');
...
assert.ok(Buffer.byteLength(history, 'utf8') <= 130 * 1024, 'release history must remain under the alpha.11 timeline budget');

for (const corpus of [currentRelease, releaseEvidence, roadmap, qaMatrix]) {
  assert.ok(corpus.includes(`v${VERSION}`), 'current docs must identify alpha.11');
  assert.ok(corpus.includes(TITLE), 'current docs must identify alpha.11 title');
}
