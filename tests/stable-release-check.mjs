...
assert.equal(release.RELEASE_VERSION, '1.0.10');
...
assert.ok(index.includes('v1.0.10 · Hosted URL CI Artifact Review + Module-Type Warning Fix'), 'stable badge missing');
...
assert.ok(migrationSource.includes('1.0.10'), 'v1.0.10 target must be supported');

const policy = release.releaseCandidatePolicy({ version: '1.0.10' });
...
}, { version: '1.0.10', externalCiPassed: true, externalCiNotes: 'external CI succeeded' });
assert.equal(report.release_candidate_version, '1.0.10');
assert.equal(report.stable_release_version, '1.0.10');
...
assert.equal(fixture.release_candidate.release_candidate_version, '1.0.10');
assert.equal(fixture.release_candidate.stable_release_version, '1.0.10');
...
assert.ok(fs.readFileSync('src/research/render-helpers.js','utf8').includes('v1.0.10 · Hosted URL CI Artifact Review + Module-Type Warning Fix'), 'render helper stable badge must not regress to RC copy');
