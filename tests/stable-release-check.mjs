...
assert.ok(index.includes('v1.3.0 · Stable Manual Workflow Release'), 'stable badge missing');
...
assert.ok(migrationSource.includes('1.3.0'), 'v1.1.0 target must be supported');

const policy = release.releaseCandidatePolicy({ version: '1.3.0' });
...
}, { version: '1.3.0', externalCiPassed: true, externalCiNotes: 'external CI succeeded' });
assert.equal(report.release_candidate_version, '1.3.0');
assert.equal(report.stable_release_version, '1.3.0');
...
assert.equal(fixture.release_candidate.release_candidate_version, '1.3.0');
...
assert.ok(fs.readFileSync('src/research/render-helpers.js','utf8').includes('v1.3.0 · Stable Manual Workflow Release'), 'render helper stable badge must not regress to RC copy');
