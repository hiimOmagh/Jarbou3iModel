...
assert.equal(release.RELEASE_VERSION, '1.1.0-alpha.17');
...
assert.ok(index.includes('v1.1.0-alpha.17 · Research Planner V2'), 'stable badge missing');
...
assert.ok(migrationSource.includes('1.1.0-alpha.17'), 'v1.1.0-alpha.17 target must be supported');

const policy = release.releaseCandidatePolicy({ version: '1.1.0-alpha.17' });
...
const report = release.buildReleaseCandidateReport({
  workflow_version: '1.1.0-alpha.17',
...
}, { version: '1.1.0-alpha.17', externalCiPassed: true, externalCiNotes: 'external CI succeeded' });
assert.equal(report.release_candidate_version, '1.1.0-alpha.17');
assert.equal(report.stable_release_version, '1.1.0-alpha.17');
...
assert.equal(fixture.release_candidate.release_candidate_version, '1.1.0-alpha.17');
...
assert.ok(fs.readFileSync('src/research/render-helpers.js','utf8').includes('v1.1.0-alpha.17 · Research Planner V2'), 'render helper stable badge must not regress to RC copy');
