...
assert.equal(release.RELEASE_VERSION, '1.0.13');
...
assert.ok(index.includes('v1.0.13 · Manual Source Packet Import'), 'stable badge missing');
...
assert.ok(migrationSource.includes('1.0.13'), 'v1.0.13 target must be supported');

const policy = release.releaseCandidatePolicy({ version: '1.0.13' });
...
}, { version: '1.0.13', externalCiPassed: true, externalCiNotes: 'external CI succeeded' });
assert.equal(report.release_candidate_version, '1.0.13');
assert.equal(report.stable_release_version, '1.0.13');
...
assert.equal(fixture.workflow_version, '1.0.13');
assert.equal(fixture.release_candidate.release_candidate_version, '1.0.13');
assert.equal(fixture.release_candidate.stable_release_version, '1.0.13');
...
assert.ok(fs.readFileSync('src/research/render-helpers.js','utf8').includes('v1.0.13 · Manual Source Packet Import'), 'render helper stable badge must not regress to RC copy');
