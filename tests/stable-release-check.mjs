...
assert.equal(release.RELEASE_VERSION, '1.0.8');
...
assert.ok(index.includes('v1.0.8 · Hosted Demo Deployment Verification + Browser Evidence Capture'), 'stable badge missing');
...
assert.ok(migrationSource.includes('1.0.8'), 'v1.0.8 target must be supported');

const policy = release.releaseCandidatePolicy({ version: '1.0.8' });
...
}, { version: '1.0.8', externalCiPassed: true, externalCiNotes: 'external CI succeeded' });
assert.equal(report.release_candidate_version, '1.0.8');
assert.equal(report.stable_release_version, '1.0.8');
...
assert.equal(fixture.release_candidate.release_candidate_version, '1.0.8');
...
assert.ok(fs.readFileSync('src/research/render-helpers.js','utf8').includes('v1.0.8 · Hosted Demo Deployment Verification + Browser Evidence Capture'), 'render helper stable badge must not regress to RC copy');
