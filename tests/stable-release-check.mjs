...
assert.equal(release.RELEASE_VERSION, '1.3.0-alpha.10');
...
assert.ok(index.includes('v1.3.0-alpha.10 · Brief Publication Pack v4'), 'stable badge missing');
...
assert.ok(migrationSource.includes('1.3.0-alpha.10'), 'v1.1.0 target must be supported');

const policy = release.releaseCandidatePolicy({ version: '1.3.0-alpha.10' });
...
const report = release.buildReleaseCandidateReport({
  workflow_version: '1.3.0-alpha.10',
...
}, { version: '1.3.0-alpha.10', externalCiPassed: true, externalCiNotes: 'external CI succeeded' });
assert.equal(report.release_candidate_version, '1.3.0-alpha.10');
assert.equal(report.stable_release_version, '1.3.0-alpha.10');
...
assert.equal(fixture.release_candidate.release_candidate_version, '1.3.0-alpha.10');
assert.equal(fixture.release_candidate.stable_release_version, '1.3.0-alpha.10');
...
assert.ok(fs.readFileSync('src/research/render-helpers.js','utf8').includes('v1.3.0-alpha.10 · Brief Publication Pack v4'), 'render helper stable badge must not regress to RC copy');
