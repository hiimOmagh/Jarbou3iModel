...
assert.equal(release.RELEASE_VERSION, '1.1.0-alpha.1');
...
assert.ok(index.includes('v1.1.0-alpha.1 · Post-Freeze Product Expansion Planning Gate'), 'stable badge missing');
...
assert.ok(migrationSource.includes('1.1.0-alpha.1'), 'v1.1.0-alpha.1 target must be supported');

const policy = release.releaseCandidatePolicy({ version: '1.1.0-alpha.1' });
...
const report = release.buildReleaseCandidateReport({
  workflow_version: '1.1.0-alpha.1',
...
}, { version: '1.1.0-alpha.1', externalCiPassed: true, externalCiNotes: 'external CI succeeded' });
assert.equal(report.release_candidate_version, '1.1.0-alpha.1');
assert.equal(report.stable_release_version, '1.1.0-alpha.1');
...
assert.equal(fixture.workflow_version, '1.1.0-alpha.1');
assert.equal(fixture.release_candidate.release_candidate_version, '1.1.0-alpha.1');
assert.equal(fixture.release_candidate.stable_release_version, '1.1.0-alpha.1');
...
assert.ok(fs.readFileSync('src/research/render-helpers.js','utf8').includes('v1.1.0-alpha.1 · Post-Freeze Product Expansion Planning Gate'), 'render helper stable badge must not regress to RC copy');
