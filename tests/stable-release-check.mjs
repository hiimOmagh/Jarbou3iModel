...
assert.equal(release.RELEASE_VERSION, '1.1.0-alpha.25');
...
assert.ok(index.includes('v1.1.0-alpha.25 · Release Candidate Hardening + Final Repo Hygiene'), 'stable badge missing');
...
assert.ok(migrationSource.includes('1.1.0-alpha.25'), 'v1.1.0-alpha.25 target must be supported');

const policy = release.releaseCandidatePolicy({ version: '1.1.0-alpha.25' });
...
const report = release.buildReleaseCandidateReport({
  workflow_version: '1.1.0-alpha.25',
...
}, { version: '1.1.0-alpha.25', externalCiPassed: true, externalCiNotes: 'external CI succeeded' });
assert.equal(report.release_candidate_version, '1.1.0-alpha.25');
assert.equal(report.stable_release_version, '1.1.0-alpha.25');
...
assert.equal(fixture.release_candidate.release_candidate_version, '1.1.0-alpha.25');
...
assert.ok(fs.readFileSync('src/research/render-helpers.js','utf8').includes('v1.1.0-alpha.25 · Release Candidate Hardening + Final Repo Hygiene'), 'render helper stable badge must not regress to RC copy');
