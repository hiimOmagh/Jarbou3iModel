...
assert.equal(release.RELEASE_VERSION, '1.0.20');
...
assert.ok(index.includes('v1.0.20 · Source Packet Templates'), 'stable badge missing');
...
assert.ok(migrationSource.includes('1.0.20'), 'v1.0.20 target must be supported');

const policy = release.releaseCandidatePolicy({ version: '1.0.20' });
...
}, { version: '1.0.20', externalCiPassed: true, externalCiNotes: 'external CI succeeded' });
assert.equal(report.release_candidate_version, '1.0.20');
assert.equal(report.stable_release_version, '1.0.20');
...
assert.equal(fixture.release_candidate.release_candidate_version, '1.0.20');
assert.equal(fixture.release_candidate.stable_release_version, '1.0.20');
...
assert.ok(fs.readFileSync('src/research/render-helpers.js','utf8').includes('v1.0.20 · Source Packet Templates'), 'render helper stable badge must not regress to RC copy');
