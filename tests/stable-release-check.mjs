...
assert.equal(release.RELEASE_VERSION, '1.2.0-alpha.5');
...
assert.ok(index.includes('v1.2.0-alpha.5 · Claim Traceability Console + Review Decision Ledger'), 'stable badge missing');
...
assert.ok(migrationSource.includes('1.2.0-alpha.5'), 'v1.1.0 target must be supported');

const policy = release.releaseCandidatePolicy({ version: '1.2.0-alpha.5' });
...
const report = release.buildReleaseCandidateReport({
  workflow_version: '1.2.0-alpha.5',
...
}, { version: '1.2.0-alpha.5', externalCiPassed: true, externalCiNotes: 'external CI succeeded' });
assert.equal(report.release_candidate_version, '1.2.0-alpha.5');
assert.equal(report.stable_release_version, '1.2.0-alpha.5');
...
assert.equal(fixture.release_candidate.release_candidate_version, '1.2.0-alpha.5');
assert.equal(fixture.release_candidate.stable_release_version, '1.2.0-alpha.5');
...
assert.ok(fs.readFileSync('src/research/render-helpers.js','utf8').includes('v1.2.0-alpha.5 · Claim Traceability Console + Review Decision Ledger'), 'render helper stable badge must not regress to RC copy');
