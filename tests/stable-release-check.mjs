...
assert.equal(release.RELEASE_VERSION, '1.1.0-alpha.22');
...
assert.ok(index.includes('v1.1.0-alpha.22 · Evidence Pack Export v3 + Brief Traceability'), 'stable badge missing');
...
assert.ok(migrationSource.includes('1.1.0-alpha.22'), 'v1.1.0-alpha.22 target must be supported');

const policy = release.releaseCandidatePolicy({ version: '1.1.0-alpha.22' });
...
const report = release.buildReleaseCandidateReport({
  workflow_version: '1.1.0-alpha.22',
...
}, { version: '1.1.0-alpha.22', externalCiPassed: true, externalCiNotes: 'external CI succeeded' });
assert.equal(report.release_candidate_version, '1.1.0-alpha.22');
assert.equal(report.stable_release_version, '1.1.0-alpha.22');
...
assert.equal(fixture.release_candidate.release_candidate_version, '1.1.0-alpha.22');
...
assert.ok(fs.readFileSync('src/research/render-helpers.js','utf8').includes('v1.1.0-alpha.22 · Evidence Pack Export v3 + Brief Traceability'), 'render helper stable badge must not regress to RC copy');
