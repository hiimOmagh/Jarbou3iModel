...
assert.equal(release.RELEASE_VERSION, '1.1.0-rc.2-fix.3');
...
assert.ok(index.includes('v1.1.0-rc.2-fix.3 · Evidence Workflow Exit-Code + Matrix Activation Fix'), 'stable badge missing');
...
assert.ok(migrationSource.includes('1.1.0-rc.2-fix.3'), 'v1.1.0-rc.2-fix.3 target must be supported');

const policy = release.releaseCandidatePolicy({ version: '1.1.0-rc.2-fix.3' });
...
const report = release.buildReleaseCandidateReport({
  workflow_version: '1.1.0-rc.2-fix.3',
...
}, { version: '1.1.0-rc.2-fix.3', externalCiPassed: true, externalCiNotes: 'external CI succeeded' });
assert.equal(report.release_candidate_version, '1.1.0-rc.2-fix.3');
assert.equal(report.stable_release_version, '1.1.0-rc.2-fix.3');
...
assert.equal(fixture.workflow_version, '1.1.0-rc.2-fix.3');
assert.equal(fixture.release_candidate.release_candidate_version, '1.1.0-rc.2-fix.3');
assert.equal(fixture.release_candidate.stable_release_version, '1.1.0-rc.2-fix.3');
...
assert.ok(fs.readFileSync('src/research/render-helpers.js','utf8').includes('v1.1.0-rc.2-fix.3 · Evidence Workflow Exit-Code + Matrix Activation Fix'), 'render helper stable badge must not regress to RC copy');
