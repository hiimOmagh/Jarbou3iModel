...
assert.equal(release.RELEASE_VERSION, '1.1.0-alpha.4');
...
assert.ok(index.includes('v1.1.0-alpha.4 · Migration + Privacy Fixture Registry Consolidation'), 'stable badge missing');
...
assert.ok(migrationSource.includes('1.1.0-alpha.4'), 'v1.1.0-alpha.4 target must be supported');

const policy = release.releaseCandidatePolicy({ version: '1.1.0-alpha.4' });
...
const report = release.buildReleaseCandidateReport({
  workflow_version: '1.1.0-alpha.4',
...
}, { version: '1.1.0-alpha.4', externalCiPassed: true, externalCiNotes: 'external CI succeeded' });
assert.equal(report.release_candidate_version, '1.1.0-alpha.4');
assert.equal(report.stable_release_version, '1.1.0-alpha.4');
...
assert.equal(fixture.workflow_version, '1.1.0-alpha.4');
assert.equal(fixture.release_candidate.release_candidate_version, '1.1.0-alpha.4');
assert.equal(fixture.release_candidate.stable_release_version, '1.1.0-alpha.4');
...
assert.ok(fs.readFileSync('src/research/render-helpers.js','utf8').includes('v1.1.0-alpha.4 · Migration + Privacy Fixture Registry Consolidation'), 'render helper stable badge must not regress to RC copy');
