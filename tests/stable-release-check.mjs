...
assert.equal(release.RELEASE_VERSION, '1.1.0-rc.1');
...
assert.ok(index.includes('v1.1.0-rc.1 · Stable Consolidation + Public Surface Freeze'), 'stable badge missing');
...
assert.ok(migrationSource.includes('1.1.0-rc.1'), 'v1.1.0-rc.1 target must be supported');

const policy = release.releaseCandidatePolicy({ version: '1.1.0-rc.1' });
...
}, { version: '1.1.0-rc.1', externalCiPassed: true, externalCiNotes: 'external CI succeeded' });
assert.equal(report.release_candidate_version, '1.1.0-rc.1');
assert.equal(report.stable_release_version, '1.1.0-rc.1');
...
assert.equal(fixture.release_candidate.release_candidate_version, '1.1.0-rc.1');
...
assert.ok(fs.readFileSync('src/research/render-helpers.js','utf8').includes('v1.1.0-rc.1 · Stable Consolidation + Public Surface Freeze'), 'render helper stable badge must not regress to RC copy');
