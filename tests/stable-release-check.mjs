...
assert.equal(release.RELEASE_VERSION, '1.3.0-alpha.2');
...
assert.ok(index.includes('v1.3.0-alpha.2 · Guided Session UX Compression + Brief Assembly Export QA'), 'stable badge missing');
...
assert.ok(migrationSource.includes('1.3.0-alpha.2'), 'v1.1.0 target must be supported');

const policy = release.releaseCandidatePolicy({ version: '1.3.0-alpha.2' });
...
const report = release.buildReleaseCandidateReport({
  workflow_version: '1.3.0-alpha.2',
...
}, { version: '1.3.0-alpha.2', externalCiPassed: true, externalCiNotes: 'external CI succeeded' });
assert.equal(report.release_candidate_version, '1.3.0-alpha.2');
assert.equal(report.stable_release_version, '1.3.0-alpha.2');
...
assert.equal(fixture.release_candidate.release_candidate_version, '1.3.0-alpha.2');
assert.equal(fixture.release_candidate.stable_release_version, '1.3.0-alpha.2');
...
assert.ok(fs.readFileSync('src/research/render-helpers.js','utf8').includes('v1.3.0-alpha.2 · Guided Session UX Compression + Brief Assembly Export QA'), 'render helper stable badge must not regress to RC copy');
