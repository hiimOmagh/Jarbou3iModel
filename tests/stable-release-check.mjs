...
assert.equal(release.RELEASE_VERSION, '1.0.9');
...
assert.ok(index.includes('v1.0.9 · Hosted Demo Smoke Fixes + Evidence Review'), 'stable badge missing');
...
assert.ok(migrationSource.includes('1.0.9'), 'v1.0.9 target must be supported');

const policy = release.releaseCandidatePolicy({ version: '1.0.9' });
...
}, { version: '1.0.9', externalCiPassed: true, externalCiNotes: 'external CI succeeded' });
assert.equal(report.release_candidate_version, '1.0.9');
assert.equal(report.stable_release_version, '1.0.9');
...
assert.equal(fixture.release_candidate.release_candidate_version, '1.0.9');
...
assert.ok(fs.readFileSync('src/research/render-helpers.js','utf8').includes('v1.0.9 · Hosted Demo Smoke Fixes + Evidence Review'), 'render helper stable badge must not regress to RC copy');
