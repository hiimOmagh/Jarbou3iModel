...
assert.equal(release.RELEASE_VERSION, '1.1.0-alpha.8');
...
assert.ok(index.includes('v1.1.0-alpha.8 · Fixture Registry Payload Compression + Test Organization Audit'), 'stable badge missing');
...
assert.ok(migrationSource.includes('1.1.0-alpha.8'), 'v1.1.0-alpha.8 target must be supported');

const policy = release.releaseCandidatePolicy({ version: '1.1.0-alpha.8' });
...
const report = release.buildReleaseCandidateReport({
  workflow_version: '1.1.0-alpha.8',
...
}, { version: '1.1.0-alpha.8', externalCiPassed: true, externalCiNotes: 'external CI succeeded' });
assert.equal(report.release_candidate_version, '1.1.0-alpha.8');
assert.equal(report.stable_release_version, '1.1.0-alpha.8');
...
assert.equal(fixture.workflow_version, '1.1.0-alpha.8');
assert.equal(fixture.release_candidate.release_candidate_version, '1.1.0-alpha.8');
assert.equal(fixture.release_candidate.stable_release_version, '1.1.0-alpha.8');
...
assert.ok(fs.readFileSync('src/research/render-helpers.js','utf8').includes('v1.1.0-alpha.8 · Fixture Registry Payload Compression + Test Organization Audit'), 'render helper stable badge must not regress to RC copy');
