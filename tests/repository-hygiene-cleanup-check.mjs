...
assert.equal(sample.workflow_version, '1.0.29');
assert.equal(migrationFixture.workflow_version, '1.0.29');
assert.equal(privacyFixture.workflow_version, '1.0.29');
assert.equal(sample.release_notes.release_title, 'v1.0.29 — Final Public Demo Hardening / Release Freeze Audit');
assert.equal(migrationFixture.release_notes.release_title, 'v1.0.29 — Final Public Demo Hardening / Release Freeze Audit');
assert.equal(privacyFixture.release_notes.release_title, 'v1.0.29 — Final Public Demo Hardening / Release Freeze Audit');
...
for (const file of [
  ...
  assert.ok(fs.existsSync(file), `missing v1.0.29 cleanup artifact: ${file}`);
}
...
assert.ok(migrationSource.includes("'1.0.29'"), 'v1.0.29 must remain a supported migration source');
assert.ok(migrationSource.includes("'1.0.19','1.0.20','1.0.21','1.0.22','1.0.23','1.0.24','1.0.25','1.0.26','1.0.27','1.0.28','1.0.29'"), 'v1.0.19 must migrate into v1.0.29');
assert.ok(migrationSource.includes("const TARGET_VERSION = '1.0.29'"));
assert.ok(migrationSource.includes("const MIGRATION_VERSION = '1.0.29'"));

for (const corpus of [manifest, changelog, readme, qaMatrix, roadmap]) {
  assert.ok(corpus.includes('v1.0.29'), 'release corpus missing v1.0.29');
  assert.ok(corpus.includes('Final Public Demo Hardening / Release Freeze Audit'), 'release corpus missing v1.0.29 release title');
}
