...
assert.equal(pkg.version, '1.1.0-alpha.10');
assert.ok(pkg.description.includes('evidence scoring'));
assert.equal(schema.properties.workflow_version.const, '1.1.0-alpha.10');
assert.equal(sample.workflow_version, '1.1.0-alpha.10');
assert.equal(migrationFixture.workflow_version, '1.1.0-alpha.10');
assert.equal(privacyFixture.workflow_version, '1.1.0-alpha.10');
assert.equal(sample.release_notes.release_title, 'v1.1.0-alpha.10 — Hosted Evidence Capture Polish + Visual Artifact Guard');
assert.equal(migrationFixture.release_notes.release_title, 'v1.1.0-alpha.10 — Hosted Evidence Capture Polish + Visual Artifact Guard');
assert.equal(privacyFixture.release_notes.release_title, 'v1.1.0-alpha.10 — Hosted Evidence Capture Polish + Visual Artifact Guard');
...
for (const file of [
  ...
  assert.ok(fixturePathExists(file) || releaseDocExists(file), `missing v1.1.0-alpha.10 cleanup artifact: ${file}`);
}
...
assert.ok(migrationSource.includes("'1.1.0-alpha.10'"), 'v1.1.0-alpha.10 must remain a supported migration source');
assert.ok(migrationSource.includes("'1.0.19','1.0.20','1.0.21','1.0.22','1.0.23','1.0.24','1.0.25','1.0.26','1.0.27','1.0.28','1.0.29','1.0.30','1.1.0-alpha.1','1.1.0-alpha.2','1.1.0-alpha.3','1.1.0-alpha.8','1.1.0-alpha.9','1.1.0-alpha.10'"), 'v1.0.19 must migrate into v1.1.0-alpha.10');
assert.ok(migrationSource.includes("const TARGET_VERSION = '1.1.0-alpha.10'"));
assert.ok(migrationSource.includes("const MIGRATION_VERSION = '1.1.0-alpha.10'"));

for (const corpus of [manifest, changelog, readme, qaMatrix, roadmap]) {
  assert.ok(corpus.includes('v1.1.0-alpha.10'), 'release corpus missing v1.1.0-alpha.10');
  assert.ok(corpus.includes('Hosted Evidence Capture Polish + Visual Artifact Guard'), 'release corpus missing v1.1.0-alpha.10 release title');
}
