...
for (const corpus of [manifest, changelog, readme, qaMatrix, roadmap]) {
  assert.ok(corpus.includes('v1.4.0-alpha.27') || corpus.includes('v1.3.0') || corpus.includes('v1.1.0'), 'release corpus missing current alpha.1, stable baseline, or retained history');
  assert.ok(corpus.includes('Adapter Replay Decision Drilldown + Evidence Trace Links') || corpus.includes('Stable Manual Workflow Release') || corpus.includes('Diagnostic Repair Queue + Export Risk Resolution'), 'release corpus missing current alpha.1, stable, or retained release title');
}
