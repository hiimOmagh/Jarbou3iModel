...
assert.equal(schema.properties.workflow_version.const, '1.1.0-alpha.2');
assert.equal(sample.workflow_version, '1.1.0-alpha.2');
assert.equal(migrationFixture.workflow_version, '1.1.0-alpha.2');
assert.equal(privacyFixture.workflow_version, '1.1.0-alpha.2');
assert.equal(sample.release_notes.release_title, 'v1.1.0-alpha.2 — Expansion Lane Acceptance Criteria Matrix');
assert.equal(migrationFixture.release_notes.release_title, 'v1.1.0-alpha.2 — Expansion Lane Acceptance Criteria Matrix');
assert.equal(privacyFixture.release_notes.release_title, 'v1.1.0-alpha.2 — Expansion Lane Acceptance Criteria Matrix');
...
for (const file of [
  ...
  assert.ok(fs.existsSync(file), `missing v1.1.0-alpha.2 cleanup artifact: ${file}`);
}
...
assert.ok(migrationSource.includes("'1.1.0-alpha.2'"), 'v1.1.0-alpha.2 must remain a supported migration source');
assert.ok(migrationSource.includes("'1.0.19','1.0.20','1.0.21','1.0.22','1.0.23','1.0.24','1.0.25','1.0.26','1.0.27','1.0.28','1.0.29','1.0.30','1.1.0-alpha.1','1.1.0-alpha.2'"), 'v1.0.19 must migrate into v1.1.0-alpha.2');
assert.ok(migrationSource.includes("const TARGET_VERSION = '1.1.0-alpha.2'"));
assert.ok(migrationSource.includes("const MIGRATION_VERSION = '1.1.0-alpha.2'"));

for (const corpus of [manifest, changelog, readme, qaMatrix, roadmap]) {
  assert.ok(corpus.includes('v1.1.0-alpha.2'), 'release corpus missing v1.1.0-alpha.2');
  assert.ok(corpus.includes('Expansion Lane Acceptance Criteria Matrix'), 'release corpus missing v1.1.0-alpha.2 release title');
}
...
assert.ok(pkg.scripts['test:v118:no-browser'].includes('v118-no-browser-suite.mjs'));
