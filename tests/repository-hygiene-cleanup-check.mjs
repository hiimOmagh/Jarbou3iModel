...
assert.equal(schema.properties.workflow_version.const, '1.0.23');
assert.equal(sample.workflow_version, '1.0.23');
assert.equal(migrationFixture.workflow_version, '1.0.23');
assert.equal(privacyFixture.workflow_version, '1.0.23');
assert.equal(sample.release_notes.release_title, 'v1.0.23 — CI Result Review + Browser Evidence Artifact Audit');
assert.equal(migrationFixture.release_notes.release_title, 'v1.0.23 — CI Result Review + Browser Evidence Artifact Audit');
assert.equal(privacyFixture.release_notes.release_title, 'v1.0.23 — CI Result Review + Browser Evidence Artifact Audit');
...
for (const file of [
  ...
  assert.ok(fs.existsSync(file), `missing v1.0.23 cleanup artifact: ${file}`);
}
...
assert.ok(migrationSource.includes("'1.0.23'"), 'v1.0.23 must remain a supported migration source');
assert.ok(migrationSource.includes("'1.0.19','1.0.20','1.0.21','1.0.22','1.0.23'"), 'v1.0.19 must migrate into v1.0.23');
assert.ok(migrationSource.includes("const TARGET_VERSION = '1.0.23'"));
assert.ok(migrationSource.includes("const MIGRATION_VERSION = '1.0.23'"));

for (const corpus of [manifest, changelog, readme, qaMatrix, roadmap]) {
  assert.ok(corpus.includes('v1.0.23'), 'release corpus missing v1.0.23');
  assert.ok(corpus.includes('CI Result Review + Browser Evidence Artifact Audit'), 'release corpus missing v1.0.23 release title');
}
