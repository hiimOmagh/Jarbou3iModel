...
assert.equal(schema.properties.workflow_version.const, '1.1.0-rc.2');
assert.equal(sample.workflow_version, '1.1.0-rc.2');
assert.equal(migrationFixture.workflow_version, '1.1.0-rc.2');
assert.equal(privacyFixture.workflow_version, '1.1.0-rc.2');
assert.equal(sample.release_notes.release_title, 'v1.1.0-rc.2 — Canonical Lock Evidence Bundle + Final Stable Handoff');
assert.equal(migrationFixture.release_notes.release_title, 'v1.1.0-rc.2 — Canonical Lock Evidence Bundle + Final Stable Handoff');
assert.equal(privacyFixture.release_notes.release_title, 'v1.1.0-rc.2 — Canonical Lock Evidence Bundle + Final Stable Handoff');
...
for (const file of [
  ...
  assert.ok(fixturePathExists(file) || releaseDocExists(file), `missing v1.1.0-rc.2 cleanup artifact: ${file}`);
}
...
assert.ok(migrationSource.includes("'1.1.0-rc.2'"), 'v1.1.0-rc.2 must remain a supported migration source');
assert.ok(migrationSource.includes("'1.0.19','1.0.20','1.0.21','1.0.22','1.0.23','1.0.24','1.0.25','1.0.26','1.0.27','1.0.28','1.0.29','1.0.30','1.1.0-alpha.1','1.1.0-alpha.2','1.1.0-alpha.3','1.1.0-alpha.8','1.1.0-alpha.9','1.1.0-alpha.10','1.1.0-rc.2'"), 'v1.0.19 must migrate into v1.1.0-rc.2');
assert.ok(migrationSource.includes("const TARGET_VERSION = '1.1.0-rc.2'"));
assert.ok(migrationSource.includes("const MIGRATION_VERSION = '1.1.0-rc.2'"));

for (const corpus of [manifest, changelog, readme, qaMatrix, roadmap]) {
  assert.ok(corpus.includes('v1.1.0-rc.2'), 'release corpus missing v1.1.0-rc.2');
  assert.ok(corpus.includes('Canonical Lock Evidence Bundle + Final Stable Handoff'), 'release corpus missing v1.1.0-rc.2 release title');
}
