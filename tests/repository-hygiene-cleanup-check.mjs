...
assert.equal(sample.workflow_version, '1.0.30');
assert.equal(migrationFixture.workflow_version, '1.0.30');
assert.equal(privacyFixture.workflow_version, '1.0.30');
assert.equal(sample.release_notes.release_title, 'v1.0.30 — Mobile Header Geometry Lock / Final Public Demo Visual Freeze');
assert.equal(migrationFixture.release_notes.release_title, 'v1.0.30 — Mobile Header Geometry Lock / Final Public Demo Visual Freeze');
assert.equal(privacyFixture.release_notes.release_title, 'v1.0.30 — Mobile Header Geometry Lock / Final Public Demo Visual Freeze');
...
for (const file of [
  ...
  assert.ok(fs.existsSync(file), `missing v1.0.30 cleanup artifact: ${file}`);
}
...
assert.ok(migrationSource.includes("'1.0.30'"), 'v1.0.30 must remain a supported migration source');
assert.ok(migrationSource.includes("'1.0.19','1.0.20','1.0.21','1.0.22','1.0.23','1.0.24','1.0.25','1.0.26','1.0.27','1.0.28','1.0.29','1.0.30'"), 'v1.0.19 must migrate into v1.0.30');
assert.ok(migrationSource.includes("const TARGET_VERSION = '1.0.30'"));
assert.ok(migrationSource.includes("const MIGRATION_VERSION = '1.0.30'"));
...
for (const corpus of [manifest, changelog, readme, qaMatrix, roadmap]) {
  ...
  assert.ok(corpus.includes('Mobile Header Geometry Lock / Final Public Demo Visual Freeze'), 'release corpus missing v1.0.30 release title');
}
