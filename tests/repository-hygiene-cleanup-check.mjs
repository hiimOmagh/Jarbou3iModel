...
for (const corpus of [manifest, changelog, readme, qaMatrix, roadmap]) {
  ...
  assert.ok(corpus.includes('Source-to-Brief Publication Readiness Suite') || corpus.includes('Stable Manual Workflow Release') || corpus.includes('Diagnostic Repair Queue + Export Risk Resolution'), 'release corpus missing current alpha.1, stable, or retained release title');
}
