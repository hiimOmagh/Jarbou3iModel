...
assert.equal(schema.properties.workflow_version.const, '1.0.26');
assert.equal(sample.workflow_version, '1.0.26');
assert.equal(migrationFixture.workflow_version, '1.0.26');
assert.equal(privacyFixture.workflow_version, '1.0.26');
assert.equal(sample.release_notes.release_title, 'v1.0.26 — Release Apply Integrity Gate');
assert.equal(migrationFixture.release_notes.release_title, 'v1.0.26 — Release Apply Integrity Gate');
assert.equal(privacyFixture.release_notes.release_title, 'v1.0.26 — Release Apply Integrity Gate');
...
for (const file of [
  ...
  assert.ok(fs.existsSync(file), `missing v1.0.26 cleanup artifact: ${file}`);
}
...
assert.ok(migrationSource.includes("'1.0.26'"), 'v1.0.26 must remain a supported migration source');
assert.ok(migrationSource.includes("'1.0.19','1.0.20','1.0.21','1.0.22','1.0.23','1.0.24','1.0.25','1.0.26'"), 'v1.0.19 must migrate into v1.0.26');
assert.ok(migrationSource.includes("const TARGET_VERSION = '1.0.26'"));
assert.ok(migrationSource.includes("const MIGRATION_VERSION = '1.0.26'"));

for (const corpus of [manifest, changelog, readme, qaMatrix, roadmap]) {
  assert.ok(corpus.includes('v1.0.26'), 'release corpus missing v1.0.26');
  assert.ok(corpus.includes('Release Apply Integrity Gate'), 'release corpus missing v1.0.26 release title');
}
