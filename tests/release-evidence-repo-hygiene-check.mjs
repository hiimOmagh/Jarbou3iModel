...
const TITLE = 'Release Evidence + Repo Hygiene Verification';
...
const doc = read('docs/v1.0.22-release-evidence-repo-hygiene-verification.md');
const migrations = read('src/research/migrations.js');
const evidenceSpec = read('tests/hosted-demo-browser-evidence.spec.mjs');

assert.equal(pkg.version, VERSION, 'package.json must identify v1.0.22');
assert.equal(lock.version, VERSION, 'package-lock root version must identify v1.0.22');
assert.equal(lock.packages[''].version, VERSION, 'package-lock package root must identify v1.0.22');
assert.equal(schema.properties.workflow_version.const, VERSION, 'schema workflow version must identify v1.0.22');
assert.equal(sample.workflow_version, VERSION, 'sample fixture must identify v1.0.22');
assert.equal(migrationFixture.workflow_version, VERSION, 'migration fixture must identify v1.0.22');
assert.equal(privacyFixture.workflow_version, VERSION, 'privacy fixture must identify v1.0.22');
assert.equal(sample.release_notes.release_title, RELEASE, 'sample release title must identify v1.0.22');
assert.equal(migrationFixture.release_notes.release_title, RELEASE, 'migration fixture release title must identify v1.0.22');
assert.equal(privacyFixture.release_notes.release_title, RELEASE, 'privacy fixture release title must identify v1.0.22');

for (const text of [manifest, notes, changelog, roadmap, qaMatrix, doc]) {
  assert.ok(text.includes('v1.0.22'), 'release corpus must mention v1.0.22');
  assert.ok(text.includes(TITLE), 'release corpus must mention release evidence + repo hygiene verification');
}
...
assert.ok(ciNoBrowser.includes('run_node --check tests/v122-no-browser-suite.mjs'), 'no-browser CI must syntax-check v122 wrapper');
assert.equal(ciNoBrowser.includes('run_node tests/v122-no-browser-suite.mjs'), false, 'no-browser CI must not recursively run v122 wrapper');
...
assert.ok(pkg.scripts['test:v122:no-browser']?.includes('v122-no-browser-suite.mjs'), 'package must expose v122 no-browser suite');
assert.ok(pkg.scripts['test:v122']?.includes('test:ci:browser'), 'package must expose v122 browser gate composition');
...
for (const required of [
  ...
  assert.ok(fs.existsSync(required), `required release evidence artifact missing: ${required}`);
}
...
assert.ok(migrations.includes("const TARGET_VERSION = '1.0.22'"), 'migration target must be v1.0.22');
assert.ok(migrations.includes("'1.0.20','1.0.21','1.0.22'"), 'migration order must preserve v1.0.21 and append v1.0.22');
...
for (const file of ['tests/release-evidence-repo-hygiene-check.mjs', 'tests/v122-no-browser-suite.mjs']) {
